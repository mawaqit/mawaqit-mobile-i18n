// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Urdu (`ur`).
class AppLocalizationsUr extends AppLocalizations {
  AppLocalizationsUr([String locale = 'ur']) : super(locale);

  @override
  String get bottomNavbar_mawaqit => 'مواقیت';

  @override
  String get bottomNavbar_favorites => 'پسندیدہ';

  @override
  String get bottomNavbar_qibla => 'قِبلہ';

  @override
  String get bottomNavbar_search => 'تلاش کریں';

  @override
  String get bottomNavbar_settings => 'ترتیبات';

  @override
  String get settings_ThemeAuto => 'آٹو';

  @override
  String get settings_ThemeDark => 'گہرا پن';

  @override
  String get settings_ThemeLight => 'ہلکا پن';

  @override
  String get settings_TimeFormat => 'وقت کی ترتیب';

  @override
  String get settings_LanguageSelection => 'زبان کا انتخاب';

  @override
  String get settings_Language => 'زبان';

  @override
  String get settings_Theme => 'تھیم';

  @override
  String get settings_TimeFormat12H => '12H';

  @override
  String get settings_TimeFormat24H => '24H';

  @override
  String get settings_PrayerNotifications => 'نماز کی اطلاع';

  @override
  String get settings_PrayerPreNotifications => 'نماز سے پہلے کی اطلاع';

  @override
  String get settings_ShuruqPreNotifications => 'اشراق سے پہلے کی اطلاع';

  @override
  String get settings_Calculations => 'حساب کتاب';

  @override
  String get settings_RateOurApp => 'ہماری ایپ کو ریٹ کریں';

  @override
  String get settings_WhatsNew => 'نیا کیا ہے';

  @override
  String get settings_SupportMawaqit => 'مَواقیت کی حمایت کریں';

  @override
  String get settings_News => 'خبریں';

  @override
  String get settings_MakeADonation => 'عطیہ کریں';

  @override
  String get bismillah => 'بِسمِ اللہ';

  @override
  String get settings_DonateToMawaqit => 'مواقیت کو عطیہ کریں';

  @override
  String get settings_CheckForUpdates => 'اَپ ڈیٹس چیک کریں';

  @override
  String get settings_Credits => 'کریڈٹ';

  @override
  String get settings_AboutUs => 'ہمارے بارے میں';

  @override
  String get settings_language_ar => 'عربی';

  @override
  String get settings_language_bn => 'بنگالی';

  @override
  String get settings_language_fr => 'فرانسیسی';

  @override
  String get settings_language_nl => 'ڈچ';

  @override
  String get settings_language_de => 'جرمن';

  @override
  String get settings_language_es => 'ہسپانوی';

  @override
  String get settings_language_en => 'انگریزی';

  @override
  String get settings_language_it => 'اِطالوی';

  @override
  String get settings_language_tr => 'تُرکی';

  @override
  String get settings_language_ru => 'رُوسی';

  @override
  String get settings_language_id => 'انڈونیشین';

  @override
  String get settings_language_ur => 'اردو';

  @override
  String get settings_language_pl => 'پولش';

  @override
  String get settings_CreditsText =>
      'کاپی رائٹ © 2021 MAWAQIT برائے مواقیت فاؤنڈیشن، جملہ حقوق محفوظ ہیں۔\n\nاس ایپلی کیشن یا اس کے پلگ انز، ایڈ آنز یا مواد کے دانشورانہ املاک اور کاپی رائٹس مواقیت برائے مواقیت ایسوسی ایشن کی ملکیت ہیں۔\n\nیہ ورژن صرف خیراتی تقسیم کے لیے لائسنس یافتہ ہے اور وقف ہے۔ کسی بھی تجارتی استعمال یا تجارتی تقسیم پر سختی سے ممانعت ہے۔\n\nیہ لائسنس اس پروڈکٹ، اس کے کسی بھی حصے، پلگ ان، ایڈ آنز یا مواد سے متعلق کسی بھی حق کی فروخت کی نشاندہی یا شامل نہیں کرتا ہے۔ بلکہ، یہ لائسنس ایپلیکیشن اسٹور کے موجودہ پلیٹ فارم کے ذریعے درخواست کی صرف خیراتی تقسیم کی اجازت دیتا ہے۔\n\nماخذ کوڈ یا بائنری فائلوں کا استعمال کرتے ہوئے ایپلیکیشن کی دوبارہ تقسیم، یا کسی ترمیم کے ساتھ یا اس کے بغیر ریورس انجینئرنگ کا استعمال، خلاف ورزی کرنے والے یا اس کاپی رائٹ کی خلاف ورزی کے ذمہ دار کو قانونی کارروائی کے لیے بے نقاب کرے گا۔\n\nکاپی رائٹ ہولڈر اور ایپلیکیشن کے مالک، مواقیت برائے دی مواقیت ایسوسی ایشن کی تحریری اجازت کے بغیر کوئی بھی فریق ایپل یا گوگل پلے کے آفیشل اسٹورز کے باہر ایپلیکیشن کو دوبارہ تقسیم کرنے کا حقدار نہیں ہے۔ اس ایپلیکیشن کو ان سرکاری اسٹورز کے باہر تقسیم کرنا املاک دانش کے تحفظ کے قانون کی خلاف ورزی ہوگی اور اس کاپی رائٹ کی خلاف ورزی کے ذمہ دار کو قانونی کارروائی کے لیے بے نقاب کرے گا۔\n\nکاپی رائٹ کے مالک سے پیشگی تحریری اجازت حاصل کیے بغیر اس ایپلی کیشن کا نام یا برانڈ یا کاپی رائٹ ہولڈر MAWAQIT کا ٹریڈ مارک یا نام MAWAQIT Foundation کے لیے ایسی مصنوعات کی تشہیر کے لیے استعمال کرنے کی اجازت نہیں ہے جو اس ایپلی کیشن سے ملتی جلتی یا اخذ کی گئی ہوں یا کسی اور پروڈکٹس کو۔\nیہ ایپلیکیشن کاپی رائٹ ہولڈرز اور شیئر ہولڈرز کے ذریعہ \"جیسا ہے\" تقسیم کرنے کا لائسنس یافتہ ہے۔\n\nایپلیکیشن کی ڈویلپر اور مالک کمپنی کوئی واضح یا مضمر وارنٹی نہیں دیتی، بشمول - لیکن اس تک محدود نہیں - کسی خاص مقصد کے لیے ایپلیکیشن کی مناسبیت کے لیے تشہیر اور تشہیر۔ کسی بھی صورت میں، MAWAQIT for The MAWAQIT ایسوسی ایشن، یا شیئر ہولڈرز کسی بھی نقصان کے ذمہ دار نہیں ہیں، براہ راست، بالواسطہ، یا حادثاتی (خاص، احتیاطی، نتیجہ خیز)، بشمول - لیکن ان تک محدود نہیں - استعمال، ڈیٹا یا منافع کا نقصان؛ یا کاروبار میں رکاوٹ، جو بھی وجوہات اور ذمہ داریاں ہوں، یہ ذمہ داریاں معاہدہ، مطلق یا اذیت (بشمول لاپرواہی یا دوسری صورت میں) ہوں جو اس ایپلی کیشن کے استعمال سے - کسی نہ کسی طریقے سے اٹھائی گئی ہیں۔\n\nدرخواست پر کسی تیسرے فریق کے کاپی رائٹس ان کے ہولڈر کے لیے محفوظ ہیں۔\n\nتصاویر اور پس منظر\n\nکاپی رائٹ شٹر اسٹاک کو جاتا ہے۔\nشبیہیں - CC BY 4.0 لائسنس\n\nکاپی رائٹ Flaticons کو جاتا ہے۔\nMIT لائسنس تمام غیر فونٹ اور غیر آئیکن فائلوں پر لاگو ہوتا ہے۔\nفونٹس - SIL OFL 1.1 لائسنس\n\nکاپی رائٹ گوگل فونٹس کو جاتا ہے (روبوٹو فونٹس)\nکاپی رائٹ Apple, Cupertino پیکیجز\nکوڈ، پیکجز اور فریم ورک\n\nFlutter by Google\n\nپیکیجز: http, geolocator, flutter_platform_widgets, shared_preferences, provider, flutter_local_notifications, audioplayers, toggle_switch, cupertino_icons, marquee_widget, intl, flutter_switch, json_annotation';

  @override
  String get settings_AboutUsText =>
      'یہ ایپلیکیشن مواقیت ایسوسی ایشن نے تیار کی ہے۔\n\nمواقیت ایسوسی ایشن فرانس میں 2021 میں بنائی گئی ایک غیر منافع بخش تنظیم ہے۔\nہمارا مقصد آئی ٹی اور ڈیجیٹل ٹیکنالوجی جیسے سافٹ ویئر اور ویب سائٹس سے متعلق جدید حل تیار کرنا ہے۔ ڈیجیٹل حل اور آئی ٹی ٹولز میں سکھائیں اور تربیت دیں۔\n\nMawaqit Ecosystem ہماری فلیگ شپ پروڈکٹ ہے اور اس کی کہانی 2016 میں شروع ہوئی، جس میں مساجد کے لیے وقف بڑی اسکرینوں پر ایک ایپلیکیشن، پھر ایک موبائل ایپ، ہوم اسسٹنٹ ایڈ آن اور ایک الیکسا سکل وغیرہ۔\n\nمواقیت آپ کو نماز کے اوقات کو ٹریک کرنے اور ان کا نظم کرنے کا ایک نیا طریقہ پیش کرتا ہے، درحقیقت ہم ایک اینڈ ٹو اینڈ سسٹم پیش کرتے ہیں جو مسجد مینیجرز کو ایک آن لائن ٹول پیش کرتا ہے جو 24/24 گھنٹے دستیاب ہے جس سے وہ ٹائم ٹیبل، مسجد کی خبروں اور دیگر بہت سی خصوصیات کا انتظام کر سکتے ہیں۔ مومنین، اپنی طرف سے، ایک موبائل ایپلیکیشن سے فائدہ اٹھاتے ہیں جس کی مدد سے وہ اپنی پسندیدہ مسجد کے صحیح اور تخمینی ٹائم ٹیبل کے ساتھ ساتھ خبروں اور دیگر افعال جیسے کہ جغرافیائی محل وقوع کے لحاظ سے مسجد کی تلاش، اذان، قبلہ ... وغیرہ۔ ہم نے وشوسنییتا اور معیار کو اپنی اہم اقدار بنا لیا ہے۔ ہماری خواہش واضح ہے: ٹیکنالوجی اور ڈیزائن کی بدولت اپنی مساجد کے لیے بہترین سروس فراہم کرنا۔\n\nاگر آپ کو یہ ایپلی کیشن پسند آئی ہے تو براہ کرم اسٹور کے بارے میں اپنی رائے دیں۔';

  @override
  String get fajr => 'فجر';

  @override
  String get shuruq => 'اشراق';

  @override
  String get duhr => 'ظہر';

  @override
  String get asr => 'عصر';

  @override
  String get maghrib => 'مغرب';

  @override
  String get isha => 'عشاء';

  @override
  String get sabah => 'صبح';

  @override
  String get settings_PrayerNotification_NoNotification =>
      'آپ نے فی الوقت اس نماز کی اطلاع نہ دینے کا انتخاب کیا ہوا ہے.';

  @override
  String get settings_PrayerNotification_Bip =>
      'اطلاع پر بیپ رنگ ٹون ایپلی کیشن۔';

  @override
  String get adhan => 'اذان';

  @override
  String get adhan_Makkah_fajr => ' (فجر)مکہ';

  @override
  String get adhan_Madinah => 'مدینہ';

  @override
  String get adhan_Qods => 'قدس';

  @override
  String get adhan_AlAfassy => 'العفاسی';

  @override
  String get adhan_AlAfassy_fajr => 'عبدالباسط (فجر)';

  @override
  String get adhan_Muhammad_jazy => 'محمد جازی';

  @override
  String get adhan_AbdAlBassat => 'عبدالباسط';

  @override
  String get adhan_type =>
      'اذان کی ایک قسم کا انتخاب کریں۔ یہ انتخاب ہر اس نماز میں لاگو ہو گا جس میں اذان کی اطلاع ہو گی۔';

  @override
  String get settings_prayer_Notification_None => 'کوئی نہیں۔';

  @override
  String get prayer_Notification_NoNotification =>
      'آپ نے اس نماز کے لیے کوئی اطلاع مقرر نہیں کی ہے۔';

  @override
  String get prayer_PreNotification_Description =>
      'یہ ترتیب آپ کو نماز سے پہلے ایک اطلاع کی وضاحت کرنے کی اجازت دیتی ہے۔';

  @override
  String get search_TryParis => 'مسجد کی تلاش کریں۔ مثال: پاریس';

  @override
  String get search_ByGPS => 'GPS کے ذریعے تلاش کریں۔';

  @override
  String get search_HelpText =>
      'آپ یہ لکھ کر مسجد تلاش کر سکتے ہیں:\n- مسجد کا نام\n- انجمن کا نام\n- شہر، پتہ، ملک... وغیرہ\n\nیا آپ آس پاس کی مساجد تلاش کرنے کے لیے لوکیشن بٹن کو دبا سکتے ہیں۔';

  @override
  String get search_MaxFavMosquesReached =>
      'آپ اپنی پسندیدہ مساجد کی حد تک پہنچ چکے ہیں۔';

  @override
  String get search_iOs14_message =>
      'iOS 14 پر GPS کا استعمال کرتے ہوئے تلاش کرنے میں کچھ وقت لگ سکتا ہے، براہ کرم انتظار کریں۔';

  @override
  String get search_error => 'اوہ! ایک اندرونی خرابی پیش آ گئی ہے۔';

  @override
  String get search_your_location => 'آپ کا مقام';

  @override
  String get search_location_disabled =>
      'براہِ کرم مسجد کو نام یا شہر کے ذریعے تلاش کریں، یا اپنی ترتیبات میں جا کر مقام تک رسائی کو دستی طور پر فعال کریں.';

  @override
  String get search_settings => 'Settings';

  @override
  String get no_services_text => 'مساجد نے یہ معلومات فراہم نہیں کیں';

  @override
  String get welcome => 'خوش آمدید';

  @override
  String get mawaqit => 'مواقيت';

  @override
  String get connecting => 'مسلمانوں کو جوڑنا';

  @override
  String get to_mosques => 'مساجد سے';

  @override
  String get get_started => 'شروع کریں';

  @override
  String get get_prayer_notifications_for => 'نماز کے لیے اطلاعات حاصل کریں';

  @override
  String get turn_on_notifications => 'نماز کے نوٹیفیکیشنز کو آن کریں';

  @override
  String get no_thanks => 'نہیں شکریہ';

  @override
  String get notification_denied =>
      'آپ نے نوٹیفکیشن اجازتوں کو مسترد کر دیا ہے، اس لیے آپ کو کوئی نوٹیفکیشن موصول نہیں ہوگا۔';

  @override
  String get at_least_one_mosque =>
      'جاری رکھنے کے لئے کم از کم ایک مسجد شامل کریں';

  @override
  String get set_up_your_prayer => 'اپنی نماز قائم کریں';

  @override
  String get notifications => 'اطلاعات';

  @override
  String get mosques_around_you => 'اپنے اردگرد کی مساجد';

  @override
  String get mosques_in_the_selected_area => 'منتخب علاقے میں مساجد';

  @override
  String get mosques_with => 'کے ساتھ مساجد';

  @override
  String get allow_mawaqit_to_access_my_location =>
      'مواقیت کو میرے مقام تک رسائی کی اجازت دیں۔ ';

  @override
  String get enable_location_service_from_app_settings => 'مقام فعال کریں';

  @override
  String get the_request_timeout => 'درخواست کا وقت ختم ہو گیا۔';

  @override
  String get no_internet_connection => 'انٹرنیٹ کنکشن نہیں ہے۔';

  @override
  String get try_again => 'دوبارہ کوشش کریں';

  @override
  String get virtual_mosque => 'ورچوئل مسجد';

  @override
  String get no_mosque_in_selected_area =>
      'منتخب علاقے میں کوئی مسجد نہیں مل سکی';

  @override
  String get no_mosque_with => 'کے ساتھ کوئی مسجد نہیں مل سکی';

  @override
  String get retry => 'دوبارہ کوشش کریں۔';

  @override
  String get search_here => 'یہاں تلاش کریں';

  @override
  String get enter_at_least_3_char =>
      'تلاش کرنے کے لیے کم از کم 3 حروف درج کریں۔';

  @override
  String get jumua1 => 'جمعہ 1';

  @override
  String get prayer_times => 'نماز کے اوقات';

  @override
  String get remove_this_city => 'اس شہر کو ہٹا دیں۔';

  @override
  String get add_this_city => 'اس شہر کو شامل کریں۔';

  @override
  String get add_this_mosque => 'اس مسجد کو شامل کریں۔';

  @override
  String get remove_this_mosque => 'اس مسجد کو ہٹا دو';

  @override
  String get services => 'خدمات';

  @override
  String get share => 'بانٹیں';

  @override
  String get itinerary => 'سفر نامہ';

  @override
  String get call => 'کال کریں۔';

  @override
  String get dnd => 'پریشان نہ کرو';

  @override
  String get battery_optimize => 'بیٹری کی اصلاح';

  @override
  String get power_saver => 'طاقت بچانے والا';

  @override
  String get permission_message1 =>
      'ہمیں تاخیر کی اطلاع ملی ہے، براہ کرم آف کر دیں۔';

  @override
  String get permission_message2 => 'بروقت اطلاع موصول کرنے کے لیے۔';

  @override
  String get notifications_delay_detected => 'اطلاعات میں تاخیر کا پتہ چلا';

  @override
  String get open_setting => 'ترتیبات کھولیں۔';

  @override
  String get noJumua => 'کوئی جمعہ نماز نہیں ہے۔';

  @override
  String get jumua => 'جمعہ';

  @override
  String get qibla_sensor =>
      'آپ کے آلہ میں وہ سنسر جس کا استعمال مواقيت قبلہ کا تعین کرنے کے لئے کرتی ہے ، وہ متوقع طور پر کام نہیں کر رہا ہے ، جو ہماری قبلہ کی سمت دکھانے کی صلاحیت پر اثر انداز ہو رہا ہے۔';

  @override
  String get connection_not_found =>
      'کمپاس کیلیبریشن سیٹنگ غیر فعال ہے۔ براہ کرم یقینی بنائیں کہ آپ کا کنکشن آن ہے اور سمت تبدیل کرکے دوبارہ کوشش کریں۔';

  @override
  String get currently_located_at => 'اس وقت واقع ہے';

  @override
  String get facing_makkah => 'آپ مکہ کی طرف رخ کر رہے ہیں';

  @override
  String get not_facing_Makkah => 'آپ مکہ کی طرف رخ نہیں کر رہے ہیں';

  @override
  String get qibla_Error_retry => 'دوبارہ کوشش کریں';

  @override
  String get qibla_Error_LocationDenied =>
      'مقام کی خدمت کی اجازت مسترد کر دی گئی۔';

  @override
  String get qibla_Error_LocationDeniedForever =>
      'مقام کی خدمت ہمیشہ کے لیے مسترد کر دی گئی!';

  @override
  String get qibla_Error_PleaseEnable => 'مقام کی خدمت کو فعال کریں۔';

  @override
  String get qibla_Direction => 'قبلہ\nسمت';

  @override
  String get favorites_FavoriteMosques => 'پسندیدہ مساجد';

  @override
  String get favorites_Error => 'کچھ غلط ہو گیا...';

  @override
  String get announcement_Image => 'تصویر';

  @override
  String get announcement_Type_Video => 'ویڈیو';

  @override
  String get announcement_Type_FlashInfo => 'فلیش کی معلومات';

  @override
  String get mosque_DonateText1 => 'عطیہ';

  @override
  String get mosque_DonateText2 => 'مسجد تک';

  @override
  String get mosque_Error_CouldNotLaunch => 'لانچ نہیں ہو سکا';

  @override
  String get mosque_PrayerTimes => 'نماز کے اوقات';

  @override
  String get mosque_Website => 'ویب سائٹ';

  @override
  String get mosque_Address => 'پتہ';

  @override
  String get mosque_Phone => 'فون';

  @override
  String get mosque_Email => 'ای میل';

  @override
  String get mosque_ByMawaqit => 'بذریعہ مواقیت https://mawaqit.net';

  @override
  String get mosque_Facilities_Parking => 'پارکنگ';

  @override
  String get mosque_Facilities_DisabledAccess => 'معذورین کی رسائی';

  @override
  String get mosque_Facilities_AblutionsRoom => 'وضو کا کمرہ';

  @override
  String get mosque_Facilities_WomanSpace => 'عورتوں کی جگہ';

  @override
  String get mosque_Facilities_AdultsCourse => 'بالغوں کے کورس';

  @override
  String get mosque_Facilities_ChildrenCourses => 'بچوں کے کورس';

  @override
  String get mosque_Facilities_SalatAlAid => 'عید کی نماز';

  @override
  String get mosque_Facilities_SalatAlJanaza => 'نماز جنازہ';

  @override
  String get mosques_Facilities_IftarRamadan => 'افطار رمضان';

  @override
  String get mosque_facilities_Info =>
      'یہ تمام معلومات مسجد کے انچارج لوگ بھرتے ہیں۔';

  @override
  String get in_ => 'میں';

  @override
  String get imsak => 'اِمسَک';

  @override
  String get eid => 'عید کی نماز';

  @override
  String get minutes => 'منٹس';

  @override
  String get jumua2 => 'جمعہ 2';

  @override
  String get please_update =>
      'MAWAQIT کا ایک نیا ورژن دستیاب ہے، براہ کرم اپ ڈیٹ کریں۔';

  @override
  String get update => 'اپ ڈیٹ';

  @override
  String get tutorial_skip => 'ٹیوٹوریل کو چھوڑیں۔';

  @override
  String get tutorial_next => 'اگلے';

  @override
  String get tutorial_title_main_mosque => 'آپ کی مرکزی مسجد';

  @override
  String get tutorial_text_main_mosque =>
      'آپ کی مرکزی مسجد یہاں دکھائی دیتی ہے۔\nآپ اپنی پسندیدہ مساجد کے درمیان سوئچ کرنے کے لیے بائیں اور دائیں سلائیڈ کر سکتے ہیں۔';

  @override
  String get tutorial_title_countdown => 'اگلی نماز کا اشارہ';

  @override
  String get tutorial_text_countdown =>
      'یہ الٹی گنتی آپ کو اگلی نماز سے پہلے باقی وقت دکھاتی ہے۔';

  @override
  String get tutorial_title_mosque_info => 'مسجد کی معلومات';

  @override
  String get tutorial_text_mosque_info =>
      'آپ کی مسجد کے بارے میں سب کچھ: سہولیات، پتہ، فون، میل، عطیہ وغیرہ۔';

  @override
  String get tutorial_title_announcements => 'مسجد کے پیغامات اور اعلانات';

  @override
  String get tutorial_text_announcements =>
      'آپ کی مسجد سے پیغامات اور اعلانات کے بارے میں اطلاعات یہاں ظاہر ہوں گی۔\nجب آپ اس بٹن کو تھپتھپاتے ہیں، تو آپ کو پوری تفصیلات نظر آئیں گی۔ یہ اعلانات ایک فلیش پیغام، ایک تصویر، یا ویڈیو بھی ہو سکتے ہیں۔';

  @override
  String get onboarding_title_1 => 'اپنی زبان منتخب کریں';

  @override
  String get onboarding_text_1 =>
      'نیچے دی گئی فہرست سے اپنی پسند کی زبان منتخب کریں۔';

  @override
  String get onboarding_title_2 => 'مواقیت میں خوش آمدید';

  @override
  String get onboarding_text_2 =>
      'مواقیت ایک ماحولیاتی نظام ہے جو آپ کو نماز کے اوقات کو ٹریک کرنے اور ان کا انتظام کرنے کا ایک نیا طریقہ فراہم کرتا ہے، آپ کے ہاتھ میں موجود ایپلی کیشن اس ماحولیاتی نظام کا حصہ ہے، یہ آپ کو مساجد تلاش کرنے اور اس کی خبروں اور نماز کے اوقات پر عمل کرنے کی اجازت دیتا ہے۔';

  @override
  String get onboarding_title_3 => 'ایک Dark/Light تھیم منتخب کریں۔';

  @override
  String get onboarding_text_3 =>
      'چاہے آپ کم روشنی والے ماحول میں ہوں یا اپنے فون پر گہرے پس منظر کے رنگوں کو ترجیح دیں، آپ بہتر بصری سکون کے لیے ڈارک موڈ کا فائدہ اٹھا سکتے ہیں۔';

  @override
  String get onboarding_title_4 => 'اپنی مرکزی مسجد کو شامل کریں۔';

  @override
  String get onboarding_text_4 =>
      'براہ کرم نیچے دی گئی فہرست سے اپنی مرکزی مسجد کا انتخاب کریں یا اپنے موجودہ مقام کے قریب مسجد تلاش کریں۔';

  @override
  String get loading => 'لوڈ ہو رہا ہے۔';

  @override
  String get loading_please_wait => 'لوڈ ہو رہا ہے انتظار کرے';

  @override
  String get loading_loaded => 'لوڈنگ مکمل';

  @override
  String get favorites_edit => 'پسندیدہ مساجد میں ترمیم کریں۔';

  @override
  String get minutes_to_athan => 'اتھن کے لیے منٹ';

  @override
  String get show_tutorial => 'ٹیوٹوریل کا جائزہ لیں۔';

  @override
  String get onboarding_chose_mosque_please =>
      'براہ کرم تلاش کی فہرست میں سے ایک مسجد کا انتخاب کریں۔';

  @override
  String get done => 'ہو گیا';

  @override
  String get ok => 'ٹھیک ہے';

  @override
  String get km => 'کلومیٹر';

  @override
  String get notification_open_app_title => 'ایپ کھولیں۔';

  @override
  String get notification_open_app_body =>
      'اطلاعات کو برقرار رکھنے کے لیے براہ کرم ایپ کھولیں۔';

  @override
  String get tutorial_title_notification => 'دعا کی اطلاع کا انتخاب کریں۔';

  @override
  String get tutorial_text_notification =>
      'یہاں آپ نوٹیفکیشن کی ایک قسم، ایتھن، بِپ یا خاموش منتخب کر سکتے ہیں۔';

  @override
  String get search_no_result =>
      'مواقیت سفیر بنیں اور اپنی مسجد میں ہمارا حل پیش کریں، یہ مفت ہے۔\nhttps://mawaqit.net';

  @override
  String get rate_popin_title => 'MAWAQIT شرح';

  @override
  String get rate_popin_message =>
      'اگر آپ کو یہ ایپ پسند ہے، تو براہ کرم اس کا جائزہ لینے کے لیے اپنا تھوڑا سا وقت نکالیں!\nیہ واقعی ہماری مدد کرتا ہے۔';

  @override
  String get rate_popin_rate => 'شرح';

  @override
  String get rate_popin_no => 'نہیں شکریہ';

  @override
  String get rate_popin_later => 'شاید بعد میں';

  @override
  String get message_see_all =>
      'آپ اس مسجد کے تمام پیغامات اسکرین کے اوپری حصے میں موجود اسپیکر پر کلک کر کے دیکھ سکتے ہیں۔';

  @override
  String get adhan_Makkah => 'مکہ';

  @override
  String get tutorial_title_iqama => 'اقامہ';

  @override
  String get tutorial_text_iqama =>
      'یہاں آپ مسجد میں اذان اور اقامت کے درمیان کا دورانیہ دیکھ سکتے ہیں۔';

  @override
  String get calendar_tooltip_download => 'کیلنڈر ڈاؤن لوڈ کریں۔';

  @override
  String get calendar_download_permission_refused =>
      'ڈاؤن لوڈ کی اجازت یقینی طور پر مسترد کر دی گئی، براہ کرم ڈاؤن لوڈ کی اجازت دیں۔';

  @override
  String get calendar_downloading => 'کیلنڈر ڈاؤن لوڈ ہو رہا ہے...';

  @override
  String get location_service_disabled =>
      'لوکیشن سروس غیر فعال ہے، براہ کرم اسے فعال کریں۔';

  @override
  String get location_permission_denied =>
      'مقام کی اجازت نامنظور، براہ کرم اجازت دیں۔';

  @override
  String get location_permission_denied_forever =>
      'مقام کی اجازت ہمیشہ کے لیے مسترد کر دی گئی، براہ کرم ترتیبات کے صفحہ میں اس کی اجازت دیں۔';

  @override
  String get at_least_one_mosque_error =>
      'آپ کے پاس کم از کم ایک مسجد ہونی چاہئے۔';

  @override
  String get and => 'اور';

  @override
  String get continue_to_app => 'جاری رہے';

  @override
  String get add => 'Add';

  @override
  String get mosque => 'Mosque';

  @override
  String get on_boarding_nearest_mosque => 'آئیے قریب ترین مسجد تلاش کریں';

  @override
  String get on_boarding_nearest_mosque_text =>
      'آپ کے جغرافیائی مقام کے مطابق آپ کے آس پاس پائی جانے والی مساجد یہ ہیں۔';

  @override
  String get search_no_result_for_text => 'ہمیں اس کے لیے کوئی نتیجہ نہیں ملا';

  @override
  String get surah_audio_downloading => 'سورۃ کی آڈیو ڈاؤن لوڈ ہو رہی ہے';

  @override
  String get msg_internet_connection =>
      'آپ کے پاس انٹرنیٹ کنکشن موجود نہیں ہے۔';

  @override
  String get read_quran => 'قرآن پڑھیں';

  @override
  String get ayahs => 'آیات';

  @override
  String get chapter => 'سورت';

  @override
  String get meaning => 'معنی';

  @override
  String get al_quran => 'القرآن';

  @override
  String get surah_information => 'سورت کی معلومات';

  @override
  String get surah_number => 'سورت نمبر';

  @override
  String get quran_browsing => 'براؤزنگ';

  @override
  String get quran_search_placeholder => 'سورہ کے نام، نمبر سے تلاش کریں';

  @override
  String get quran_verse_search => 'آیت کی تلاش';

  @override
  String get quran_translation => 'ترجمہ';

  @override
  String get quran_mushaf => 'مصحف';

  @override
  String get quran_juzz => 'پارہ';

  @override
  String get ayah => 'Ayah';

  @override
  String get quran_verses => 'آیاتہا';

  @override
  String get surah => 'سورت';

  @override
  String get continue_reading => 'پڑھنا جاری رکھیں';

  @override
  String get bookmarks => 'بک مارکس';

  @override
  String get juzz => 'Juzz';

  @override
  String get quran_page => 'صفحہ';

  @override
  String get quran_turning_page => 'ورق پلٹنا';

  @override
  String get quran_from_right_to_left => 'دائیں سے بائیں';

  @override
  String get quran_from_left_to_right => 'بائیں سے دائیں';

  @override
  String get quran_from_top_to_bottom => 'اوپر سے نیچے';

  @override
  String get quran_from_bottom_to_top => 'نیچے سے اوپر';

  @override
  String get quran_pages_color => 'صفحات کا رنگ';

  @override
  String get english_tr => 'انگریزی';

  @override
  String get french_tr => 'فرانسیسی';

  @override
  String get spanish_tr => 'ہسپانوی';

  @override
  String get muhammad_hamidullah => 'محمد حمید اللہ';

  @override
  String get abdullah_yusuf_ali => 'عبداللہ یوسف علی';

  @override
  String get sheikh_isa_garcia => 'شیخ عیسیٰ گارسیا';

  @override
  String get reciters => 'قاری';

  @override
  String get favorite_reciters => 'پسندیدہ قراء';

  @override
  String get surahs_downloaded => 'ڈاؤن لوڈ کی گئی سورتیں';

  @override
  String get surahs_chapter => 'سورتیں - ابواب';

  @override
  String get my_likes => 'میری پسندیدہ';

  @override
  String get cancel => 'منسوخ کریں';

  @override
  String get next => 'اگلے';

  @override
  String get search_surah => 'سورت تلاش کریں';

  @override
  String get not_downloaded_recitation =>
      'آپ نے ابھی تک کوئی تلاوت ڈاؤن لوڈ نہیں کی ہے۔';

  @override
  String get not_favorite_reciter =>
      'آپ نے ابھی تک کسی قاری کو پسندیدہ نشان زد نہیں کیا ہے۔';

  @override
  String get search_reciter => 'قاری تلاش کریں';

  @override
  String get al_quran_al_kareem => 'القرآن الکریم';

  @override
  String get surahs => 'سورتیں';

  @override
  String get chapters => 'ابواب';

  @override
  String get calculation_info =>
      'اس حصے کی ترتیبات صرف ان نماز کے اوقات سے متعلق ہیں جو آپ کے جغرافیائی مقام کی بنیاد پر حساب کیے جاتے ہیں، اور ان کا مساجد کے نماز کے اوقات سے کوئی تعلق نہیں ہے.';

  @override
  String get calculation_madhab => 'فقہ - مذہب';

  @override
  String get calculation_madhab_shafii => 'شافعی، حنبلی اور مالکی';

  @override
  String get calculation_madhab_shafii_countries =>
      'اردن، فلسطین، یمن، صومالیہ، اریٹیریا، ملائیشیا، انڈونیشیا، برونائی، جنوبی مصر، سعودی عرب، قطر، شمالی افریقہ، مغربی افریقہ، متحدہ عرب امارات، کویت...';

  @override
  String get calculation_madhab_hanafi => 'Hanafi';

  @override
  String get calculation_madhab_hanafi_countries =>
      'بلادِ شام، پاکستان، افغانستان، بھارت، بنگلہ دیش، عراق، ترکی، روس، چین، بلقان...';

  @override
  String get calculation_methods => 'اوقاتِ نماز کے حساب کے طریقے';

  @override
  String get calculation_methods_france => 'مسلم آف فرانس (سابق UOIF)';

  @override
  String get calculation_methods_algeria =>
      'الجزائر کی وزارتِ مذہبی امور و اوقاف';

  @override
  String get calculation_methods_morocco =>
      'مراکش کی وزارتِ اوقاف و اسلامی امور';

  @override
  String get calculation_methods_muslim_world => 'رابطہ عالمِ اسلامی';

  @override
  String get calculation_methods_egyptian => 'مصری جنرل اتھارٹی برائے سروے';

  @override
  String get calculation_methods_karachi => 'جامعہ علومِ اسلامیہ، کراچی';

  @override
  String get calculation_methods_umm_al_qura => 'جامعہ اُمّ القریٰ، مکہ مکرمہ';

  @override
  String get calculation_methods_dubai => 'خلیجی خطہ';

  @override
  String get calculation_methods_moon => 'رویتِ ہلال کمیٹی';

  @override
  String get calculation_methods_north_america =>
      'اسلامک سوسائٹی آف نارتھ امریکہ';

  @override
  String get calculation_methods_kuwait => 'کویت';

  @override
  String get calculation_methods_qatar => 'قطر';

  @override
  String get calculation_methods_singapore => 'سنگاپور';

  @override
  String get calculation_methods_turkey => 'ترکی';

  @override
  String get calculation_methods_tehran =>
      'تہران یونیورسٹی کا ادارۂ ارضی طبیعیات';

  @override
  String get calculation_methods_russia =>
      'روس میں مسلمانوں کی روحانی انتظامیہ';

  @override
  String get calculation_methods_custom => 'حسبِ ضرورت زاویے';

  @override
  String get calculation_high_latitude => 'بلند عرض البلد';

  @override
  String get calculation_high_latitude_none => 'کوئی نہیں';

  @override
  String get calculation_high_latitude_middle => 'آدھی رات';

  @override
  String get calculation_high_latitude_one_seventh => 'رات کا ایک ساتواں حصہ';

  @override
  String get calculation_high_latitude_angle_based => 'زاویہ پر مبنی طریقہ';

  @override
  String get calculation_adjustment => 'دستی تصحیح';

  @override
  String get calculation_adjustment_subtitle =>
      'نماز کے اوقات میں تبدیلی اور حسبِ ضرورت زاویے';

  @override
  String get calculation_adjustment_prayers => 'نماز کے اوقات میں تبدیلیاں';

  @override
  String get calculation_adjustment_prayers_minutes => 'منٹ';

  @override
  String get calculation_adjustment_angles => 'حسبِ ضرورت زاویے';

  @override
  String get calculation_prayer_custom_angle => 'زاوية مخصصة لـ';

  @override
  String get error_download_quran_failed => 'قرآن پاک ڈاؤن لوڈ کرنے میں ناکام';

  @override
  String get downloading_quran => 'قرآن ڈاؤن لوڈ ہو رہا ہے...';

  @override
  String get processing => 'پروسیسنگ...';

  @override
  String get copied_to_clipboard => 'آپ کے کلپ بورڈ پر کاپی کیا گیا!';

  @override
  String get download_quran_popup_title => 'آخری ٹچ اپس';

  @override
  String get download_quran_popup_downloading_1 =>
      'ہمیں قرآن کے صفحات ڈاؤن لوڈ کرنے کی ضرورت ہے، اس میں صرف چند سیکنڈ لگیں گے۔ ہم آپ کا موبائل ڈیٹا استعمال کریں گے، ضرورت پڑنے پر Wi-Fi پر سوئچ کریں۔';

  @override
  String get download_quran_popup_downloading_2 =>
      'ہم صرف آپ کے لیے قرآن تیار کر رہے ہیں، براہ کرم چند سیکنڈ انتظار کریں!';

  @override
  String get download_quran_popup_downloading_3 =>
      'قرآن کی خوبصورتی دریافت کریں! اس کی لازوال حکمت تک رسائی حاصل کرنے کے لیے \'ڈاؤن لوڈ\' کو دبائیں اور اس کے الہی الفاظ سے متاثر ہوں۔';

  @override
  String get download_quran_popup_confirm_cancel =>
      'کیا آپ واقعی ڈاؤن لوڈ کو منسوخ کرنا چاہتے ہیں؟ اس ڈیٹا کے بغیر آپ قرآن نہیں پڑھ سکیں گے، صفحات دستیاب نہیں ہوں گے۔';

  @override
  String get download_quran_popup_cancelled =>
      'کچھ غلط ہو گیا، براہ کرم ڈاؤن لوڈ بٹن کو دوبارہ دبائیں!';

  @override
  String get download => 'ڈاؤن لوڈ کریں';

  @override
  String get continue_download => 'ڈاؤن لوڈ جاری رکھیں';

  @override
  String get cancel_download => 'ڈاؤن لوڈ منسوخ کریں';

  @override
  String get download_completed => 'ڈاؤن لوڈ مکمل';

  @override
  String get download_failed => 'ڈاؤن لوڈ ناکام ہو گیا';

  @override
  String get download_pages => 'صفحات ڈاؤن لوڈ کریں';

  @override
  String get notification => 'نوٹیفکیشن';

  @override
  String get notification_not_supported_below_7 =>
      'آپ کو اذان یا نماز کے اوقات کی اطلاعات موصول نہیں ہو سکیں کیونکہ آپ کا موبائل Android 7 یا اس سے کم ورژن استعمال کرتا ہے۔';

  @override
  String get battery_optimization => 'بیٹری کی بچت کی ترتیبات';

  @override
  String get battery_optimization_subtitle =>
      'اگر آپ بغیر کسی رکاوٹ کے نماز کی اطلاع حاصل کرنا چاہتے ہیں، تو آپ کو MAWAQIT کے لیے بیٹری کی اصلاح کو بند کرنا ہوگا۔';

  @override
  String get allow => 'اجازت دیں';

  @override
  String get deny => 'انکار کریں';

  @override
  String get force_notification => 'زبردستی اطلاعات';

  @override
  String get tutorial_text_foreground_service =>
      'اس پر ٹیپ کرنے سے آپ \'فور گراؤنڈ سروس\' کو آن یا آف کر سکتے ہیں۔';

  @override
  String get enable => 'آن';

  @override
  String get disable => 'بند';

  @override
  String get do_not_show_again => 'اسے دوبارہ مت دکھانا';

  @override
  String get force_notification_title =>
      'MAWAQIT اس بات کو یقینی بنانے کے لیے سرگرم ہے کہ نماز کی اطلاعات کو صحیح وقت پر متحرک کیا جائے۔';

  @override
  String get force_notification_body =>
      'بے شک نماز مومنوں پر مقررہ اوقات میں فرض کی گئی ہے۔ نیسا(103)';

  @override
  String get force_notification_dialog1_body =>
      'اس آپشن کو فعال کرنے سے، ایپلی کیشن فعال رہے گی تاکہ اس بات کو یقینی بنایا جا سکے کہ نماز کی اطلاعات کو بغیر کسی تاخیر کے صحیح وقت پر متحرک کیا جائے۔ MAWAQIT ایپلی کیشن صرف اسی مقصد کے لیے فعال ہو گی، اور آپ کسی بھی وقت سیٹنگز سے اس آپشن کو آف کر سکتے ہیں۔';

  @override
  String get force_notification_dialog2_body =>
      '\'فورس نوٹیفیکیشنز\' کو فعال کرنے کے بعد نوٹیفکیشن ٹرے کو نیچے سوائپ کریں آپ کو یہ اطلاع وہاں نظر آئے گی۔';

  @override
  String get notification_debugging => 'نوٹیفکیشن ڈیبگنگ';

  @override
  String get notification_debugging_details =>
      'نوٹیفکیشن ڈیبگنگ فعال ہے۔ اگر آپ اطلاع کی ترتیبات کو تبدیل کرتے ہیں، تو ڈیبگنگ ڈیٹا ضائع ہو جائے گا، اور ڈیبگنگ غیر فعال ہو جائے گی۔';

  @override
  String get skip => 'چھوڑ دو';

  @override
  String get agree => 'متفق';

  @override
  String get return_no_search => 'واپسی کوئی تلاش نہیں۔';

  @override
  String get backend_environment_switch => 'بیک اینڈ انوائرمنٹ سوئچ';

  @override
  String get set_development_environment => 'ترقی کے ماحول کو مرتب کریں۔';

  @override
  String get set_production_environment => 'پیداواری ماحول کا تعین کریں۔';

  @override
  String get delay_notification => 'تاخیر کی اطلاع';

  @override
  String get notification_delay_debugging_is => 'اطلاع میں تاخیر ڈیبگنگ ہے۔';

  @override
  String get banner_enable => 'بینر فعال کریں۔';

  @override
  String get banner => 'بینر';

  @override
  String get show_details => 'تفصیلات دکھائیں';

  @override
  String get auto_detect_location => 'قریبی مسجد کا تجویز کریں';

  @override
  String get auto_detect_explanation =>
      'جب آپ اپنے مسجد سے دور ہوتے ہیں، ایپ خود بخود تجویز کرے گی کہ آپ اپنے مقام کے مبنی سب سے قریبی مسجد کا تلاش کریں۔';

  @override
  String get auto_detect_location_title =>
      'یہ لگتا ہے کہ آپ اپنے مسجد سے دور ہیں';

  @override
  String get auto_detect_location_message =>
      'کیا آپ چاہتے ہیں کہ آپ نماز کے صحیح اوقات کے لیے نزدیک ترین مسجد کو تلاش کریں؟';

  @override
  String get donation_success_toast => 'عطیہ کامیابی سے ہو گیا';

  @override
  String get donation_success =>
      'آپ کے عطیہ کا شکریہ، اللہ آپ کو جزائے خیر دے۔';

  @override
  String get donation_failed => 'عطیہ ناکام';

  @override
  String get donate_for_good => 'نیکی کے لیے عطیہ کریں';

  @override
  String get enter_amount_hint => 'رقم درج کریں';

  @override
  String get enter_donation_amount => 'عطیہ کی رقم درج کریں';

  @override
  String get google_pay_not_available =>
      'Google Pay اس ڈیوائس میں دستیاب نہیں ہے';

  @override
  String get error_performing_donation =>
      'عطیہ کرنے کی کوشش کرتے وقت ایک خرابی تھی';

  @override
  String get recurring_pay_text => 'یہ عطیہ ماہانہ کریں۔';

  @override
  String get adjust_angles => 'زاویے ایڈجسٹ کریں';

  @override
  String get silent => 'خاموش';

  @override
  String get default_ => 'ڈیفالٹ';

  @override
  String get muazzin => 'موذن';

  @override
  String get information => 'معلومات';

  @override
  String get news => 'خبریں';

  @override
  String get calendar => 'کیلنڈر';

  @override
  String get system => 'سسٹم';

  @override
  String get change_mosque_order =>
      'ترتیب تبدیل کرنے اور مرکزی مسجد مقرر کرنے کے لیے مساجد کو گھسیٹیں اور چھوڑیں';

  @override
  String get remove => 'ہٹائیں';

  @override
  String get save => 'محفوظ';

  @override
  String get main_mosque => 'مرکزی مسجد';

  @override
  String get mosques => 'مساجد';

  @override
  String get no_news => 'مسجد سے کوئی خبر نہیں';

  @override
  String get mawaqit_notification => 'مواقيت کا نوٹیفکیشن';

  @override
  String get no_mawaqit_notification => 'مواقيت کی طرف سے کوئی نوٹیفکیشن نہیں';

  @override
  String get meccan => 'مکی';

  @override
  String get medinan => 'مدنی';

  @override
  String get montada_islamic_foundation => 'مونتادا اسلامک فاؤنڈیشن';

  @override
  String get rashid_maash => 'رشید معاش';

  @override
  String get abdul_haleem => 'عبد الحلیم';

  @override
  String get dr_mustafa_khattab => 'ڈاکٹر مصطفی خطاب';

  @override
  String get frank_bubenheim_and_nadeem => 'فرینک بوبنہیم اور ندیم';

  @override
  String get abu_reda_muhammad_ibn_ahmad => 'ابو رضا محمد ابن احمد';

  @override
  String get unknown_author => 'نامعلوم مصنف';

  @override
  String get translation_settings => 'ترجمہ کی ترتیبات';

  @override
  String get reading => 'پڑھنا';

  @override
  String get listening => 'سننا';

  @override
  String get font_size => 'فونٹ کا سائز';

  @override
  String get font_size_arabic => 'عربی فونٹ کا سائز';

  @override
  String get horizontal => 'افقی';

  @override
  String get vertical => 'عمودی';

  @override
  String get add_page_bookmarks => 'صفحہ کو بُک مارکس میں شامل کریں';

  @override
  String get remove_page_bookmarks => 'صفحہ کو بُک مارکس سے ہٹائیں';

  @override
  String get no_bookmarks => 'کوئی بُک مارکس نہیں';

  @override
  String get liked => 'پسندیدہ';

  @override
  String get all_recitators => 'تمام قاری';

  @override
  String get downloaded => 'ڈاؤن لوڈ کیا گیا';

  @override
  String get search_for_recitator => 'قاری تلاش کریں';

  @override
  String get search_for_surah => 'سورۃ تلاش کریں';

  @override
  String get search_for_fav_recitator => 'پسندیدہ قاری تلاش کریں';

  @override
  String get listen_to_quran => 'قرآن سنیں';

  @override
  String get search_hint_text => 'مسجد یا شہر تلاش کریں';

  @override
  String get downloading => 'ڈاؤن لوڈ ہو رہا ہے';

  @override
  String get cant_download_more_than_3 =>
      'ایک وقت میں 3 سے زیادہ ڈاؤن لوڈ نہیں کر سکتے۔';

  @override
  String get boost => 'بوسٹ';

  @override
  String get recitation_deleted_successfully => 'تلاوت کامیابی سے حذف ہو گئی';

  @override
  String get quran => 'قرآن';

  @override
  String get qibla => 'قبلہ';

  @override
  String get search => 'تلاش کریں';

  @override
  String get more => 'مزید';

  @override
  String get download_cancelled => 'ڈاؤن لوڈ منسوخ ہو گئی';

  @override
  String get no_virtual_mosque_found =>
      'معذرت، ہمیں آپ کے ارد گرد کوئی مسجد نہیں ملی';

  @override
  String get no_surah_found => 'کوئی سورہ نہیں ملی';

  @override
  String get no_favorite_recitor_found => 'کوئی پسندیدہ قاری نہیں ملا';

  @override
  String get no_recitator_found => 'کوئی قاری نہیں ملا';

  @override
  String get empty_quran_search => 'تلاش کے نتائج یہاں ظاہر ہوں گے';

  @override
  String get no_bookmark_ayah =>
      'کوئی بُک مارکس دستیاب نہیں ہیں۔ برائے مہربانی اس آیت پر لمبی دبا کر رکھیں جسے آپ بُک مارک کرنا چاہتے ہیں۔';

  @override
  String get page => 'صفحہ';

  @override
  String get prayer_times_in => 'نماز کے اوقات';

  @override
  String get update_new_version =>
      'کتابت کی بہتری کے ساتھ ایک نظرِ ثانی شدہ ورژن جاری کر دیا گیا ہے۔ پڑھنے اور غور و فکر کے اپنے سفر کو جاری رکھنے کے لیے \'اپ ڈیٹ\' پر کلک کریں';

  @override
  String get update_quran_zip =>
      'کتابت کی بہتری کے ساتھ ایک نظرِ ثانی شدہ ورژن جاری کر دیا گیا ہے۔ پڑھنے اور غور و فکر کے اپنے سفر کو جاری رکھنے کے لیے \'اپ ڈیٹ\' پر کلک کریں.';

  @override
  String get update_latest_quran_version =>
      'کتابت کی بہتری کے ساتھ ایک نظرِ ثانی شدہ ورژن جاری کر دیا گیا ہے۔ پڑھنے اور غور و فکر کے اپنے سفر کو جاری رکھنے کے لیے \'اپ ڈیٹ\' پر کلک کریں.';

  @override
  String get update_latest_quran =>
      'کتابت کی بہتری کے ساتھ ایک نظرِ ثانی شدہ ورژن جاری کر دیا گیا ہے۔ پڑھنے اور غور و فکر کے اپنے سفر کو جاری رکھنے کے لیے \'اپ ڈیٹ\' پر کلک کریں.';

  @override
  String get set_notification_for_all_parayer => 'اسے ہر نماز کے لیے مقرر کریں';

  @override
  String get powered_by_mawaqit => 'Powered by MAWAQIT';

  @override
  String get adjust_hijri_date => 'ہجری تاریخ کو ایڈجسٹ کریں';

  @override
  String get prayer_times_by_location => 'مقام کے مطابق نماز کے اوقات';

  @override
  String get already_in_list => 'پہلے ہی آپ کی فہرست میں موجود ہے!';

  @override
  String get got_it => 'Got it!';

  @override
  String get mushaf_warsh => 'ورش';

  @override
  String get mushaf_hafs => 'حفص';

  @override
  String get select_rewaya => 'Select Rewaya';

  @override
  String get download_in_progress =>
      'ڈاؤن لوڈ جاری ہے، براہِ کرم انتظار کریں...';

  @override
  String get highlight_next_prayer_after_Iqama =>
      'اقامت کے بعد اگلی نماز کو نمایاں دکھائیں';

  @override
  String get helmi_nasr => 'ڈاکٹر حلمی نصر';

  @override
  String get settings_language_pt => 'پرتگالی';

  @override
  String get al_zikr => 'الذکر';

  @override
  String get favorite_azkar => 'پسندیدہ اذکار';

  @override
  String get all_azkar => 'تمام اذکار';

  @override
  String get device_audio_not_found => 'ڈیوائس میں کوئی آڈیو فائل نہیں ملی';

  @override
  String get device_media => 'ڈیوائس میڈیا';

  @override
  String get add_new_audio => 'نئی آڈیو فائل شامل کریں';

  @override
  String get selecet_audio_before_delete =>
      'منتخب آڈیو فائل کو حذف کرنے سے پہلے آپ کو نوٹیفکیشنز کے لیے ایک اور آڈیو فائل شامل کرنا ہوگی۔';

  @override
  String get addding_file_prompt =>
      'آپ متعدد آڈیو فائلیں شامل کر سکتے ہیں، لیکن براہِ کرم یقینی بنائیں کہ ایک ہی آڈیو فائل دوبارہ شامل نہ کریں.';

  @override
  String get ios_audio_file_picking_prompt =>
      'براہِ کرم ایسی آڈیو فائل منتخب کریں جس کا دورانیہ زیادہ سے زیادہ 30 سیکنڈ یا اس سے کم ہو۔ زیادہ دورانیے والی آڈیو فائل آپ کی نماز کی اطلاع میں خلل ڈال سکتی ہے.';

  @override
  String get select_audio_from_device => 'ڈیوائس سے آڈیو (mp3) منتخب کریں.';

  @override
  String get add_ticket => 'ٹکٹ شامل کریں';

  @override
  String get projects => 'پروجیکٹس';

  @override
  String get fill_form => 'اپنا پروجیکٹ یہاں شامل کرنے کے لیے اس';

  @override
  String get form => 'فارم کو پُر کریں';

  @override
  String get updates_draw =>
      'تازہ ترین معلومات حاصل کریں اور انعامات جیتنے کے لیے قرعہ اندازی میں شامل ہوں';

  @override
  String get email_info =>
      'یہ ای میل آپ سے رابطہ کرنے کے لیے استعمال کی جائے گی اگر آپ قرعہ اندازی جیت جاتے ہیں';

  @override
  String get enter_email => 'اپنا ای میل درج کریں';

  @override
  String get email_privacy =>
      'MAWAQIT اس ای میل کو کسی کے ساتھ شیئر نہیں کرے گا۔ یہ ہر قرعہ اندازی کے بعد خود بخود ہمارے ڈیٹا بیس سے حذف ہو جائے گا';

  @override
  String get submit => 'جمع کرائیں';

  @override
  String get disclaimer =>
      'MAWAQIT درج شدہ پروجیکٹس کے ذریعہ فراہم کردہ خدمات یا مصنوعات کی کوئی ذمہ داری قبول نہیں کرتا۔ براہ کرم اپنی تحقیق خود کریں';

  @override
  String get participating_with => 'آپ ان کے ساتھ شرکت کر رہے ہیں:';

  @override
  String get no_of_tickets => 'ٹکٹوں کی تعداد:';

  @override
  String get unparticipate => 'شرکت ختم کرنے کے لیے کلک کریں';

  @override
  String get next_ticket => 'اگلا ٹکٹ دستیاب ہوگا:';

  @override
  String get click_here => 'یہاں کلک کریں';

  @override
  String get mawaqit_store => 'MAWAQIT اسٹور';

  @override
  String get no_projects => 'کوئی منصوبے نہیں ہیں';

  @override
  String get draw_info =>
      'ہر مہینے ایک قرعہ اندازی۔ جیتنے کے اپنے امکانات بڑھانے کے لیے ہر 24 گھنٹے میں ایک نیا ٹکٹ شامل کریں!';

  @override
  String get enter_valid_email => 'درست ای میل درج کریں';

  @override
  String get please_enter_email => 'براہ کرم ای میل درج کریں';

  @override
  String get select_device_audo_for_ios_propmpt =>
      'ایک آڈیو فائل منتخب کریں، ہم اسے iOS کے موافق فارمیٹ میں تبدیل کر دیں گے اور iOS پلیٹ فارم کی حدود کی وجہ سے اسے 30 سیکنڈ تک مختصر کر دیں گے.';

  @override
  String get confirm_deletion => 'حذف کی تصدیق کریں';

  @override
  String get sure_remove_rewaya => 'کیا آپ واقعی اس روایت کو ہٹانا چاہتے ہیں؟';

  @override
  String get file_not_found => 'File not found';

  @override
  String get notified_boost =>
      'MAWAQIT Boost منصوبوں کی تازہ ترین معلومات کے بارے میں اطلاعات حاصل کریں.';

  @override
  String get contact_us_boost =>
      'اپنے پروجیکٹ کو یہاں شامل کرنے کے لیے، ہم سے اس پر رابطہ کریں:';

  @override
  String get prayer_times_set_by_mosque =>
      'نماز کے اوقات اور یہاں موجود تمام معلومات مسجد کی طرف سے مقرر کی جاتی ہیں، نہ کہ MAWAQIT کی طرف سے';

  @override
  String get hasan_nahi => 'حسن ناہی';

  @override
  String get albanian => 'البانی';

  @override
  String get delete_all => 'سب حذف کریں';

  @override
  String get delete_all_downloads => 'تمام ڈاؤن لوڈز حذف کریں';

  @override
  String get delete_recitations_message =>
      'کیا آپ واقعی تمام ڈاؤن لوڈ شدہ تلاوتیں حذف کرنا چاہتے ہیں؟';

  @override
  String get downloaded_deleted_successfully =>
      'تمام ڈاؤن لوڈ شدہ تلاوتیں کامیابی کے ساتھ حذف کر دی گئیں۔';

  @override
  String get failed_delete_recitations =>
      'تمام تلاوتیں حذف کرنے میں ناکامی ہوئی۔';

  @override
  String get no_azkar_result => 'کوئی یاد دہانی کے نتائج دستیاب نہیں ہیں\t';

  @override
  String get search_title => 'عنوان کے ذریعے تلاش کریں';

  @override
  String get compass_calibration =>
      'کمپاس کی کیلیبریشن کی ترتیب غیر فعال ہے۔ اپنے فون کو اس طرح آٹھ کے ہندسے کی شکل میں 3 بار جھکائیں اور حرکت دیں۔';

  @override
  String get compass_accuracy => 'Compass Accuracy';

  @override
  String get low => 'Low';

  @override
  String get tafsir => 'تفسیر';

  @override
  String get copy => 'نقل کریں';

  @override
  String get no_tafsir => 'ابھی تک کوئی تفسیر ڈاؤن لوڈ نہیں کی گئی ہے';

  @override
  String get download_tafsir => 'تفسیر ڈاؤن لوڈ کریں';

  @override
  String get hussary => 'محمود خلیل الحصری';

  @override
  String get hussary_mujawwad => 'الحصری (مجوّد)';

  @override
  String get shuraim => 'سعود الشریم';

  @override
  String get arrafai => 'ہانی عرفی';

  @override
  String get meaqli => 'ماہر المیقلی';

  @override
  String get afasy => 'مشاری العفاسی';

  @override
  String get minshawi => 'المنشاوی';

  @override
  String get abdulbasit => 'عبدالباسط عبدالصمد';

  @override
  String get abdulbasit_mujawwad => 'عبدالباسط (مجوّد)';

  @override
  String get ghamdi => 'سعد الغامدی';

  @override
  String get dosari => 'یاسر الدوسری';

  @override
  String get shatri => 'ابو بکر الشاطری';

  @override
  String get tunaiji => 'خلیفہ التیجی';

  @override
  String get sudais => 'عبدالرحمن السدیس';

  @override
  String get tablawi => 'محمد الطبلاوی';

  @override
  String get abdul_rashid_ali_sufi => 'عبد الرشید علی صوفی';

  @override
  String get abdullaah_ajuhaynee_murattal => 'عبداللہ الجہنی';

  @override
  String get abdullaah_basfar_murattal => 'عبداللہ بصفر';

  @override
  String get abdullah_matroud_murattal => 'عبداللہ مطرود';

  @override
  String get ahmed_al_ajamy_murattal => 'احمد العجمی';

  @override
  String get ahmed_neana_murattal => 'احمد نعینع';

  @override
  String get akram_alaqimy_murattal => 'اکرم العلاقمی';

  @override
  String get ali_hajjaj_suesy_murattal => 'علی حجاج السويسی';

  @override
  String get ali_jaber_murattal => 'علی جابر';

  @override
  String get ayman_sowaid_murattal => 'ایمن سویّد';

  @override
  String get aziz_alii_murattal => 'عزیز علیلی';

  @override
  String get fares_abbad_murattal => 'فارس عباد';

  @override
  String get hudhaify_murattal => 'الحذیفی';

  @override
  String get husary_muallim => 'الحصری (معلم انداز)';

  @override
  String get ibrahim_akhdar_murattal => 'ابراہیم الاخضر';

  @override
  String get khaalid_qahataanee_murattal => 'خالد القحطانی';

  @override
  String get khalid_al_mohanna => 'خالد المہنا';

  @override
  String get mahmoud_ali_al_banna_murattal => 'محمود علی البنا';

  @override
  String get minshawy_mujawwad => 'المنشاوی (مجود)';

  @override
  String get mohammad_rashad_alshareef => 'محمد رشاد الشریف';

  @override
  String get muhammad_abdulKareem_murattal => 'محمد عبد الکریم';

  @override
  String get muhammad_ayyoub_murattal => 'محمد ایوب';

  @override
  String get muhammad_jibreel_murattal => 'محمد جبریل';

  @override
  String get muhsin_al_qasim_murattal => 'عبد المحسن القاسم';

  @override
  String get nabil_rifai3_murattal => 'نبیل الرفاعی';

  @override
  String get nasser_alqatami_murattal => 'ناصر القطامی';

  @override
  String get sahl_yassin_murattal => 'سہل یاسین';

  @override
  String get salaah_abdulRahman_bukhatir_murattal => 'صلاح عبد الرحمن بخاطر';

  @override
  String get salah_albudair_murattal => 'صلاح البدیر';

  @override
  String get tawfeeq_assayegh => 'توفیق الصائغ';

  @override
  String get yaser_salamah_murattal => 'یاسر سلامہ';

  @override
  String get yassin_aljazairi => 'یاسن الجزائری';

  @override
  String get mohammed_abdelkarim => 'محمد عبد الکریم';

  @override
  String get alayoun_elkouchi => 'العیون الکوشی';

  @override
  String get abdelkarim_daghoush => 'عبد الکریم داغوش';

  @override
  String get murattal => 'مرتل';

  @override
  String get mujawwad => 'مجوّد';

  @override
  String get moallim => 'المصحف المعلم';

  @override
  String get play => 'چلائیں';

  @override
  String get playback_settings => 'پلے بیک ترتیبات';

  @override
  String get from => 'شروع';

  @override
  String get to => 'اختتام';

  @override
  String get range => 'حد';

  @override
  String get select_starting_verse => 'ابتدائی آیت منتخب کریں';

  @override
  String get select_ending_verse => 'اختتامی آیت منتخب کریں';

  @override
  String get range_repeat => 'حد دہرائیں';

  @override
  String get verse_repeat => 'آیت دہرائیں';

  @override
  String get improved_Hafs_Rewaya_Force_Update =>
      'ہم نے آپ کی حفص روایت کو بہتر بنا دیا ہے۔ ابھی ڈاؤن لوڈ کریں، تفسیر دریافت کریں اور کسی بھی آیت پر لمبا دباکر تلاوت سنیں۔';

  @override
  String get mawaqit_three_sixty_description =>
      'نئی متحرک گھڑی کے ساتھ نماز کے اوقات کو جیتا جاگتا دیکھیں۔';

  @override
  String get hnpai_User_Guidance_MAWAQIT_360 =>
      'ویو استعمال کرتے وقت آپ اس سیٹنگ کو بند نہیں کر سکتے۔ 360';

  @override
  String get mawaqit_three_sixty_beta => 'مواقيت 360 (بیٹا)';

  @override
  String get three_sixty_setting => '360 سیٹنگز';

  @override
  String get behavior => 'رویہ';

  @override
  String get next_prayer_open => 'اگلی نماز';

  @override
  String get all_prayer_closed => 'سب بند';

  @override
  String get recomended_next_prayer => 'تجویز کردہ: صرف اگلی نماز';

  @override
  String get live_preview => 'Live Preview';

  @override
  String get font_Size_Normal => 'Normal';

  @override
  String get font_Size_Large => 'Large';

  @override
  String get all_Prayer_Open => 'سب کھلے';

  @override
  String get settingFontSize => 'Font Size';

  @override
  String get last_Third_Of_The_Night => 'رات کا آخری تہائی حصہ';

  @override
  String get shaykh_aminuddin_muhammad => 'شیخ امینُ الدین محمد';

  @override
  String get hafs_an_asim => 'حفص عن عاصم';

  @override
  String get most_widespread_riwayah =>
      'دنیا بھر میں سب سے زیادہ پڑھی جانے والی روایت';

  @override
  String get warsh_an_nafi => 'ورش عن نافع';

  @override
  String get popular_across_north_west_africa =>
      'شمالی اور مغربی افریقہ میں مقبول';

  @override
  String get select => 'منتخب کریں';

  @override
  String get unknown_smartwatch => 'نامعلوم اسمارٹ واچ';

  @override
  String get connected_watch => 'منسلک گھڑی';

  @override
  String get ready_play_watch => 'آپ کی گھڑی پر چلانے کے لیے تیار ہے';

  @override
  String get transliteration => 'ٹرانسلِٹریشن';

  @override
  String get highlight_Tahajjud_Time => 'تہجد کے وقت کو نمایاں کریں';

  @override
  String get calendar_access_not_available => 'کیلنڈر تک رسائی دستیاب نہیں ہے۔';

  @override
  String get calendar_allow_access_mawaqit =>
      '\"براہ کرم اپنے ایونٹس MAWAQIT 360 پر دکھانے کے لیے کیلنڈر کی رسائی کی اجازت دیں۔\"';

  @override
  String get calendar_access_required =>
      'اپنے پروگرام دیکھنے کے لیے تقویم تک رسائی ضروری ہے۔';

  @override
  String get calendar_allow_access_button => 'کیلنڈر کی رسائی کی اجازت دیں';

  @override
  String get calendar_loading => 'کیلنڈر لوڈ ہو رہے ہیں…';

  @override
  String get calendar_select_for_mawaqit =>
      'وہ کیلنڈرز منتخب کریں جو آپ MAWAQIT 360 پر دکھانا چاہتے ہیں۔';

  @override
  String get calendar_enable_all => 'سب فعال کریں';

  @override
  String get calendar_disable_all => 'سب غیر فعال کریں';

  @override
  String get calendar_other_calendars => 'دیگر تقاویم';

  @override
  String get calendar_times_shown_in => 'اوقات دکھائے گئے ہیں';

  @override
  String get calendar_starts_on => 'شروع ہوتا ہے';

  @override
  String get calendar_ends_on => 'ختم ہوتا ہے';

  @override
  String get calendar_events_at_this_time => 'اس وقت کے ایونٹس';

  @override
  String get hamza_roberto_piccardo => 'حمزہ روبرٹو پککارڈو';

  @override
  String get open_in_mawaqit_app => 'اس مسجد کو MAWAQIT ایپ میں کھولیں';

  @override
  String get mosque_name => 'مسجد کا نام';

  @override
  String get mosque_not_found => 'مسجد نہیں ملی';

  @override
  String get invalid_mosque_link => 'غلط مسجد لنک';

  @override
  String get not_open_mosque_link =>
      'اس مسجد کا لنک نہیں کھولا جا سکا۔ دوبارہ کوشش کریں۔';

  @override
  String get no_calendars_on_this_device =>
      'اس ڈیوائس پر کوئی کیلنڈر نہیں ملا۔.';

  @override
  String get calendar_event_at_this_time => 'اس وقت کا ایونٹ';

  @override
  String get search_ayah_surah => 'آیت یا سورت کے ذریعے تلاش کریں';

  @override
  String get matching_surahs => 'مطابقت والی سورتیں';

  @override
  String get matching_ayahs => 'مطابقت والی آیات';

  @override
  String get live_timer => 'براہِ راست ٹائمر';

  @override
  String get notify_before => 'پہلے اطلاع دیں';

  @override
  String get take_time_appear_watch =>
      'آپ کی گھڑی پر ظاہر ہونے میں چند سیکنڈ لگ سکتے ہیں۔.';

  @override
  String get eid1 => 'عید';

  @override
  String get hide_live_activity_for_prayer =>
      'صرف اس نماز کے لیے لائیو ایکٹیویٹی چھپائیں';

  @override
  String get settings => 'ترتیبات';

  @override
  String get azkar => 'اذکار';

  @override
  String get quran_tajweed => 'قرآنِ تجوید';

  @override
  String get tajweed_description =>
      'رنگوں سے نشان زد تجوید کے قواعد تاکہ آپ درستگی سے تلاوت کر سکیں';

  @override
  String get tajwid_rules => 'تجوید کے قواعد';

  @override
  String get examples => 'مثالیں';

  @override
  String get real_time_weather => 'حقیقی وقت کا موسم';

  @override
  String get layout => 'لےآؤٹ';

  @override
  String get list => 'فہرست';

  @override
  String get grid => 'گرڈ';

  @override
  String get real_time_weather_needs_internet =>
      'حقیقی وقت کے موسم کی تازہ معلومات حاصل کرنے کے لیے انٹرنیٹ کنکشن ضروری ہے.';

  @override
  String get view_detail => 'تفصیل دیکھیں';

  @override
  String get temperature => 'درجہ حرارت';

  @override
  String get tahajjud_time => 'تہجد کا وقت';

  @override
  String get last_third => 'آخری تہائی حصہ';

  @override
  String get midnight => 'آدھی رات';

  @override
  String get failed_to_update => 'اپڈیٹ ناکام ہو گئی۔';

  @override
  String get tap_to_retry => 'دوبارہ کوشش کرنے کے لیے ٹیپ کریں';

  @override
  String get support_mawaqit_association => 'مواقيت ایسوسی ایشن کی حمایت کریں';

  @override
  String get help_improve_mawaqit => 'مواقيت کو بہتر بنانے میں مدد کریں';

  @override
  String get analytics_description =>
      'ایپ کو بہتر بنانے کے لیے گمنام استعمال کا تجزیہ کرنے کی اجازت دیں۔ کوئی ذاتی معلومات نہیں لی جاتی اور نہ ہی شیئر کی جاتی ہیں۔';

  @override
  String get weather_clear => 'صاف';

  @override
  String get weather_clouds => 'بادل';

  @override
  String get weather_tornado => 'طوفانی بگولا';

  @override
  String get weather_ash => 'راکھ';

  @override
  String get weather_dust => 'گرد';

  @override
  String get weather_sand => 'ریت';

  @override
  String get weather_fog => 'دھند';

  @override
  String get weather_haze => 'کہر';

  @override
  String get weather_smoke => 'دھواں';

  @override
  String get weather_mist => 'کہر آور';

  @override
  String get weather_snow => 'برف';

  @override
  String get weather_rain => 'بارش';

  @override
  String get weather_drizzle => 'بوندا باندی';

  @override
  String get weather_thunderstorm => 'گرج چمک کا طوفان';

  @override
  String get weather_squall => 'تیز جھکڑ';

  @override
  String get weather_celsius => '°سِ';

  @override
  String get animated_background => 'متحرک پس منظر';

  @override
  String get weather => 'موسم';

  @override
  String get weather_need_internet =>
      'اس فیچر کو اپڈیٹس ڈاؤن لوڈ کرنے کے لیے انٹرنیٹ کنکشن درکار ہے۔';

  @override
  String get weather_light_rain => 'ہلکی بارش';

  @override
  String get edit => 'ترمیم کریں';

  @override
  String get iqama_countdown => 'اقامہ کاؤنٹ ڈاؤن';

  @override
  String get next_prayer_countdown => 'اگلی نماز کا کاؤنٹ ڈاؤن';

  @override
  String get next_prayer => 'اگلی نماز';

  @override
  String get live_countdown_for_all_pray =>
      'لائیو کاؤنٹ ڈاؤن تمام نمازوں کے لیے فعال رہے گا اور ہر آنے والی نماز کے لیے خودکار طور پر ٹائمر دکھائے گا۔';

  @override
  String get live_countdown => 'لائیو کاؤنٹ ڈاؤن';

  @override
  String get quick_quran_reading_access => 'قرآن جلدی پڑھنے کی رسائی';

  @override
  String get quick_quran_discription =>
      'جب آپ قرآن ٹیب کھولیں، وہیں سے جاری رکھیں جہاں آپ نے چھوڑا تھا۔';

  @override
  String get live_activity_tooltip =>
      'لائیو سرگرمیاں آپ کی لاک اسکرین اور ڈائنامک آئی لینڈ (اگر سپورٹڈ ہو) پر ظاہر ہوتی ہیں۔ جب یہ فعال ہوں تو اگلی نماز یا اقامت تک کا وقت ایک لائیو ٹائمر کے ذریعے دکھایا جاتا ہے۔';

  @override
  String get prayer => 'نماز';

  @override
  String get weather_fahrenheit => '°ف';

  @override
  String get weather_your_location =>
      'یہ موسم آپ کے مقام کے لیے دکھایا گیا ہے، نہ کہ مسجد کے لیے۔';

  @override
  String get view_360_currently_selected => 'مواقيت 360 ویو اس وقت منتخب ہے';

  @override
  String get tap_change_layout => 'لے آؤٹ تبدیل کرنے کے لیے یہاں ٹیپ کریں';

  @override
  String get step => 'مرحلہ';

  @override
  String get jump_to_page => 'صفحہ پر جائیں';

  @override
  String get select_surah => 'سورہ منتخب کریں';

  @override
  String get please_select_surah => 'براہ کرم ایک سورہ منتخب کریں';

  @override
  String get ayah_number => 'آیت نمبر';

  @override
  String get ayah_num_greater_zero => 'آیت نمبر 0 سے بڑا ہونا چاہیے';

  @override
  String get ayah_number_exceeds_max =>
      'اس سورہ کے لیے آیت نمبر حد سے زیادہ ہے';

  @override
  String get update_page => 'صفحہ اپڈیٹ کریں';

  @override
  String get enter_ayah_num => 'آیت نمبر درج کریں';

  @override
  String get help => 'مدد';

  @override
  String get sofian_s_siregar => 'صوفیان ایس. سیرگار';

  @override
  String get download_all_surahs => 'تمام سورتیں ڈاؤن لوڈ کریں';

  @override
  String get this_will_download_all_surahs_for_offline =>
      'یہ تمام سورتوں کو آف لائن سننے کے لیے ڈاؤن لوڈ کرے گا۔ براہ کرم جاری رکھنے سے پہلے یقینی بنائیں کہ آپ Wi-Fi سے منسلک ہیں۔';

  @override
  String get tap_change_notification => 'نوٹیفکیشن تبدیل کرنے کے لیے ٹیپ کریں';

  @override
  String get adhan_at => 'اذان';

  @override
  String get semantic_mawaqit_notification =>
      'MAWAQIT سے غیر پڑھے گئے نوٹیفکیشنز';

  @override
  String get semantic_open_notifications => 'نوٹیفکیشنز کھولیں';

  @override
  String get semantic_tap_to_change_notification =>
      'نوٹیفکیشن سیٹنگز تبدیل کرنے کے لیے ٹیپ کریں';

  @override
  String get semantic_edit_mosques => 'اپنی مساجد میں ترمیم کرنے کے لیے کھولیں';

  @override
  String get semantic_change_layout => 'ہوم لے آؤٹ تبدیل کریں';

  @override
  String get semantic_open_layout_menu => 'لے آؤٹ مینو کھولیں';

  @override
  String get semantic_edit_home_settings => 'ہوم سیٹنگز میں ترمیم کریں';

  @override
  String get semantic_list_layout => 'فہرست لے آؤٹ';

  @override
  String get semantic_grid_layout => 'گرڈ لے آؤٹ';

  @override
  String get sematnic_360_clock_layout => '360 گھڑی کا لے آؤٹ';

  @override
  String get semantic_layout_menu_open => 'لے آؤٹ مینو کھلا ہے';

  @override
  String get close => 'بند کریں';

  @override
  String get semantic_mosque_page => 'مسجد کا صفحہ';

  @override
  String get semantic_of => 'کا';

  @override
  String get semantic_unread_mawaqit_notifications =>
      'MAWAQIT کے غیر پڑھے گئے نوٹیفکیشنز';

  @override
  String get back => 'واپس';

  @override
  String get semantic_download_all_surahs_for_offline_listening =>
      'آف لائن سننے کے لیے تمام سورتیں ڈاؤن لوڈ کریں';

  @override
  String get semantic_download_all_surahs => 'تمام سورتیں ڈاؤن لوڈ کریں';

  @override
  String get semantic_bulk_download_progress => 'بلک ڈاؤن لوڈ کی پیش رفت';

  @override
  String get semantic_surahs_downloaded => 'ڈاؤن لوڈ کی گئی سورتیں';

  @override
  String get semantic_cancel_download_all_surahs =>
      'تمام سورتوں کا ڈاؤن لوڈ منسوخ کریں';

  @override
  String get semantic_bulk_download_in_progress => 'بلک ڈاؤن لوڈ جاری ہے۔';

  @override
  String get semantic_replay_current_surah => 'موجودہ سورت دوبارہ چلائیں';

  @override
  String get semantic_pause_current_surah => 'موجودہ سورت کو روکیں';

  @override
  String get semantic_play_current_surah => 'موجودہ سورت چلائیں';

  @override
  String get semantic_open_play_on_watch_options_for =>
      'گھڑی پر پلے کے آپشنز کھولیں برائے';

  @override
  String get semantic_now_playing_surah => 'اب چل رہا ہے۔ سورت';

  @override
  String get semantic_recited_by => 'تلاوت کرنے والے';

  @override
  String get semantic_minimize_player => 'پلیئر کو چھوٹا کریں';

  @override
  String get semantic_elapsed => 'گزرا ہوا';

  @override
  String get semantic_playback_time_elapsed => 'پلے بیک کا وقت۔ گزرا ہوا';

  @override
  String get semantic_remaining => 'باقی';

  @override
  String get semantic_turn_shuffle_off => 'شفل بند کریں';

  @override
  String get semantic_turn_shuffle_on => 'شفل آن کریں';

  @override
  String get semantic_previous_surah => 'پچھلی سورت';

  @override
  String get semantic_next_surah => 'اگلی سورت';

  @override
  String get semantic_turn_repeat_current_surah_off =>
      'موجودہ سورت کا ریپیٹ بند کریں';

  @override
  String get semantic_turn_repeat_current_surah_on =>
      'موجودہ سورت کا ریپیٹ آن کریں';

  @override
  String get semantic_pause => 'روکیں';

  @override
  String get semantic_use_current_location => 'موجودہ مقام استعمال کریں';

  @override
  String get semantic_collapse_mosque_list => 'مساجد کی فہرست سمیٹیں';

  @override
  String get semantic_expand_mosque_list => 'مساجد کی فہرست پھیلائیں';

  @override
  String get added => 'شامل کیا گیا';

  @override
  String get not_added => 'شامل نہیں کیا گیا';

  @override
  String get semantic_navigate_to_mosque => 'مسجد تک جائیں';

  @override
  String get semantic_decrease_pre_notification =>
      'پہلے نوٹیفکیشن کا وقت کم کریں';

  @override
  String get semantic_increase_pre_notification =>
      'پہلے نوٹیفکیشن کا وقت بڑھائیں';

  @override
  String get semantic_facing_qibla => 'آپ قبلہ رخ ہیں';

  @override
  String get semantic_clockwise => 'گھڑی کی سمت میں';

  @override
  String get sematnic_counter_clockwise => 'گھڑی کی مخالف سمت میں';

  @override
  String get semantic_rotate_slightly => 'تقریباً ہو گیا۔ تھوڑا سا گھمائیں';

  @override
  String get semanitc_degrees_to_qibla => 'قبلہ کی سمت ڈگری';

  @override
  String get about => 'تقریباً';

  @override
  String get rotate => 'گھمائیں';

  @override
  String get semantic_close_audio_player => 'آڈیو پلیئر بند کریں';

  @override
  String get reciter_settings => 'قاری کی ترتیبات';

  @override
  String get previous_verse => 'پچھلی آیت';

  @override
  String get next_verse => 'اگلی آیت';

  @override
  String get decrease => 'کم کریں';

  @override
  String get increase => 'بڑھائیں';

  @override
  String get quran_settings => 'قرآن کی ترتیبات';

  @override
  String get semantic_mushaf_view => 'مصحف ویو';

  @override
  String get semantic_verse_list_view => 'آیات کی فہرست ویو';

  @override
  String get not_downloaded => 'ڈاؤن لوڈ نہیں ہوا';

  @override
  String get verse_list_enabled_for_accessibility =>
      'رسائی کے لیے آیات کی فہرست موڈ فعال ہے';

  @override
  String get semantic_switch_verse_list =>
      'مصحف صفحہ۔ مکمل رسائی کے لیے آیات کی فہرست ویو پر جائیں۔';

  @override
  String get semantic_decrease_hijri_adjustment => 'ہجری ایڈجسٹمنٹ کم کریں';

  @override
  String get semantic_minimum_adjustment_reached =>
      'کم از کم ایڈجسٹمنٹ تک پہنچ گیا';

  @override
  String get semantic_current_adjustment => 'موجودہ ایڈجسٹمنٹ';

  @override
  String get semantic_increase_hijri_adjustment => 'ہجری ایڈجسٹمنٹ بڑھائیں';

  @override
  String get semantic_maximum_adjustment_reached =>
      'زیادہ سے زیادہ ایڈجسٹمنٹ تک پہنچ گیا';

  @override
  String get semantic_minimum_size_reached => 'کم از کم سائز تک پہنچ گیا';

  @override
  String get semantic_decrease => 'کم کریں';

  @override
  String get semantic_current_size => 'موجودہ سائز';

  @override
  String get semantic_increase => 'بڑھائیں';

  @override
  String get semantic_maximum_size_reached => 'زیادہ سے زیادہ سائز تک پہنچ گیا';

  @override
  String get semantic_favorite => 'پسندیدہ';

  @override
  String get semantic_added_to_favorites => 'پسندیدہ میں شامل کر دیا گیا';

  @override
  String get semantic_not_in_favorites => 'پسندیدہ میں شامل نہیں';

  @override
  String get semantic_for => 'پسندیدہ میں شامل نہیں';

  @override
  String get semantic_view_style => 'کے لیے';

  @override
  String get semantic_double_tap_to_change_view_style =>
      'ویو اسٹائل تبدیل کرنے کے لیے ڈبل ٹیپ کریں';

  @override
  String get semantic_share_dua => 'دعا شیئر کریں';

  @override
  String get semantic_tasbih_settings => 'تسبیح کی ترتیبات';

  @override
  String get semantic_tasbih_counter => 'تسبیح کاؤنٹر';

  @override
  String get semantic_previous_dua => 'پچھلی دعا';

  @override
  String get semantic_next_dua => 'اگلی دعا';

  @override
  String get semantic_dua => 'دعا';

  @override
  String get semantic_opens => 'کھولتا ہے';

  @override
  String get semantic_opens_language_selection => 'زبان کا انتخاب کھولتا ہے';

  @override
  String get semantic_device => 'ڈیوائس';

  @override
  String get semantic_light => 'لائٹ';

  @override
  String get semantic_dark => 'ڈارک';

  @override
  String get semantic_on => 'آن';

  @override
  String get semantic_off => 'آف';

  @override
  String get semantic_ayah_list_actions => 'بک مارک ہٹائیں';

  @override
  String get remove_bookmark => 'بک مارک ہٹائیں';

  @override
  String get add_bookmark => 'بک مارک شامل کریں';

  @override
  String get selected => 'منتخب';

  @override
  String get double_tap_to_continue => 'جاری رکھنے کے لیے دو بار ٹیپ کریں';

  @override
  String get clear_search => 'تلاش صاف کریں';

  @override
  String get mosques_found => 'مساجد مل گئیں';

  @override
  String get mosque_added => 'مسجد شامل کر دی گئی';

  @override
  String get mosque_removed => 'مسجد ہٹا دی گئی';

  @override
  String get scroll_left_right_select_surah =>
      'سورت منتخب کرنے کے لیے بائیں یا دائیں اسکرول کریں';

  @override
  String get swipe_left_right_select_mushaf_type =>
      'مختلف مصحف منتخب کرنے کے لیے بائیں یا دائیں سوائپ کریں';

  @override
  String get location_permission_required => 'لوکیشن کی اجازت درکار ہے';

  @override
  String get semantic_previous_month => 'پچھلا مہینہ';

  @override
  String get semantic_next_month => 'اگلا مہینہ';

  @override
  String get semantic_view_image => 'تصویر دیکھیں';

  @override
  String get access_location_details =>
      'آپ کے موجودہ مقام کی بنیاد پر درست مقامی موسم، قبلہ کی سمت اور مساجد کی معلومات دکھانے کے لیے MAWAQIT کو آپ کے مقام تک رسائی درکار ہے۔ آپ پہلے اس اجازت کو مسترد کر چکے ہیں۔';

  @override
  String get location_off => 'لوکیشن بند ہے';

  @override
  String get semantic_pre_notification_time => 'پہلے نوٹیفکیشن کا وقت';

  @override
  String get search_for_mosque_or_enable_location =>
      'ہم قریبی مسجد تجویز نہیں کر سکتے کیونکہ لوکیشن تک رسائی مسترد کر دی گئی ہے۔ براہِ کرم کسی مسجد کو تلاش کریں یا لوکیشن کی اجازت فعال کریں۔';

  @override
  String get location => 'لوکیشن';

  @override
  String get stop => 'روکیں';

  @override
  String get plays_adhan_prayer_arrives => 'نماز کا وقت ہونے پر اذان چلاتا ہے';

  @override
  String get play_in_silent_mode => 'سائلنٹ موڈ میں چلائیں';

  @override
  String get notifications_not_arriving => 'نوٹیفیکیشنز نہیں آ رہے؟';

  @override
  String get android_phones_aggressively =>
      'کچھ اینڈرائیڈ فونز بیک گراؤنڈ میں ایپس کو سختی سے بند کر دیتے ہیں۔.';

  @override
  String get elmir_kuliev => 'ایلمیر کولییف';

  @override
  String get besim_korkut => 'بیسم کورکوت';

  @override
  String get settings_language_bs => 'بوسنیائی';

  @override
  String get something_wrong_getting_location =>
      'آپ کی لوکیشن حاصل کرتے وقت ایک خرابی پیش آگئی۔ براہ کرم دوبارہ کوشش کریں۔.';

  @override
  String get custom_volume => 'حسبِ ضرورت آواز';

  @override
  String get general => 'عمومی';

  @override
  String get prayer_location => 'نماز اور لوکیشن';

  @override
  String get privacy => 'رازداری';

  @override
  String get play_full_adhan => 'مکمل اذان چلائیں';

  @override
  String get full_adhan_will_keep_repeating =>
      'مکمل اذان اس وقت تک بار بار چلتی رہے گی جب تک آپ اسے بند نہ کریں۔';

  @override
  String get hussein_taji_kal_dari => 'حسین تاجی کال داری';

  @override
  String get settings_language_fa => 'فارسی';

  @override
  String get mawaqit_non_profit => 'مواقيت غیر منافع بخش تنظیم · سب کے لیے مفت';

  @override
  String get garmin_watch_detected => 'Garmin گھڑی دریافت ہوئی';

  @override
  String get garmin_support_detected =>
      'آپ کے آلے پر Garmin سپورٹ دستیاب ہے۔ کیا آپ اپنی نماز کے اوقات Garmin گھڑی کے ساتھ ہم آہنگ کرنا چاہتے ہیں؟';

  @override
  String get never_remind_me => 'دوبارہ یاد نہ دلائیں';

  @override
  String get remind_me_later => 'بعد میں یاد دلائیں';

  @override
  String get you_can_enable_garmin_sync =>
      'آپ کسی بھی وقت Settings → Pending Actions سے Garmin ہم آہنگی فعال کر سکتے ہیں۔';

  @override
  String get pending_actions => 'زیر التواء اقدامات';

  @override
  String get setup_garmin_watch => 'Garmin گھڑی سیٹ اپ کریں';

  @override
  String get sync_prayer_times_garmin =>
      'نماز کے اوقات Garmin گھڑی سے ہم آہنگ کریں';

  @override
  String get setup_garmin => 'Garmin سیٹ اپ کریں';

  @override
  String get open_garmin_device_selection => 'Garmin ڈیوائس منتخب کریں';

  @override
  String get admin_adminTitle => 'مسجد کا انتظام';

  @override
  String get admin_adminTileSubtitle => 'اعلانات، نماز کی ترتیبات اور اذان';

  @override
  String get admin_signInHeadline => 'اپنی مسجد کا انتظام کریں';

  @override
  String get admin_signInIntro =>
      'جن مساجد کے آپ ذمہ دار ہیں انہیں سنبھالنے کے لیے اسی اکاؤنٹ سے سائن ان کریں جو آپ mawaqit.net پر استعمال کرتے ہیں۔';

  @override
  String get admin_capabilityMessages => 'اعلانات اور فوری پیغام شائع کرنا';

  @override
  String get admin_capabilityTimes => 'اقامت، جمعہ اور عید کے اوقات مقرر کرنا';

  @override
  String get admin_capabilityAdhan => 'اذان اور اس کے چلنے کا انداز منتخب کرنا';

  @override
  String get admin_notAnAdminTitle => 'ابھی تک منتظم نہیں ہیں؟';

  @override
  String get admin_notAnAdminBody =>
      'مساجد MAWAQIT کی ویب سائٹ پر رجسٹر ہوتی ہیں، جہاں ٹیم اکاؤنٹ فعال کرنے سے پہلے ان کا جائزہ لیتی ہے۔';

  @override
  String get admin_registerMosque => 'مسجد رجسٹر کریں';

  @override
  String get admin_registerUnavailable => 'ویب سائٹ نہیں کھل سکی۔';

  @override
  String get admin_signIn => 'سائن ان';

  @override
  String get admin_signInAgain => 'دوبارہ سائن ان کریں';

  @override
  String get admin_sessionExpired => 'آپ کا سیشن ختم ہو گیا ہے۔';

  @override
  String get admin_myMosques => 'میری مساجد';

  @override
  String get admin_logOut => 'لاگ آؤٹ';

  @override
  String get admin_noMosques => 'کوئی مسجد نہیں ملی۔';

  @override
  String get admin_archivedNotConfigurable =>
      'یہ مسجد محفوظ شدہ ہے — اس کی ترتیب ممکن نہیں۔';

  @override
  String admin_mosqueId(String id) {
    return 'ID $id';
  }

  @override
  String get admin_statusArchived => 'محفوظ شدہ';

  @override
  String get admin_openMessages => 'پیغامات';

  @override
  String get admin_openConfiguration => 'ترتیبات';

  @override
  String get admin_configurationTitle => 'ترتیبات';

  @override
  String get admin_configurationSaved => 'ترتیبات محفوظ ہو گئیں';

  @override
  String get admin_oneUnsavedChange => '1 غیر محفوظ تبدیلی';

  @override
  String get admin_sectionRegional => 'علاقائی ترتیبات';

  @override
  String get admin_sectionAdhan => 'اذان';

  @override
  String get admin_sectionIqama => 'اقامت';

  @override
  String get admin_sectionJumua => 'جمعہ';

  @override
  String get admin_sectionPrayerDuration => 'نماز کا دورانیہ';

  @override
  String get admin_sectionDuas => 'دعائیں اور احادیث';

  @override
  String get admin_sectionEid => 'عید اور رمضان';

  @override
  String get admin_sectionNotYet => 'ایپ میں ابھی قابلِ ترمیم نہیں';

  @override
  String get admin_iqamaEnabled => 'اقامت فعال';

  @override
  String get admin_iqamaEnabledHelp =>
      'اسے بند کرنے سے مسجد کی اسکرین پر انتظار کے اوقات، اقامت کا اشارہ اور نماز کے بعد کی دعائیں بھی نظر نہیں آئیں گی۔';

  @override
  String get admin_iqamaWaitingTimes => 'اذان کے بعد انتظار (منٹ)';

  @override
  String get admin_iqamaSignal => 'اقامت کا اشارہ';

  @override
  String get admin_iqamaSound => 'آواز';

  @override
  String get admin_iqamaCountdown => 'پوری اسکرین پر الٹی گنتی';

  @override
  String get admin_iqamaDisplayTime => 'اشارے کے دکھائے جانے کا وقت';

  @override
  String get admin_iqamaAlwaysHhMm =>
      'اقامت کے اوقات ہمیشہ گھنٹہ:منٹ میں دکھائیں';

  @override
  String get admin_iqamaCalendarNote =>
      'کیلنڈر سے مقرر کردہ اوقاتِ اقامت ویسے ہی رہیں گے — انہیں ویب سائٹ پر تبدیل کریں۔';

  @override
  String get admin_eidPrayerTimes => 'نمازِ عید کے اوقات';

  @override
  String get admin_ramadanImsak => 'رمضان';

  @override
  String get admin_eidPrayerHelp =>
      'عید سے پہلے والے ہفتے مسجد کی اسکرین پر دکھائے جاتے ہیں۔ جو نماز نہ ہو اسے وقت کے بغیر چھوڑ دیں۔';

  @override
  String get admin_imsakBeforeFajr => 'فجر سے پہلے امساک دکھائیں';

  @override
  String get admin_imsakHelp =>
      'امساک کتنی جلدی ظاہر ہو۔ فجر 04:00 اور یہاں 30 منٹ ہوں تو اسکرین 03:30 دکھائے گی۔';

  @override
  String get admin_imsakOff => 'بند';

  @override
  String get admin_eidNoPrayerTime => 'کوئی وقت مقرر نہیں';

  @override
  String get admin_adhanSoundLabel => 'آواز';

  @override
  String get admin_adhanPlaysFor => 'ان اوقات میں چلتی ہے';

  @override
  String get admin_adhanDurationLabel => 'اذان کا دورانیہ';

  @override
  String get admin_adhanDurationHelp =>
      'یہ آواز اذان سے پہلے ختم ہو جاتی ہے، اس لیے اسکرین کو معلوم ہونا چاہیے کہ آپ کے مؤذن کو کتنا وقت لگتا ہے۔';

  @override
  String admin_customAdhanFile(String name) {
    return 'اپ لوڈ کردہ فائل: $name';
  }

  @override
  String get admin_customAdhanMissing =>
      'ابھی کوئی فائل اپ لوڈ نہیں ہوئی — ویب سائٹ پر ایک شامل کریں۔';

  @override
  String get admin_adhanNoPrayers => 'کوئی نماز نہیں';

  @override
  String get admin_prayerDurationGroup => 'فی نماز منٹ';

  @override
  String get admin_prayerDurationHelp =>
      'نماز کے دوران سیاہ اسکرین اور اس کے بعد کی دعائیں اتنی ہی دیر رہتی ہیں۔';

  @override
  String get admin_duasInvocations => 'دعائیں';

  @override
  String get admin_duasHadiths => 'احادیث';

  @override
  String get admin_duaAfterAdhan => 'اذان کے بعد دعا';

  @override
  String get admin_duaAfterPrayer => 'نماز کے بعد دعائیں';

  @override
  String get admin_randomHadith => 'ہر 5 منٹ بعد ایک حدیث دکھائیں';

  @override
  String get admin_hadithSource => 'امام نووی کی ریاض الصالحین سے۔';

  @override
  String get admin_hadithPause => 'وقفہ درمیان';

  @override
  String get admin_hadithPauseNever => 'کبھی نہیں';

  @override
  String get admin_hadithPauseHelp =>
      'اُس مسجد کے لیے جہاں دو نمازوں کے درمیان درس ہوتا ہے۔';

  @override
  String get admin_duasAllOff => 'سب بند';

  @override
  String get admin_summaryDuaAdhan => 'اذان کے بعد';

  @override
  String get admin_summaryDuaPrayer => 'نماز کے بعد';

  @override
  String get admin_summaryHadiths => 'احادیث';

  @override
  String get admin_regionalHijri => 'ہجری تاریخ';

  @override
  String get admin_regionalMosqueScreen => 'مسجد کی اسکرین';

  @override
  String get admin_regionalRegion => 'علاقہ';

  @override
  String get admin_timezoneLabel => 'ٹائم زون (ویب سائٹ پر مقرر ہوتا ہے)';

  @override
  String get admin_timezoneUnknown => 'نامعلوم';

  @override
  String get admin_dstLabel => 'موسمِ گرما کا وقت';

  @override
  String get admin_dstDatesNote =>
      'گرمیوں اور سردیوں کے وقت کی تبدیلی کی تاریخیں ویسی ہی رہیں گی — انہیں ویب سائٹ پر مقرر کریں۔';

  @override
  String get admin_hijriAdjustmentLabel => 'تاریخ کی درستی';

  @override
  String get admin_jumuaTimeGroup => 'جمعہ کا وقت';

  @override
  String get admin_jumuaDuringGroup => 'جمعہ کے دوران';

  @override
  String get admin_jumuaSameAsDuhr => 'ظہر کے وقت پر';

  @override
  String get admin_jumuaFollowsDuhr =>
      'ظہر کے وقت ہوتا ہے — آپ کا اپنا وقت محفوظ رہتا ہے۔';

  @override
  String get admin_jumuaApplyDst => 'موسمِ گرما میں ایک گھنٹہ شامل کریں';

  @override
  String get admin_jumuaDuration => 'دورانیہ (خطبہ + نماز)';

  @override
  String get admin_jumuaDurationHelp =>
      'جمعہ کے دوران جو کچھ دکھایا جاتا ہے وہ اتنی ہی دیر رہتا ہے۔';

  @override
  String get admin_jumuaBlackScreen => 'جمعہ کے دوران سیاہ اسکرین';

  @override
  String get admin_jumuaDhikr => 'جمعہ کے آداب پر حدیث دکھائیں';

  @override
  String get admin_jumuaNoTime => 'کوئی وقت مقرر نہیں';

  @override
  String get admin_summaryAsDuhr => 'ظہر';

  @override
  String get admin_timeNotSet => 'مقرر نہیں';

  @override
  String get admin_clearTime => 'صاف کریں';

  @override
  String get admin_flashContent => 'مواد';

  @override
  String get admin_flashContentHint =>
      'مسجد کی اسکرین پر دکھایا جانے والا پیغام';

  @override
  String get admin_flashOrientation => 'سمت';

  @override
  String get admin_flashColor => 'رنگ';

  @override
  String get admin_flashStartDate => 'آغاز کی تاریخ';

  @override
  String get admin_flashExpireDate => 'اختتام کی تاریخ';

  @override
  String get admin_flashTooShort => 'کم از کم 30 حروف';

  @override
  String get admin_flashSaved => 'فوری پیغام محفوظ ہو گیا';

  @override
  String get admin_flashSaving => 'محفوظ کیا جا رہا ہے…';

  @override
  String get admin_flashDelete => 'حذف کریں';

  @override
  String get admin_flashDeleteTitle => 'کیا یہ فوری پیغام حذف کریں؟';

  @override
  String get admin_deleteBody =>
      'یہ مسجد کی اسکرین پر دکھائی نہیں دے گا۔ اسے واپس نہیں کیا جا سکتا۔';

  @override
  String get admin_flashDeleted => 'فوری پیغام حذف ہو گیا';

  @override
  String get admin_tabAnnouncements => 'اعلانات';

  @override
  String get admin_tabFlash => 'فوری پیغام';

  @override
  String get admin_announcementsEmpty => 'ابھی کوئی اعلان نہیں۔';

  @override
  String get admin_announcementNew => 'نیا اعلان';

  @override
  String get admin_announcementEdit => 'اعلان میں ترمیم';

  @override
  String get admin_announcementSaved => 'اعلان محفوظ ہو گیا';

  @override
  String get admin_announcementDeleted => 'اعلان حذف ہو گیا';

  @override
  String get admin_announcementDeleteTitle => 'کیا یہ اعلان حذف کریں؟';

  @override
  String get admin_announcementLimitReached =>
      'آپ فعال اعلانات کی حد کو پہنچ چکے ہیں۔';

  @override
  String get admin_discardTitle => 'کیا تبدیلیاں ترک کریں؟';

  @override
  String get admin_discardBody => 'آپ نے یہاں جو لکھا ہے وہ محفوظ نہیں ہوگا۔';

  @override
  String get admin_discard => 'ترک کریں';

  @override
  String get admin_keepEditing => 'ترمیم جاری رکھیں';

  @override
  String get admin_chipScreen => 'اسکرین';

  @override
  String get admin_chipMobile => 'موبائل';

  @override
  String get admin_statusEnabled => 'فعال';

  @override
  String get admin_statusDisabled => 'غیر فعال';

  @override
  String get admin_typeText => 'متن';

  @override
  String get admin_fieldTitle => 'عنوان';

  @override
  String get admin_fieldMessage => 'پیغام';

  @override
  String get admin_fieldVideo => 'یوٹیوب لنک';

  @override
  String get admin_fieldVideoHint => 'https://www.youtube.com/watch?v=…';

  @override
  String get admin_fieldVideoHelp => 'صرف یوٹیوب ویڈیوز — شارٹس قبول نہیں۔';

  @override
  String get admin_fieldOrientation => 'اسکرین کی سمت';

  @override
  String get admin_fieldDuration => 'دورانیہ';

  @override
  String get admin_fieldIsEvent => 'یہ ایک تقریب ہے';

  @override
  String admin_fieldStartsAt(String date) {
    return 'آغاز $date';
  }

  @override
  String admin_fieldEndsAt(String date) {
    return 'اختتام $date';
  }

  @override
  String get admin_fieldShowOn => 'یہاں دکھائیں';

  @override
  String get admin_fieldMainScreen => 'مسجد کی مرکزی اسکرین';

  @override
  String get admin_fieldSecondaryScreen => 'ثانوی اسکرین';

  @override
  String get admin_fieldMobile => 'مواقیت ایپ';

  @override
  String get admin_pickImage => 'تصویر منتخب کریں';

  @override
  String get admin_replaceImage => 'تصویر تبدیل کریں';

  @override
  String get admin_orientationLandscape => 'افقی';

  @override
  String get admin_orientationPortrait => 'عمودی';

  @override
  String get admin_requiredField => 'لازمی';

  @override
  String get admin_invalidYoutube => 'یوٹیوب ویڈیو کا لنک درج کریں۔';

  @override
  String get admin_flashUnavailable =>
      'فوری پیغام میں ترمیم اس وقت دستیاب نہیں۔';

  @override
  String admin_unsavedChanges(int count) {
    return 'غیر محفوظ تبدیلیاں: $count';
  }

  @override
  String get admin_prayerSlot1 => 'پہلی نماز';

  @override
  String get admin_prayerSlot2 => 'دوسری نماز';

  @override
  String get admin_prayerSlot3 => 'تیسری نماز';

  @override
  String admin_minutes(int count) {
    return '$count منٹ';
  }

  @override
  String admin_seconds(int count) {
    return '$count سیکنڈ';
  }

  @override
  String admin_announcementCounts(
    int total,
    int maxTotal,
    int enabled,
    int maxEnabled,
  ) {
    return '$maxTotal میں سے $total · $maxEnabled میں سے $enabled فعال';
  }

  @override
  String get beta => 'بیٹا';

  @override
  String get for_mosque_administrators_only =>
      'صرف مواقيت مساجد کے منتظمین کے لیے';

  @override
  String get admins_only => 'صرف ایڈمنز';

  @override
  String get notification_permission_required => 'اطلاعات کی اجازت درکار ہے';

  @override
  String get enable_location => 'Enable Location';

  @override
  String get enable_notifications => 'اطلاعات فعال کریں';

  @override
  String get notifications_required_for_adhan =>
      'MAWAQIT کے لیے اذان چلانے اور نماز کی اطلاعات بھیجنے کے لیے اطلاعات کو فعال کرنا ضروری ہے.';

  @override
  String get location_required_for_prayer_times =>
      'درست نماز کے اوقات، مساجد کی معلومات اور موسم کی تازہ ترین صورتحال حاصل کرنے کے لیے مقام درکار ہے.';

  @override
  String get manage_mosque => 'مسجد کا انتظام';

  @override
  String get how_managing_your_mosque => 'How is managing your mosque going?';

  @override
  String get rating_helps_improve_admin =>
      'Your rating helps us improve the admin area.';

  @override
  String get tell_us_more => 'Tell us more (optional)';

  @override
  String get what_worked_well => 'What worked well, what did not?';

  @override
  String get thank_you_for_your_feedback => 'Thank you for your feedback.';

  @override
  String get could_not_send_your_feedback =>
      'Could not send your feedback. Please try again.';

  @override
  String get analog_clock => 'اینالاگ گھڑی';

  @override
  String get credit_card => 'کریڈٹ یا ڈیبٹ کارڈ';

  @override
  String get donate_paypal => 'PayPal کے ذریعے عطیہ کریں';

  @override
  String get or_bank_transfer => 'یا mawaqit.net پر بینک ٹرانسفر کے ذریعے';

  @override
  String get please_enter_valid_amount => 'براہِ کرم درست رقم درج کریں';

  @override
  String get payment_declined =>
      'ادائیگی مسترد کر دی گئی۔ کوئی دوسرا کارڈ یا عطیہ کرنے کا دوسرا طریقہ آزمائیں۔.';

  @override
  String get donations_unavailable =>
      'عطیات عارضی طور پر دستیاب نہیں ہیں۔ براہِ کرم کچھ دیر بعد دوبارہ کوشش کریں۔.';

  @override
  String get mute_with_volume_buttons => 'والیوم بٹن سے خاموش کریں';

  @override
  String get may_allah_reward_you => 'اللہ آپ کو جزائے خیر عطا فرمائے';

  @override
  String get thank_you_for_your_generosity => 'آپ کی سخاوت کا شکریہ';

  @override
  String get bluetooth_permission => 'بلوٹوتھ کی اجازت';

  @override
  String get bluetooth_permission_required_garmin =>
      'آپ کی Garmin گھڑی سے منسلک اور ہم آہنگ کرنے کے لیے درکار ہے۔';

  @override
  String get bayan_ul_quran_israr_ahmed => 'Bayan-ul-Quran (Dr. Israr Ahmed)';

  @override
  String get dr_abu_bakr_muhammad => 'Dr Abu Bakr Muhammad Zakaria';

  @override
  String get maulana_azizul_haque_al_umari => 'مولانا عزیز الحق العمری';

  @override
  String get hindi => 'ہندی';

  @override
  String get adhan_custom_volume_description =>
      'اذان کے لیے مخصوص والیوم لیول منتخب کریں۔ اذان ختم ہونے کے بعد آپ کے فون کا اصل والیوم بحال کر دیا جائے گا.';

  @override
  String get adhan_alarm_channel_description =>
      'اذان الارم چینل کے ذریعے چلائی جاتی ہے، اس لیے اسے سائلنٹ یا وائبریٹ موڈ میں، اور جب الارمز کی اجازت ہو تو ڈو ناٹ ڈسٹرب (DND) میں بھی سنا جا سکتا ہے۔';

  @override
  String get adhan_stop_with_volume_button_description =>
      'اپنے فون کا کوئی بھی والیوم بٹن دبا کر اذان فوراً بند کریں.';

  @override
  String get adhan_audio_interruption_description =>
      'مختصر اطلاعاتی آوازیں اذان کو بند نہیں کریں گی۔ اذان صرف اس وقت بند ہوگی جب فون کال آئے یا کوئی دوسری ایپ آڈیو کا کنٹرول سنبھال لے۔';
}
