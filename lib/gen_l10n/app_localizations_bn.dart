// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Bengali Bangla (`bn`).
class AppLocalizationsBn extends AppLocalizations {
  AppLocalizationsBn([String locale = 'bn']) : super(locale);

  @override
  String get bottomNavbar_mawaqit => 'মাওয়াকিত';

  @override
  String get bottomNavbar_favorites => 'প্রিয়';

  @override
  String get bottomNavbar_qibla => 'কিবলা';

  @override
  String get bottomNavbar_search => 'অনুসন্ধান';

  @override
  String get bottomNavbar_settings => 'বিন্যাস সমূহ';

  @override
  String get settings_ThemeAuto => 'অটো';

  @override
  String get settings_ThemeDark => 'অন্ধকার';

  @override
  String get settings_ThemeLight => 'উজ্জ্বল';

  @override
  String get settings_TimeFormat => 'সময় ফর্ম্যাট';

  @override
  String get settings_LanguageSelection => 'ভাষা নির্বাচন';

  @override
  String get settings_Language => 'ভাষা';

  @override
  String get settings_Theme => 'থিম';

  @override
  String get settings_TimeFormat12H => '১২ H';

  @override
  String get settings_TimeFormat24H => '২৪ H';

  @override
  String get settings_PrayerNotifications => 'প্রার্থনা বিজ্ঞপ্তি';

  @override
  String get settings_PrayerPreNotifications => 'প্রাক-নামাজ বিজ্ঞপ্তি';

  @override
  String get settings_ShuruqPreNotifications => 'প্রাক-শুরুক বিজ্ঞপ্তি';

  @override
  String get settings_Calculations => 'Calculations';

  @override
  String get settings_RateOurApp => 'আমাদের অ্যাপ্লিকেশন রেট';

  @override
  String get settings_WhatsNew => 'What\'s new';

  @override
  String get settings_SupportMawaqit => 'মাওয়াকিতকে সমর্থন করুন';

  @override
  String get settings_News => 'খবর';

  @override
  String get settings_MakeADonation => 'অনুদান করুন';

  @override
  String get bismillah => 'বিসমিল্লাহ';

  @override
  String get settings_DonateToMawaqit => 'মাওয়াকীতে দান করুন';

  @override
  String get settings_CheckForUpdates => 'হালনাগাদ এর জন্য অনুসন্ধান করুন';

  @override
  String get settings_Credits => 'ক্রেডিট';

  @override
  String get settings_AboutUs => 'আমাদের সম্পর্কে';

  @override
  String get settings_language_ar => 'আরবি';

  @override
  String get settings_language_bn => 'বাংলা';

  @override
  String get settings_language_fr => 'ফরাসি';

  @override
  String get settings_language_nl => 'ডাচ';

  @override
  String get settings_language_de => 'জার্মান';

  @override
  String get settings_language_es => 'স্পেনীয়';

  @override
  String get settings_language_en => 'ইংরেজি';

  @override
  String get settings_language_it => 'ইতালীয়';

  @override
  String get settings_language_tr => 'তুর্কি';

  @override
  String get settings_language_ru => 'রাশিয়ান';

  @override
  String get settings_language_id => 'ইন্দোনেশিয়ান';

  @override
  String get settings_language_ur => 'উর্দু';

  @override
  String get settings_language_pl => 'পোলিশ';

  @override
  String get settings_CreditsText =>
      'কপিরাইট © 2021 মাওয়াকিত ফাউন্ডেশনের জন্য মাওয়াকিট, সর্বস্বত্ব সংরক্ষিত।\n\nএই অ্যাপ্লিকেশনের মেধা সম্পত্তি এবং কপিরাইট বা এর প্লাগ-ইন, অ্যাড-অন বা বিষয়বস্তু মাওয়াকিট অ্যাসোসিয়েশনের মালিকানাধীন।\n\nএই সংস্করণটি শুধুমাত্র দাতব্য বিতরণের জন্য লাইসেন্সপ্রাপ্ত এবং এটি একটি ওয়াকফ। কোন বাণিজ্যিক ব্যবহার বা বাণিজ্যিক বিতরণ কঠোরভাবে নিষিদ্ধ.\n\nএই লাইসেন্সটি নির্দেশ করে না বা অন্তর্ভুক্ত করে না - যে কোনও উপায়ে - এই পণ্যের সাথে সম্পর্কিত কোনও অধিকার, এর কোনও অংশ, প্লাগ-ইন, অ্যাড-অন বা সামগ্রীর বিক্রয়। বরং, এই লাইসেন্সটি অ্যাপ্লিকেশন স্টোরের বর্তমান প্ল্যাটফর্মের মাধ্যমে অ্যাপ্লিকেশনটির শুধুমাত্র দাতব্য বিতরণের অনুমতি দেয়।\n\nসোর্স কোড বা বাইনারি ফাইলগুলি ব্যবহার করে অ্যাপ্লিকেশনটির পুনঃবন্টন, অথবা কোনো সংশোধনীর সাথে বা ছাড়াই বিপরীত প্রকৌশল ব্যবহার করে, লঙ্ঘনকারীকে বা এই কপিরাইট লঙ্ঘনের জন্য দায়ী ব্যক্তিকে আইনি পদক্ষেপের মুখোমুখি করবে।\n\nকপিরাইট ধারক এবং অ্যাপ্লিকেশনটির মালিক, মাওয়াকিট অ্যাসোসিয়েশনের জন্য মাওয়াকিটের অফিসিয়াল লিখিত অনুমতি ব্যতীত অ্যাপল বা গুগল প্লে-এর অফিসিয়াল স্টোরের বাইরে অ্যাপ্লিকেশনটি পুনরায় বিতরণ করার অধিকার কোনো পক্ষের নেই। এই অফিসিয়াল স্টোরের বাইরে এই অ্যাপ্লিকেশনটি বিতরণ করা মেধা সম্পত্তি সুরক্ষা আইনের লঙ্ঘন হবে এবং এই কপিরাইট লঙ্ঘনের জন্য দায়ী ব্যক্তিকে আইনি পদক্ষেপের মুখোমুখি করবে৷\n\nকপিরাইট মালিকের কাছ থেকে পূর্বে লিখিত অনুমতি না নিয়ে এই অ্যাপ্লিকেশন বা অন্য কোনো পণ্যের অনুরূপ বা প্রাপ্ত পণ্যের প্রচারের জন্য মাওয়াকিট ফাউন্ডেশনের জন্য আবেদনের নাম বা ব্র্যান্ড বা কপিরাইট ধারক মাওয়াকিতের ট্রেডমার্ক বা নাম ব্যবহার করার অনুমতি নেই।\nএই অ্যাপ্লিকেশনটি কপিরাইট ধারক এবং শেয়ারহোল্ডারদের দ্বারা \"যেমন আছে\" বিতরণ করার লাইসেন্সপ্রাপ্ত।\n\nঅ্যাপ্লিকেশনটির বিকাশকারী এবং মালিক কোম্পানি কোনো সুস্পষ্ট বা অন্তর্নিহিত ওয়ারেন্টি দেয় না, যার মধ্যে - কিন্তু সীমাবদ্ধ নয় - একটি নির্দিষ্ট উদ্দেশ্যে অ্যাপ্লিকেশনের উপযুক্ততার জন্য বিজ্ঞাপন এবং প্রচার করা। যে কোনো ক্ষেত্রে, মাওয়াকিট অ্যাসোসিয়েশনের জন্য মাওয়াকিট, বা শেয়ারহোল্ডাররা প্রত্যক্ষ, পরোক্ষ, বা দুর্ঘটনাজনিত (বিশেষ, সতর্কতামূলক, পরিণতিমূলক) কোনো ক্ষতির জন্য দায়ী নয়, যার মধ্যে - কিন্তু সীমাবদ্ধ নয় - ব্যবহার, ডেটা বা লাভের ক্ষতি; বা ব্যবসায়িক বাধা, কারণ এবং দায় যাই হোক না কেন, এই দায়গুলি চুক্তিভিত্তিক, নিখুঁত বা ক্ষতিকর (অবহেলা বা অন্যথা সহ) হতে পারে - যা এই অ্যাপ্লিকেশনটি ব্যবহার করে - একভাবে বা অন্যভাবে হয়৷\n\nআবেদনের কোনো তৃতীয় পক্ষের কপিরাইট তাদের ধারকের কাছে সংরক্ষিত।\n\nছবি এবং পটভূমি\n\nকপিরাইট Shutterstock যায়\nআইকন - CC BY 4.0 লাইসেন্স\n\nকপিরাইট Flaticons এর কাছে যায়\nMIT লাইসেন্স সমস্ত নন-ফন্ট এবং নন-আইকন ফাইলের জন্য প্রযোজ্য।\nহরফ - SIL OFL 1.1 লাইসেন্স\n\nকপিরাইট Google ফন্টে যায় (রোবোটো ফন্ট)\nকপিরাইট Apple, Cupertino প্যাকেজ\nকোড, প্যাকেজ এবং ফ্রেমওয়ার্ক\n\nGoogle দ্বারা ফ্লাটার\nপ্যাকেজ: http, geolocator, flutter_platform_widgets, shared_preferences, provider, flutter_local_notifications, audioplayers, toggle_switch, cupertino_icons, marquee_widget, intl, flutter_switch, json_annotation';

  @override
  String get settings_AboutUsText =>
      'এই অ্যাপ্লিকেশনটি MAWAQIT অ্যাসোসিয়েশন দ্বারা উন্নত করা হয়েছে.\n\nমাওয়াকিট অ্যাসোসিয়েশন ফ্রান্সে 2021 সালে তৈরি একটি অলাভজনক সংস্থা।\nআমাদের উদ্দেশ্য হল সফ্টওয়্যার এবং ওয়েবসাইটগুলির মতো আইটি এবং ডিজিটাল প্রযুক্তি সম্পর্কিত উদ্ভাবনী সমাধানগুলির বিকাশ; ডিজিটাল সমাধান এবং আইটি সরঞ্জাম শেখান এবং প্রশিক্ষণ.\n\nমাওয়াকিত ইকোসিস্টেম হল আমাদের ফ্ল্যাগশিপ প্রোডাক্ট এবং এর গল্পটি 2016 সালে শুরু হয়েছিল, মসজিদে নিবেদিত বড় স্ক্রিনে একটি অ্যাপ্লিকেশন, তারপর একটি মোবাইল অ্যাপ, একটি হোম-অ্যাসিস্ট্যান্ট অ্যাড-অন এবং একটি আলেক্সা দক্ষতা ইত্যাদি।\n\nমাওয়াকিট আপনাকে নামাজের সময় ট্র্যাক এবং পরিচালনা করার একটি নতুন উপায় অফার করে, আসলে আমরা একটি এন্ড-টু-এন্ড সিস্টেম অফার করি যা মসজিদ পরিচালকদের একটি অনলাইন টুল 24/24ঘন্টা উপলব্ধ করে যা তাদের সময়সূচী, মসজিদের খবর এবং অন্যান্য অনেক বৈশিষ্ট্য পরিচালনা করতে দেয়। বিশ্বস্তরা, তাদের পক্ষ থেকে, একটি মোবাইল অ্যাপ্লিকেশন থেকে উপকৃত হয় যা তাদের তাদের প্রিয় মসজিদের সঠিক এবং আনুমানিক সময়সূচী, সেইসাথে সংবাদ এবং অন্যান্য কার্যকারিতা যেমন ভৌগলিক অবস্থান, আযান, আযান, কিবলা ... ইত্যাদি। আমরা নির্ভরযোগ্যতা এবং গুণমানকে আমাদের প্রধান মান করেছি। আমাদের উচ্চাকাঙ্ক্ষা পরিষ্কার: প্রযুক্তি এবং ডিজাইনের জন্য আমাদের মসজিদের জন্য সেরা পরিষেবা তৈরি করা।\n\nআপনি যদি এই অ্যাপ্লিকেশনটি পছন্দ করেন তবে অনুগ্রহ করে দোকানে আপনার মতামত দিন।';

  @override
  String get fajr => 'ফজর';

  @override
  String get shuruq => 'সূর্যোদয়';

  @override
  String get duhr => 'দুহর';

  @override
  String get asr => 'আসর';

  @override
  String get maghrib => 'মাগরিব';

  @override
  String get isha => 'ইশা';

  @override
  String get sabah => 'সকাল';

  @override
  String get settings_PrayerNotification_NoNotification =>
      'আপনি বর্তমানে এই প্রার্থনার জন্য অবহিত না হওয়ার জন্য সেট আপ করেছেন৷';

  @override
  String get settings_PrayerNotification_Bip =>
      'বিজ্ঞপ্তির উপর বীপ রিংটোন অ্যাপ্লিকেশন।';

  @override
  String get adhan => 'আযান';

  @override
  String get adhan_Makkah_fajr => 'মক্কা (ফজর)';

  @override
  String get adhan_Madinah => 'মদীনা';

  @override
  String get adhan_Qods => 'কওদস';

  @override
  String get adhan_AlAfassy => 'আল আফাসি';

  @override
  String get adhan_AlAfassy_fajr => 'আল আফাসি (ফজর)';

  @override
  String get adhan_Muhammad_jazy => 'মুহাম্মাদ জাযী';

  @override
  String get adhan_AbdAlBassat => 'আব্দুল বাসিত';

  @override
  String get adhan_type =>
      'এক প্রকার আযান বেছে নিন। আযান বিজ্ঞপ্তি আছে এমন প্রতিটি নামাজে এই পছন্দটি প্রয়োগ করা হবে।';

  @override
  String get settings_prayer_Notification_None => 'কোনোটিই নয়';

  @override
  String get prayer_Notification_NoNotification =>
      'আপনি এই প্রার্থনার জন্য কোন বিজ্ঞপ্তি সেট করেননি।';

  @override
  String get prayer_PreNotification_Description =>
      'এই সেটিং আপনাকে প্রার্থনার আগে একটি বিজ্ঞপ্তি সংজ্ঞায়িত করতে দেয়।';

  @override
  String get search_TryParis => 'প্যারিস, এসসালাম ইত্যাদি চেষ্টা করুন।';

  @override
  String get search_ByGPS => 'জিপিএস দ্বারা অনুসন্ধান করুন';

  @override
  String get search_HelpText =>
      'আপনি লিখে একটি মসজিদ অনুসন্ধান করতে পারেন:\n- মসজিদের নাম\n- সমিতির নাম\n- শহর, ঠিকানা, দেশ...ইত্যাদি।\n\nঅথবা আপনি আশেপাশের মসজিদগুলি খুঁজতে লোকেটিং বোতামটি চাপতে পারেন।';

  @override
  String get search_MaxFavMosquesReached =>
      'প্রিয় মসজিদের সর্বোচ্চ সীমানায় পৌঁছে গেছেন।';

  @override
  String get search_iOs14_message =>
      'iOS 14 এ GPS ব্যবহার করে অনুসন্ধান করতে কিছু সময় লাগতে পারে, অনুগ্রহ করে অপেক্ষা করুন।';

  @override
  String get search_error => 'Oops an internal error has occurred\'';

  @override
  String get search_your_location => 'Your location';

  @override
  String get search_location_disabled =>
      'Please look up for a mosque by name or city, or allow location manually in your settings.';

  @override
  String get search_settings => 'Settings';

  @override
  String get no_services_text => 'মসজিদগুলি এই তথ্য সরবরাহ করেনি';

  @override
  String get welcome => 'স্বাগতম';

  @override
  String get mawaqit => 'মাওয়াকিট';

  @override
  String get connecting => 'মুসলিমদের সংযোগ স্থাপন';

  @override
  String get to_mosques => 'মসজিদের সাথে';

  @override
  String get get_started => 'শুরু করুন';

  @override
  String get get_prayer_notifications_for => 'নামাজের জন্য নোটিফিকেশন পান';

  @override
  String get turn_on_notifications => 'প্রার্থনা বিজ্ঞপ্তি চালু করুন';

  @override
  String get no_thanks => 'না ধন্যবাদ';

  @override
  String get notification_denied =>
      'আপনি বিজ্ঞপ্তির অনুমতি অস্বীকার করেছেন, তাই আপনি কোনো বিজ্ঞপ্তি পাবেন না।';

  @override
  String get at_least_one_mosque => 'চালিয়ে যেতে অন্তত একটি মসজিদ যোগ করুন';

  @override
  String get set_up_your_prayer => 'আপনার নামাজ স্থাপন করুন';

  @override
  String get notifications => 'বিজ্ঞপ্তিগুলি';

  @override
  String get mosques_around_you => 'আপনার চারপাশে মসজিদ';

  @override
  String get mosques_in_the_selected_area => 'নির্বাচিত এলাকায় মসজিদ';

  @override
  String get mosques_with => 'সঙ্গে মসজিদ';

  @override
  String get allow_mawaqit_to_access_my_location =>
      'মাওয়াকিতকে আমার অবস্থান অ্যাক্সেস করার অনুমতি দিন.';

  @override
  String get enable_location_service_from_app_settings =>
      'অবস্থান সক্রিয় করুন';

  @override
  String get the_request_timeout => 'অনুরোধ সময় শেষ হয়েছে';

  @override
  String get no_internet_connection => 'ইন্টারনেট সংযোগ নেই';

  @override
  String get try_again => 'আবার চেষ্টা কর';

  @override
  String get virtual_mosque => 'ভার্চুয়াল মসজিদ';

  @override
  String get no_mosque_in_selected_area =>
      'নির্বাচিত এলাকায় কোন মসজিদ খুঁজে পাওয়া যায়নি';

  @override
  String get no_mosque_with => 'সাথে কোন মসজিদ খুঁজে পাইনি';

  @override
  String get retry => 'পুনরায় চেষ্টা করা';

  @override
  String get search_here => 'এখানে অনুসন্ধান করুন';

  @override
  String get enter_at_least_3_char => 'অনুসন্ধান করতে কমপক্ষে 3টি অক্ষর লিখুন৷';

  @override
  String get jumua1 => 'জুমা ১';

  @override
  String get prayer_times => 'নামাজের সময়';

  @override
  String get remove_this_city => 'এই শহরকে সরিয়ে দাও';

  @override
  String get add_this_city => 'এই শহর যোগ করুন';

  @override
  String get add_this_mosque => 'এই মসজিদকে যোগ করুন';

  @override
  String get remove_this_mosque => 'এই মসজিদ সরিয়ে দাও';

  @override
  String get services => 'সেবা';

  @override
  String get share => 'শেয়ার করুন';

  @override
  String get itinerary => 'ভ্রমণসূচী';

  @override
  String get call => 'কল';

  @override
  String get dnd => 'বিরক্ত করবেন না';

  @override
  String get battery_optimize => 'ব্যাটারি অপ্টিমাইজেশান';

  @override
  String get power_saver => 'পাওয়ার সেভার';

  @override
  String get permission_message1 =>
      'আমরা একটি বিলম্বিত বিজ্ঞপ্তি সনাক্ত করেছি, দয়া করে বন্ধ করুন৷';

  @override
  String get permission_message2 => 'বিজ্ঞপ্তি বিলম্ব সনাক্ত করা হয়েছে';

  @override
  String get notifications_delay_detected => 'বিজ্ঞপ্তির বিলম্ব শনাক্ত হয়েছে';

  @override
  String get open_setting => 'ওপেন সেটিংস';

  @override
  String get noJumua => 'কোনো জুমা নেই';

  @override
  String get jumua => 'জুমা';

  @override
  String get qibla_sensor =>
      'আপনার ডিভাইসের সেন্সরটি যা MAWAQIT কিবলা নির্ধারণের জন্য ব্যবহার করে তা প্রত্যাশিত হিসেবে কাজ করছে না, যা আমাদের কিবলা দিক প্রদর্শন ক্ষমতার উপর প্রভাব ফেলছে।';

  @override
  String get connection_not_found =>
      'কম্পাস ক্যালিব্রেশন সেটিংস নিষ্ক্রিয় করা হয়েছে। আপনার সংযোগ চালু আছে কিনা তা নিশ্চিত করুন এবং দিক পরিবর্তন করে আবার চেষ্টা করুন।';

  @override
  String get currently_located_at => 'বর্তমানে অবস্থান করছে';

  @override
  String get facing_makkah => 'আপনি মক্কার দিকে মুখ করে আছেন';

  @override
  String get not_facing_Makkah => 'আপনি মক্কার দিকে মুখ করে নেই';

  @override
  String get qibla_Error_retry => 'পুনরায় চেষ্টা করা';

  @override
  String get qibla_Error_LocationDenied =>
      'অবস্থান পরিষেবার অনুমতি অস্বীকার করা হয়েছে৷';

  @override
  String get qibla_Error_LocationDeniedForever =>
      'অবস্থান পরিষেবা চিরতরে অস্বীকার!';

  @override
  String get qibla_Error_PleaseEnable =>
      'অনুগ্রহ করে অবস্থান পরিষেবা সক্ষম করুন৷';

  @override
  String get qibla_Direction => 'QIBLA\nঅভিমুখ';

  @override
  String get favorites_FavoriteMosques => 'প্রিয় মসজিদ';

  @override
  String get favorites_Error => 'কিছু ভুল হয়েছে...';

  @override
  String get announcement_Image => 'ছবি';

  @override
  String get announcement_Type_Video => 'ভিডিও';

  @override
  String get announcement_Type_FlashInfo => 'ফ্ল্যাশ তথ্য';

  @override
  String get mosque_DonateText1 => 'দান করুন';

  @override
  String get mosque_DonateText2 => 'মসজিদে';

  @override
  String get mosque_Error_CouldNotLaunch => 'চালু করা যায়নি';

  @override
  String get mosque_PrayerTimes => 'নামাজের সময়';

  @override
  String get mosque_Website => 'ওয়েবসাইট';

  @override
  String get mosque_Address => 'ঠিকানা';

  @override
  String get mosque_Phone => 'মোবাইল';

  @override
  String get mosque_Email => 'ইমেইল';

  @override
  String get mosque_ByMawaqit => 'মাওয়াকিত https://mawaqit.net দ্বারা';

  @override
  String get mosque_Facilities_Parking => 'পার্কিং';

  @override
  String get mosque_Facilities_DisabledAccess => 'বেশিরভাগ খারাপ';

  @override
  String get mosque_Facilities_AblutionsRoom => 'ওযুর ঘর';

  @override
  String get mosque_Facilities_WomanSpace => 'নারীর স্থান';

  @override
  String get mosque_Facilities_AdultsCourse => 'প্রাপ্তবয়স্কদের কোর্স';

  @override
  String get mosque_Facilities_ChildrenCourses => 'শিশুদের কোর্স';

  @override
  String get mosque_Facilities_SalatAlAid => 'সালাত আল ঈদ';

  @override
  String get mosque_Facilities_SalatAlJanaza => 'সালাত আল জানাজা';

  @override
  String get mosques_Facilities_IftarRamadan => 'রমজানের ইফতার';

  @override
  String get mosque_facilities_Info =>
      'এই সমস্ত তথ্য মসজিদের দায়িত্বে থাকা ব্যক্তিদের দ্বারা পূরণ করা হয়';

  @override
  String get in_ => 'ভিতরে';

  @override
  String get imsak => 'ইমসাক';

  @override
  String get eid => 'এ ঈদের নামাজ';

  @override
  String get minutes => 'মিনিট';

  @override
  String get jumua2 => 'জুমা ২';

  @override
  String get please_update =>
      'MAWAQIT-এর একটি নতুন সংস্করণ উপলব্ধ, অনুগ্রহ করে আপডেট করুন';

  @override
  String get update => 'হালনাগাদ';

  @override
  String get tutorial_skip => 'টিউটোরিয়াল এড়িয়ে যান';

  @override
  String get tutorial_next => 'পরবর্তী';

  @override
  String get tutorial_title_main_mosque => 'আপনার প্রধান মসজিদ';

  @override
  String get tutorial_text_main_mosque =>
      'আপনার প্রধান মসজিদ এখানে দেখান.\nআপনি আপনার প্রিয় মসজিদগুলির মধ্যে স্যুইচ করতে বাম এবং ডানদিকে স্লাইড করতে পারেন।';

  @override
  String get tutorial_title_countdown => 'পরবর্তী প্রার্থনা সূচক';

  @override
  String get tutorial_text_countdown =>
      'এই কাউন্টডাউন আপনাকে পরবর্তী নামাজের আগে বাকি সময় দেখায়।';

  @override
  String get tutorial_title_mosque_info => 'মসজিদ তথ্য';

  @override
  String get tutorial_text_mosque_info =>
      'আপনার মসজিদ সম্পর্কে সবকিছু: সুবিধা, ঠিকানা, ফোন, মেইল, দান এবং আরও অনেক কিছু।';

  @override
  String get tutorial_title_announcements => 'মসজিদের বার্তা এবং ঘোষণা';

  @override
  String get tutorial_text_announcements =>
      'আপনার মসজিদ থেকে বার্তা এবং ঘোষণা সম্পর্কে বিজ্ঞপ্তিগুলি এখানে প্রদর্শিত হবে৷\nআপনি এই বোতামটি আলতো চাপলে, আপনি সম্পূর্ণ বিবরণ দেখতে পাবেন। এই ঘোষণাগুলি একটি ফ্ল্যাশ বার্তা, একটি চিত্র বা এমনকি একটি ভিডিও হতে পারে৷';

  @override
  String get onboarding_title_1 => 'আপনার ভাষা নির্বাচন করুন';

  @override
  String get onboarding_text_1 =>
      'নীচের তালিকা থেকে আপনার পছন্দের ভাষা নির্বাচন করুন';

  @override
  String get onboarding_title_2 => 'মাওয়াকিতে স্বাগতম';

  @override
  String get onboarding_text_2 =>
      'মাওয়াকিত হল একটি ইকোসিস্টেম যা আপনাকে নামাজের সময় ট্র্যাক এবং পরিচালনা করার জন্য একটি নতুন উপায় সরবরাহ করে, আপনার হাতে থাকা অ্যাপ্লিকেশনটি এই ইকোসিস্টেমের অংশ, এটি আপনাকে মসজিদ খুঁজে পেতে এবং এর খবর এবং নামাজের সময়গুলি অনুসরণ করতে দেয়';

  @override
  String get onboarding_title_3 => 'একটি গাঢ়/হালকা থিম নির্বাচন করুন';

  @override
  String get onboarding_text_3 =>
      'আপনি কম আলোর পরিবেশে থাকুন বা আপনার ফোনে গাঢ় ব্যাকগ্রাউন্ডের রঙ পছন্দ করুন, আপনি উন্নত ভিজ্যুয়াল আরামের জন্য ডার্ক মোডের সুবিধা নিতে পারেন';

  @override
  String get onboarding_title_4 => 'আপনার প্রধান মসজিদ যোগ করুন';

  @override
  String get onboarding_text_4 =>
      'নীচের তালিকা থেকে আপনার প্রধান মসজিদ বাছাই করুন অথবা আপনার বর্তমান অবস্থানের কাছাকাছি একটি মসজিদ অনুসন্ধান করুন';

  @override
  String get loading => 'লোডিং';

  @override
  String get loading_please_wait =>
      'প্রেরণ করা হচ্ছে, অনুগ্রহ করে অপেক্ষা করুন';

  @override
  String get loading_loaded => 'লোডিং সম্পূর্ণ';

  @override
  String get favorites_edit => 'প্রিয় মসজিদ সম্পাদনা করুন';

  @override
  String get minutes_to_athan => 'এথান থেকে মিনিট';

  @override
  String get show_tutorial => 'টিউটোরিয়াল পর্যালোচনা করুন';

  @override
  String get onboarding_chose_mosque_please =>
      'অনুগ্রহ করে অনুসন্ধান তালিকা থেকে একটি মসজিদ বেছে নিন';

  @override
  String get done => 'সম্পন্ন';

  @override
  String get ok => 'ঠিক আছে';

  @override
  String get km => 'কিমি';

  @override
  String get notification_open_app_title => 'অ্যাপটি খুলুন';

  @override
  String get notification_open_app_body => 'বিজ্ঞপ্তি বজায় রাখতে অ্যাপ খুলুন';

  @override
  String get tutorial_title_notification =>
      'একটি প্রার্থনা বিজ্ঞপ্তি নির্বাচন করুন';

  @override
  String get tutorial_text_notification =>
      'এখানে আপনি একটি ধরনের বিজ্ঞপ্তি, আথান, বিপ বা নীরব নির্বাচন করতে পারেন';

  @override
  String get search_no_result =>
      'একজন মাওয়াকিত রাষ্ট্রদূত হন এবং আপনার মসজিদে আমাদের সমাধান অফার করুন, এটি বিনামূল্যে\nhttps://mawaqit.net';

  @override
  String get rate_popin_title => 'মাওয়াক্বিতকে হার দিন';

  @override
  String get rate_popin_message =>
      'আপনি যদি এই অ্যাপটি পছন্দ করেন, দয়া করে এটি পর্যালোচনা করতে আপনার একটু সময় নিন!\nএটা সত্যিই আমাদের সাহায্য করে.';

  @override
  String get rate_popin_rate => 'হার';

  @override
  String get rate_popin_no => 'না ধন্যবাদ';

  @override
  String get rate_popin_later => 'পরে হতে পারে';

  @override
  String get message_see_all =>
      'আপনি স্ক্রিনের উপরের স্পীকারে ক্লিক করে এই মসজিদ থেকে সমস্ত বার্তা দেখতে পারেন।';

  @override
  String get adhan_Makkah => 'মক্কা';

  @override
  String get tutorial_title_iqama => 'ইকামা';

  @override
  String get tutorial_text_iqama =>
      'এখানে আপনি মসজিদে আযান এবং ইকামার মধ্যে সময়কাল পাবেন';

  @override
  String get calendar_tooltip_download => 'ক্যালেন্ডার ডাউনলোড করুন';

  @override
  String get calendar_download_permission_refused =>
      'ডাউনলোডের অনুমতি অবশ্যই প্রত্যাখ্যান করা হয়েছে, দয়া করে ডাউনলোড করার অনুমতি দিন';

  @override
  String get calendar_downloading => 'ক্যালেন্ডার ডাউনলোড হচ্ছে...';

  @override
  String get location_service_disabled =>
      'অবস্থান পরিষেবা অক্ষম করা হয়েছে, দয়া করে এটি সক্ষম করুন৷';

  @override
  String get location_permission_denied =>
      'অবস্থানের অনুমতি অস্বীকার করা হয়েছে, অনুগ্রহ করে অনুমতি দিন।';

  @override
  String get location_permission_denied_forever =>
      'অবস্থানের অনুমতি চিরতরে অস্বীকৃত, দয়া করে সেটিংস পৃষ্ঠায় অনুমতি দিন৷';

  @override
  String get at_least_one_mosque_error => 'আপনার অন্তত একটি মসজিদ থাকা উচিত।';

  @override
  String get and => 'এবং';

  @override
  String get continue_to_app => 'চালিয়ে যান';

  @override
  String get add => 'যোগ করুন';

  @override
  String get mosque => 'মসজিদ';

  @override
  String get on_boarding_nearest_mosque => 'চলুন নিকটতম মসজিদটি খুঁজে বের করি';

  @override
  String get on_boarding_nearest_mosque_text =>
      'আপনার অবস্থানের উপর ভিত্তি করে আশেপাশের এই মসজিদ গুলো পাওয়া গিয়েছে';

  @override
  String get search_no_result_for_text =>
      'এগুলোর জন্য আমরা কোনো ফলাফল খুঁজে পাইনি';

  @override
  String get surah_audio_downloading => 'সুরা অডিও ডাউনলোড হচ্ছে ';

  @override
  String get msg_internet_connection => 'আপনার ইন্টারনেট সংযোগ নেই';

  @override
  String get read_quran => 'কুরআন পড়ুন';

  @override
  String get ayahs => 'আয়াত';

  @override
  String get chapter => 'সুরা';

  @override
  String get meaning => 'অর্থ';

  @override
  String get al_quran => 'আল কুরআন ';

  @override
  String get surah_information => 'সুরা বিষয়ক তথ্য';

  @override
  String get surah_number => 'সুরা নাম্বার';

  @override
  String get quran_browsing => 'ব্রাউজিং';

  @override
  String get quran_search_placeholder => 'সুরা নাম, নাম্বার.. দিয়ে খুঁজুন';

  @override
  String get quran_verse_search => 'ভার্স খুঁজুন';

  @override
  String get quran_translation => 'অনুবাদ';

  @override
  String get quran_mushaf => 'মুসহাফ';

  @override
  String get quran_juzz => 'পারা';

  @override
  String get ayah => 'আয়াত';

  @override
  String get quran_verses => 'ভার্সেস';

  @override
  String get surah => 'সুরা';

  @override
  String get continue_reading => 'পড়া চালিয়ে যান';

  @override
  String get bookmarks => 'বুকমার্ক';

  @override
  String get juzz => 'পারা';

  @override
  String get quran_page => 'পৃষ্ঠা';

  @override
  String get quran_turning_page => 'পাতা উলটানো';

  @override
  String get quran_from_right_to_left => 'ডান থেকে বামে';

  @override
  String get quran_from_left_to_right => 'বাম থেকে ডানে';

  @override
  String get quran_from_top_to_bottom => 'উপর থেকে নিচে';

  @override
  String get quran_from_bottom_to_top => 'নিচে থেকে উপরে';

  @override
  String get quran_pages_color => 'পাতার রঙ';

  @override
  String get english_tr => 'ইংরেজি';

  @override
  String get french_tr => 'ফ্রেঞ্চ';

  @override
  String get spanish_tr => 'স্প্যানিশ';

  @override
  String get muhammad_hamidullah => 'মুহাম্মদ হামিদুল্লাহ';

  @override
  String get abdullah_yusuf_ali => 'আবদুল্লাহ ইউসুফ আলী';

  @override
  String get sheikh_isa_garcia => 'শাইখ ঈসা গার্সিয়া';

  @override
  String get reciters => 'ক্বারীগণ';

  @override
  String get favorite_reciters => 'প্রিয় ক্বারীগণ';

  @override
  String get surahs_downloaded => 'ডাউনলোড কৃত সুরাসমুহ';

  @override
  String get surahs_chapter => 'সুরা- অধ্যায়';

  @override
  String get my_likes => 'আমার পছন্দসমুহ';

  @override
  String get cancel => 'বাতিল';

  @override
  String get next => 'পরবর্তী';

  @override
  String get search_surah => 'সুরা খুজুন';

  @override
  String get not_downloaded_recitation => 'আপনি কোনো তেলাওয়াত ডাউনলোড করেন নি';

  @override
  String get not_favorite_reciter =>
      'আপনি এখনো কোনো তেলাওয়াত কারিকে পছন্দের তালিকায় যোগ করেন নি।';

  @override
  String get search_reciter => 'তেলাওয়াতকারি খুঁজুন';

  @override
  String get al_quran_al_kareem => 'আল কুরআন কারিম';

  @override
  String get surahs => 'সুরা সমুহ';

  @override
  String get chapters => 'অধ্যায়';

  @override
  String get calculation_info =>
      'এই অংশের সেটিংস শুধুমাত্র আপনার অবস্থানের (জিওলোকেশন) ভিত্তিতে হিসাব করা নামাজের সময়ের সাথে সম্পর্কিত, মসজিদের নামাজের সময়ের সাথে নয়.';

  @override
  String get calculation_madhab => 'ফিকাহ - মাজহাব';

  @override
  String get calculation_madhab_shafii => 'শাফীঈ, হানবালি এবং মালিকি';

  @override
  String get calculation_madhab_shafii_countries =>
      'জর্ডান, ফিলিস্তিন, ইয়েমেন, সোমালিয়া, ইরিত্রিয়া, মালয়েশিয়া, ইন্দোনেশিয়া, ব্রুনেই, দক্ষিণ মিশর, সৌদি আরব, কাতার, উত্তর আফ্রিকা, পশ্চিম আফ্রিকা, সংযুক্ত আরব আমিরাত, কুয়েত...';

  @override
  String get calculation_madhab_hanafi => 'হানাফি';

  @override
  String get calculation_madhab_hanafi_countries =>
      'লেভান্ট, পাকিস্তান, আফগানিস্তান, ভারত, বাংলাদেশ, ইরাক, তুরস্ক, রাশিয়া, চীন, বালকান...';

  @override
  String get calculation_methods => 'ক্যালকুলেশ এর পদ্ধতিসমুহ';

  @override
  String get calculation_methods_france => 'ফ্রান্স এর মুসলিম (ex UOIF)';

  @override
  String get calculation_methods_algeria =>
      'আলজেরিয়ান মিনিস্ট্রি অফ রিলিজিয়াস এফেয়ার এন্ড ওয়াকফ';

  @override
  String get calculation_methods_morocco =>
      'আলজেরিয়ান মিনিস্ট্রি অফ ওয়াকফ এন্ড রিলিজিয়াস এফেয়ারস';

  @override
  String get calculation_methods_muslim_world => 'মুসলিম ওয়ার্ল্ড লীগ';

  @override
  String get calculation_methods_egyptian => 'মিশরীয় জেনারেল অথরিটি অফ সার্ভে';

  @override
  String get calculation_methods_karachi =>
      'ইউনিভার্সিটি অব ইসলামিক সায়েন্সেস, করাচি';

  @override
  String get calculation_methods_umm_al_qura =>
      'উম্মুল কুরা বিশ্ববিদ্যালয়, মক্কা';

  @override
  String get calculation_methods_dubai => 'গালফ অঞ্চল';

  @override
  String get calculation_methods_moon => 'চাঁদ দেখা কমিটি';

  @override
  String get calculation_methods_north_america =>
      'ইসলামিক সোসাইটি অফ নর্থ আমেরিকা';

  @override
  String get calculation_methods_kuwait => 'কুয়েত';

  @override
  String get calculation_methods_qatar => 'কাতার';

  @override
  String get calculation_methods_singapore => 'সিংগাপুর';

  @override
  String get calculation_methods_turkey => 'তুর্কী';

  @override
  String get calculation_methods_tehran =>
      'তেহরান বিশ্ববিদ্যালয়, ইনস্টিটিউট অব জিওফিজিক্স';

  @override
  String get calculation_methods_russia =>
      'স্পিরিচুয়াল এডমিনিস্ট্রেশন অফ মুসলিমস ইন রাশিয়া';

  @override
  String get calculation_methods_custom => 'কাস্টম এংগেল';

  @override
  String get calculation_high_latitude => 'উচ্চতর অক্ষাংশ';

  @override
  String get calculation_high_latitude_none => 'কোনটাই না';

  @override
  String get calculation_high_latitude_middle => 'মধ্যরাতে';

  @override
  String get calculation_high_latitude_one_seventh => 'রাতের এক সপ্তমাংশ';

  @override
  String get calculation_high_latitude_angle_based => 'এংগেল বেজড পদ্ধতি';

  @override
  String get calculation_adjustment => 'ম্যানুয়াল সংশোধন';

  @override
  String get calculation_adjustment_subtitle =>
      'নামাজের সমন্বয় এবং কাস্টম এংগেল';

  @override
  String get calculation_adjustment_prayers => 'নামাজের সমন্বয়';

  @override
  String get calculation_adjustment_prayers_minutes => 'মিনিট';

  @override
  String get calculation_adjustment_angles => 'কাস্টম এংগেল';

  @override
  String get calculation_prayer_custom_angle => 'জন্য কাস্টম কোণ';

  @override
  String get error_download_quran_failed => 'কুরআন ডাউনলোড করতে ব্যার্থ হয়েছে';

  @override
  String get downloading_quran => 'কুরআন ডাউনলোড হচ্ছে...';

  @override
  String get processing => 'প্রক্রিয়াধীন....';

  @override
  String get copied_to_clipboard => 'ক্লিপবোর্ড এ কপি হয়েছে!';

  @override
  String get download_quran_popup_title => 'সর্বশেষ সজানো';

  @override
  String get download_quran_popup_downloading_1 =>
      'আমাদের করণীয় হলো কোরআনের পৃষ্ঠা ডাউনলোড করা, এটা কেবল কিছু সেকেন্ড নেবে। আমরা আপনার মোবাইল ডেটা ব্যবহার করব, যদি প্রয়োজন হয় তবে Wi-Fi-তে স্যুইচ করুন।';

  @override
  String get download_quran_popup_downloading_2 =>
      'আমরা কেবল আপনার জন্য কোরআন তৈরি করছি, দয়া করে কিছু সেকেন্ড অপেক্ষা করুন!';

  @override
  String get download_quran_popup_downloading_3 =>
      'Discover the Quran\'s beauty! Tap \'Download\' to access its timeless wisdom and be inspired by its divine words.';

  @override
  String get download_quran_popup_confirm_cancel =>
      'আপনি ডাউনলোড বাতিল করতে চান কি? এই ডেটা ছাড়া আপনি কোরআন পড়তে পারবেন না, পৃষ্ঠা অনুপলব্ধ হবে।';

  @override
  String get download_quran_popup_cancelled =>
      'কিছু ভুল হয়েছে, দয়া করে আবার ডাউনলোড বোতাম চাপুন!';

  @override
  String get download => 'ডাউনলোড';

  @override
  String get continue_download => 'ডাউনলোড চালিয়ে যান';

  @override
  String get cancel_download => 'ডাউনলোড বাতিল করুন';

  @override
  String get download_completed => 'ডাউনলোড সম্পন্ন হয়েছে';

  @override
  String get download_failed => 'ডাউনলোড ব্যর্থ হয়েছে';

  @override
  String get download_pages => 'পৃষ্ঠা ডাউনলোড করুন';

  @override
  String get notification => 'বিজ্ঞপ্তি';

  @override
  String get notification_not_supported_below_7 =>
      'আপনার ডিভাইস Android 7 বা তাতো নেটওয়ার্কের নোটিফিকেশন প্রাপ্ত করতে পারবেন না।';

  @override
  String get battery_optimization => 'ব্যাটারি অপ্টিমাইজেশন';

  @override
  String get battery_optimization_subtitle =>
      'প্রার্থনা সময়ের বিজ্ঞপ্তি সমস্যার নেটিভ অপটিমাইজেশন বন্ধ করতে, আপনার ডিভাইসের ব্যাটারি অপটিমাইজেশন বন্ধ করতে হবে।';

  @override
  String get allow => 'অনুমতি দিন';

  @override
  String get deny => 'অস্বীকার করুন';

  @override
  String get force_notification => 'ফোর্স নোটিফিকেশন';

  @override
  String get tutorial_text_foreground_service =>
      'এটিতে ট্যাপ করলে আপনি \'ফোরগ্রাউন্ড সার্ভিস\' চালু বা বন্ধ করতে পারবেন';

  @override
  String get enable => 'চালু';

  @override
  String get disable => 'বন্ধ';

  @override
  String get do_not_show_again => 'আবার দেখাবেন না';

  @override
  String get force_notification_title =>
      'নামাজের বিজ্ঞপ্তি সঠিক সময়ে ট্রিগার করা হয় তা নিশ্চিত করতে মাওয়াকিত সক্রিয়।';

  @override
  String get force_notification_body =>
      'নিঃসন্দেহে সালাত মুমিনদের উপর নির্দিষ্ট সময়ে ফরজ করা হয়েছে। নিসা (103)';

  @override
  String get force_notification_dialog1_body =>
      'এই বিকল্পটি সক্রিয় করার মাধ্যমে, বিলম্ব না করে সঠিক সময়ে প্রার্থনা বিজ্ঞপ্তিগুলি ট্রিগার করা হয়েছে তা নিশ্চিত করতে অ্যাপ্লিকেশনটি সক্রিয় থাকবে। MAWAQIT অ্যাপ্লিকেশনটি শুধুমাত্র সেই উদ্দেশ্যে সক্রিয় থাকবে, এবং আপনি সেটিংস থেকে যে কোনো সময় সেই বিকল্পটি বন্ধ করতে পারেন।';

  @override
  String get force_notification_dialog2_body =>
      '\'ফোর্স নোটিফিকেশন\' সক্ষম করার পরে বিজ্ঞপ্তি ট্রে থেকে নিচের দিকে সোয়াইপ করুন আপনি সেখানে এই বিজ্ঞপ্তিটি দেখতে পাবেন';

  @override
  String get notification_debugging => 'বিজ্ঞপ্তি ডিবাগিং';

  @override
  String get notification_debugging_details =>
      'বিজ্ঞপ্তি ডিবাগিং সক্ষম করা হয়েছে৷ আপনি বিজ্ঞপ্তি সেটিংস পরিবর্তন করলে, ডিবাগিং ডেটা হারিয়ে যাবে, এবং ডিবাগিং অক্ষম করা হবে৷';

  @override
  String get skip => 'এড়িয়ে যান';

  @override
  String get agree => 'একমত';

  @override
  String get return_no_search => 'কোন অনুসন্ধান ফেরত';

  @override
  String get backend_environment_switch => 'ব্যাকএন্ড এনভায়রনমেন্ট সুইচ';

  @override
  String get set_development_environment =>
      'ডেভেলপমেন্ট এনভায়রনমেন্ট সেট করুন';

  @override
  String get set_production_environment => 'উত্পাদন পরিবেশ সেট করুন';

  @override
  String get delay_notification => 'বিলম্ব বিজ্ঞপ্তি';

  @override
  String get notification_delay_debugging_is => 'বিজ্ঞপ্তি বিলম্ব ডিবাগিং হয়';

  @override
  String get banner_enable => 'ব্যানার সক্ষম করুন';

  @override
  String get banner => 'ব্যানার';

  @override
  String get show_details => 'বিস্তারিত দেখাও';

  @override
  String get auto_detect_location => 'সাজেস্ট করুন সবচেয়ে কাছাকাছি মসজিদ';

  @override
  String get auto_detect_explanation =>
      'আপনি যখন আপনার মসজিদ থেকে দূরে থাকবেন, তখন অ্যাপ্লিকেশনটি আপনার অবস্থানের ভিত্তিতে স্বয়ংক্রিয়ভাবে সবচেয়ে কাছাকাছি মসজিদ অনুসারে অনুসারণের জন্য সুপারিশ করবে।';

  @override
  String get auto_detect_location_title =>
      'এটা মনে হচ্ছে যে, আপনি আপনার মসজিদ থেকে দূরে';

  @override
  String get auto_detect_location_message =>
      'আপনি কি সঠিক নামাজের সময়ের জন্য সবচেয়ে কাছাকাছি মসজিদ খোঁজার জন্য চান?';

  @override
  String get donation_success_toast => 'দান সফলভাবে সম্পন্ন হয়েছে';

  @override
  String get donation_success =>
      'আপনার দানের জন্য ধন্যবাদ, আল্লাহ আপনাকে প্রতিদান দিন';

  @override
  String get donation_failed => 'অনুদান ব্যর্থ হয়েছে';

  @override
  String get donate_for_good => 'ভালোর জন্য দান করুন';

  @override
  String get enter_amount_hint => 'পরিমান লিখুন';

  @override
  String get enter_donation_amount => 'অনুদানের পরিমাণ লিখুন';

  @override
  String get google_pay_not_available => 'এই ডিভাইসে Google Pay উপলব্ধ নেই';

  @override
  String get error_performing_donation =>
      'দান করার চেষ্টা করার সময় একটি ত্রুটি ছিল';

  @override
  String get recurring_pay_text => 'প্রতি মাসে এই দান করুন।';

  @override
  String get adjust_angles => 'কোণ সংশোধন করুন';

  @override
  String get silent => 'নীরব';

  @override
  String get default_ => 'ডিফল্ট';

  @override
  String get muazzin => 'মুয়াজ্জিন';

  @override
  String get information => 'তথ্য';

  @override
  String get news => 'সংবাদ';

  @override
  String get calendar => 'ক্যালেন্ডার';

  @override
  String get system => 'সিস্টেম';

  @override
  String get change_mosque_order =>
      'অর্ডার পরিবর্তন এবং প্রধান মসজিদ সেট করতে মসজিদগুলি টেনে আনুন এবং ছেড়ে দিন';

  @override
  String get remove => 'সংরক্ষণ';

  @override
  String get save => 'সেভ করুন';

  @override
  String get main_mosque => 'প্রধান মসজিদ';

  @override
  String get mosques => 'মসজিদগুলি';

  @override
  String get no_news => 'মসজিদ থেকে কোন খবর নেই';

  @override
  String get mawaqit_notification => 'মাওয়াকিত বিজ্ঞপ্তি';

  @override
  String get no_mawaqit_notification => 'মাওয়াকিত থেকে কোন বিজ্ঞপ্তি নেই';

  @override
  String get meccan => 'মক্কান';

  @override
  String get medinan => 'মদিনান';

  @override
  String get montada_islamic_foundation => 'মোনতাদা ইসলামিক ফাউন্ডেশন';

  @override
  String get rashid_maash => 'রশিদ মাআশ';

  @override
  String get abdul_haleem => 'আবদুল হালিম';

  @override
  String get dr_mustafa_khattab => 'ডাঃ মুস্তফা খত্তাব';

  @override
  String get frank_bubenheim_and_nadeem => 'ফ্রাঙ্ক বুবেনহেইম এবং নাদিম';

  @override
  String get abu_reda_muhammad_ibn_ahmad => 'আবু রেদা মুহাম্মদ ইবনে আহমদ';

  @override
  String get unknown_author => 'অজানা লেখক';

  @override
  String get translation_settings => 'অনুবাদ সেটিংস';

  @override
  String get reading => 'পড়া';

  @override
  String get listening => 'শুনা';

  @override
  String get font_size => 'ফন্ট সাইজ';

  @override
  String get font_size_arabic => 'আরবি ফন্ট সাইজ';

  @override
  String get horizontal => 'আনুভূমিক';

  @override
  String get vertical => 'উল্লম্ব';

  @override
  String get add_page_bookmarks => 'বুকমার্কে পাতা যোগ করুন';

  @override
  String get remove_page_bookmarks => 'বুকমার্ক থেকে পাতা সরান';

  @override
  String get no_bookmarks => 'কোন বুকমার্ক নেই';

  @override
  String get liked => 'পছন্দ হয়েছে';

  @override
  String get all_recitators => 'সমস্ত ক্বারী';

  @override
  String get downloaded => 'ডাউনলোড করা হয়েছে';

  @override
  String get search_for_recitator => 'ক্বারী সন্ধান করুন';

  @override
  String get search_for_surah => 'সূরা সন্ধান করুন';

  @override
  String get search_for_fav_recitator => 'পছন্দের ক্বারী সন্ধান করুন';

  @override
  String get listen_to_quran => 'কুরআন শুনুন';

  @override
  String get search_hint_text => 'একটি মসজিদ বা শহর খুঁজুন';

  @override
  String get downloading => 'ডাউনলোড হচ্ছে';

  @override
  String get cant_download_more_than_3 =>
      'একই সময়ে 3টির বেশি ডাউনলোড করা যাবে না।';

  @override
  String get boost => 'বুস্ট';

  @override
  String get recitation_deleted_successfully =>
      'তেলাওয়াত সফলভাবে মুছে ফেলা হয়েছে';

  @override
  String get quran => 'কুরআন';

  @override
  String get qibla => 'কিবলা';

  @override
  String get search => 'অনুসন্ধান';

  @override
  String get more => 'আরও';

  @override
  String get download_cancelled => 'ডাউনলোড বাতিল করা হয়েছে';

  @override
  String get no_virtual_mosque_found =>
      'দুঃখিত, আমরা আপনার আশেপাশে কোন মসজিদ খুঁজে পাইনি';

  @override
  String get no_surah_found => 'কোন সূরা পাওয়া যায়নি';

  @override
  String get no_favorite_recitor_found =>
      'কোন প্রিয় তেলাওয়াতকারী পাওয়া যায়নি';

  @override
  String get no_recitator_found => 'কোন তেলাওয়াতকারী পাওয়া যায়নি';

  @override
  String get empty_quran_search => 'অনুসন্ধানের ফলাফল এখানে প্রদর্শিত হবে';

  @override
  String get no_bookmark_ayah =>
      'কোনো বুকমার্ক উপলব্ধ নেই। দয়া করে আপনি যে আয়াতটি বুকমার্ক করতে চান সেটিতে দীর্ঘক্ষণ চাপ দিয়ে রাখুন।';

  @override
  String get page => 'পৃষ্ঠা';

  @override
  String get prayer_times_in => 'নামাজের সময়';

  @override
  String get update_new_version =>
      'কুরআনের একটি নতুন সংস্করণ ডাউনলোডের জন্য উপলভ্য। সর্বশেষ আপডেট পেতে ‘আপডেট’ চাপুন এবং আল্লাহর জ্ঞানের আলোয় আপনার যাত্রা চালিয়ে যান';

  @override
  String get update_quran_zip =>
      'কুরআনের একটি নতুন সংস্করণ ডাউনলোডের জন্য প্রস্তুত। সর্বশেষ আপডেট পেতে ‘আপডেট’ চাপুন এবং আল্লাহর জ্ঞানে আপনার যাত্রা অব্যাহত রাখুন।';

  @override
  String get update_latest_quran_version =>
      'কুরআনের একটি নতুন সংস্করণ ডাউনলোডের জন্য উপলভ্য। সর্বশেষ উন্নয়নগুলো পেতে ‘আপডেট’ চাপুন এবং আল্লাহর জ্ঞানের আলোয় আপনার যাত্রা চালিয়ে যান।.';

  @override
  String get update_latest_quran =>
      'কুরআনের একটি নতুন সংস্করণ ডাউনলোডের জন্য উপলভ্য। সর্বশেষ উন্নয়ন পেতে ‘আপডেট’ চাপুন এবং আল্লাহর জ্ঞানের পথে আপনার যাত্রা অব্যাহত রাখুন।';

  @override
  String get set_notification_for_all_parayer =>
      'প্রতিটি নামাজের জন্য এটি নির্ধারণ করুন';

  @override
  String get powered_by_mawaqit => 'MAWAQIT দ্বারা পরিচালিত';

  @override
  String get adjust_hijri_date => 'অবস্থান অনুযায়ী নামাজের সময়';

  @override
  String get prayer_times_by_location => 'অবস্থান অনুযায়ী নামাজের সময়';

  @override
  String get already_in_list => 'এটি আপনার তালিকায় ইতিমধ্যেই আছে!';

  @override
  String get got_it => 'বুঝেছি!';

  @override
  String get mushaf_warsh => 'ওয়ারশ';

  @override
  String get mushaf_hafs => 'হাফস';

  @override
  String get select_rewaya => 'রেওয়ায়া নির্বাচন করুন';

  @override
  String get download_in_progress =>
      'ডাউনলোড চলছে, অনুগ্রহ করে অপেক্ষা করুন...';

  @override
  String get highlight_next_prayer_after_Iqama =>
      'ইকামার পরে পরবর্তী নামাজ হাইলাইট করুন';

  @override
  String get helmi_nasr => 'ড. হেলমি নাসর';

  @override
  String get settings_language_pt => 'পর্তুগীজ';

  @override
  String get al_zikr => 'আল-যিকর';

  @override
  String get favorite_azkar => 'প্রিয় যিকির';

  @override
  String get all_azkar => 'সমস্ত যিকির';

  @override
  String get device_audio_not_found => 'ডিভাইসের অডিও পাওয়া যায়নি';

  @override
  String get device_media => 'ডিভাইস মিডিয়া';

  @override
  String get add_new_audio => 'নতুন অডিও ফাইল যোগ করুন';

  @override
  String get selecet_audio_before_delete =>
      'নির্বাচিত অডিও ফাইল মুছে ফেলার আগে আপনাকে নোটিফিকেশনের জন্য আরেকটি অডিও ফাইল যোগ করতে হবে';

  @override
  String get addding_file_prompt =>
      'আপনি একাধিক অডিও ফাইল যোগ করতে পারেন, তবে একই অডিও ফাইল দুইবার যোগ করবেন না দয়া করে।';

  @override
  String get ios_audio_file_picking_prompt =>
      'দয়া করে সর্বোচ্চ ৩০ সেকেন্ড বা তার কম সময়ের একটি অডিও ফাইল নির্বাচন করুন। বেশি দীর্ঘ অডিও ফাইল আপনার নামাজের নোটিফিকেশন বাধাগ্রস্থ করতে পারে';

  @override
  String get select_audio_from_device =>
      'ডিভাইস থেকে অডিও (এমপি৩) নির্বাচন করুন';

  @override
  String get add_ticket => 'টিকিট যোগ করুন';

  @override
  String get projects => 'প্রকল্পসমূহ';

  @override
  String get fill_form => 'আপনার প্রকল্প এখানে যোগ করতে এই';

  @override
  String get form => 'ফর্মটি';

  @override
  String get updates_draw =>
      'আপডেট পান এবং লটারিতে অংশ নিন পুরস্কার জেতার জন্য থেকে';

  @override
  String get email_info =>
      'আপনার যদি লটারি জিতে যান তবে এই ইমেইলটি আপনার সাথে যোগাযোগের জন্য ব্যবহৃত হবে';

  @override
  String get enter_email => 'আপনার ইমেইল লিখুন';

  @override
  String get email_privacy =>
      'MAWAQIT এই ইমেইলটি কারও সাথে ভাগ করবে না। প্রতিটি লটারির পরে এটি আমাদের ডাটাবেস থেকে স্বয়ংক্রিয়ভাবে মুছে ফেলা হবে';

  @override
  String get submit => 'জমা দিন';

  @override
  String get disclaimer =>
      'MAWAQIT তালিকাভুক্ত প্রকল্পগুলির প্রস্তাবিত পরিষেবা বা পণ্যগুলির বিষয়ে কোনো দায় নেয় না। অনুগ্রহ করে নিজের গবেষণা করুন';

  @override
  String get participating_with => 'আপনি অংশ নিচ্ছেন';

  @override
  String get no_of_tickets => 'টিকিটের সংখ্যা:';

  @override
  String get unparticipate => 'অংশগ্রহণ বাতিল করতে ক্লিক করুন';

  @override
  String get next_ticket => 'পরবর্তী টিকিট পাওয়া যাবে';

  @override
  String get click_here => 'এখানে ক্লিক করুন';

  @override
  String get mawaqit_store => 'MAWAQIT স্টোর';

  @override
  String get no_projects => 'কোন প্রকল্প নেই';

  @override
  String get draw_info =>
      'প্রতি মাসে একবার লটারি। প্রতি ২৪ ঘণ্টায় আপনি নতুন একটি টিকিট যোগ করতে পারেন আপনার জয়ের সুযোগ বাড়ানোর জন্য!';

  @override
  String get enter_valid_email => 'একটি বৈধ ইমেইল লিখুন';

  @override
  String get please_enter_email => 'অনুগ্রহ করে একটি ইমেইল লিখুন';

  @override
  String get select_device_audo_for_ios_propmpt =>
      'একটি অডিও ফাইল নির্বাচন করুন, আমরা সেটিকে iOS-এর সঙ্গে সামঞ্জস্যপূর্ণ ফরম্যাটে রূপান্তর করব এবং iOS প্ল্যাটফর্মের সীমাবদ্ধতার কারণে ৩০ সেকেন্ডে ট্রিম করব';

  @override
  String get confirm_deletion => 'মুছে ফেলা নিশ্চিত করুন';

  @override
  String get sure_remove_rewaya =>
      'আপনি কি নিশ্চিত যে আপনি এই রেওয়ায়া মুছে ফেলতে চান';

  @override
  String get file_not_found => 'ফাইল পাওয়া যায়নি';

  @override
  String get notified_boost =>
      'MAWAQIT Boost প্রকল্পের সর্বশেষ আপডেট সম্পর্কে জানতে নোটিফিকেশন পান';

  @override
  String get contact_us_boost =>
      'আপনার প্রকল্প এখানে তালিকাভুক্ত করতে, আমাদের সাথে যোগাযোগ করুন';

  @override
  String get prayer_times_set_by_mosque =>
      'নামাজের সময়সূচি এবং এখানে থাকা সব তথ্য মসজিদ কর্তৃক নির্ধারিত, MAWAQIT দ্বারা নয়।';

  @override
  String get hasan_nahi => 'হাসান নাহি';

  @override
  String get albanian => 'আলবেনীয়';

  @override
  String get delete_all => 'সব মুছে ফেলুন';

  @override
  String get delete_all_downloads => 'সব ডাউনলোড মুছে ফেলুন';

  @override
  String get delete_recitations_message =>
      'আপনি কি নিশ্চিত যে আপনি সমস্ত ডাউনলোডকৃত তেলাওয়াত মুছে ফেলতে চান?';

  @override
  String get downloaded_deleted_successfully =>
      'সমস্ত ডাউনলোডকৃত তেলাওয়াত সফলভাবে মুছে ফেলা হয়েছে।';

  @override
  String get failed_delete_recitations =>
      'সমস্ত তেলাওয়াত মুছে ফেলা ব্যর্থ হয়েছে';

  @override
  String get no_azkar_result => ' স্মরন করার মত কোনো  ফলাফল পাওয়া যায়নি';

  @override
  String get search_title => 'শিরোনাম ব্যবহার করে অনুসন্ধান করুন';

  @override
  String get compass_calibration =>
      'কম্পাস ক্যালিব্রেশন সেটিং নিষ্ক্রিয় রয়েছে। ফোনটি তিনবার এইভাবে আটের আকৃতিতে ঢালু করে এবং নাড়ান।';

  @override
  String get compass_accuracy => 'কম্পাসের নির্ভুলতা';

  @override
  String get low => 'লো';

  @override
  String get tafsir => 'তাফসির';

  @override
  String get copy => 'কপি করুন';

  @override
  String get no_tafsir => 'এখনও কোনো তাফসির ডাউনলোড করা হয়নি';

  @override
  String get download_tafsir => 'তাফসির ডাউনলোড করুন';

  @override
  String get hussary => 'মাহমুদ খালিল আল-হুসারী';

  @override
  String get hussary_mujawwad => 'হুসারী (মুজাওয়াদ)';

  @override
  String get shuraim => 'সাউদ আল-শুরাইম';

  @override
  String get arrafai => 'হানি আররাফাই';

  @override
  String get meaqli => 'মাহের আল-মেয়াকলি';

  @override
  String get afasy => 'মিশারি আল-আফাসি';

  @override
  String get minshawi => 'আল-মিনশাওয়ি';

  @override
  String get abdulbasit => 'আবদুলবাসিত আবদুসসামাদ';

  @override
  String get abdulbasit_mujawwad => 'আবদুলবাসিত (মুজাওয়াদ)';

  @override
  String get ghamdi => 'সাআদ আল-গামদি';

  @override
  String get dosari => 'ইয়াসির আল-দোসারি';

  @override
  String get shatri => 'আবু বকর আশ-শত্রি';

  @override
  String get tunaiji => 'খলিফা আল-তুনাইজি';

  @override
  String get sudais => 'আব্দুর রহমান আস-সুদাইস';

  @override
  String get tablawi => 'মোহাম্মদ আল-তাবলাওয়ি';

  @override
  String get abdul_rashid_ali_sufi => 'আবদুর রশীদ আলী সুফি';

  @override
  String get abdullaah_ajuhaynee_murattal => 'আবদুল্লাহ আজুহাইনি';

  @override
  String get abdullaah_basfar_murattal => 'আবদুল্লাহ বাসফার';

  @override
  String get abdullah_matroud_murattal => 'আবদুল্লাহ মাতরুদ';

  @override
  String get ahmed_al_ajamy_murattal => 'আহমাদ আল-আজামী';

  @override
  String get ahmed_neana_murattal => 'আহমদ নায়না';

  @override
  String get akram_alaqimy_murattal => 'আকরাম আল-আকিমি';

  @override
  String get ali_hajjaj_suesy_murattal => 'আলি হাজ্জাজ সুয়েসি';

  @override
  String get ali_jaber_murattal => 'আলি জাবের';

  @override
  String get ayman_sowaid_murattal => 'আয়মান সুলাইদ';

  @override
  String get aziz_alii_murattal => 'আজিজ আলী';

  @override
  String get fares_abbad_murattal => 'ফারেস আববাদ';

  @override
  String get hudhaify_murattal => 'হুঝাইফি';

  @override
  String get husary_muallim => 'হুসারি (শিক্ষক ধাঁচে)';

  @override
  String get ibrahim_akhdar_murattal => 'ইব্রাহিম আখদার';

  @override
  String get khaalid_qahataanee_murattal => 'খালিদ কাহতানী';

  @override
  String get khalid_al_mohanna => 'খালিদ আল-মুহান্না';

  @override
  String get mahmoud_ali_al_banna_murattal => 'মাহমুদ আলী আল-বান্না';

  @override
  String get minshawy_mujawwad => 'মিনশাওয়ি (মুজাওয়্বাদ)';

  @override
  String get mohammad_rashad_alshareef => 'মোহাম্মদ রাশাদ আশ-শরীফ';

  @override
  String get muhammad_abdulKareem_murattal => 'মুহাম্মদ আবদুল কারীম';

  @override
  String get muhammad_ayyoub_murattal => 'মুহাম্মদ আইয়ুব';

  @override
  String get muhammad_jibreel_murattal => 'মুহাম্মদ জিবরীল';

  @override
  String get muhsin_al_qasim_murattal => 'মুহসিন আল-কাসিম';

  @override
  String get nabil_rifai3_murattal => 'নাবিল রিফাঈ';

  @override
  String get nasser_alqatami_murattal => 'নাসের আল-কাতামি';

  @override
  String get sahl_yassin_murattal => 'সাহেল ইয়াসিন';

  @override
  String get salaah_abdulRahman_bukhatir_murattal =>
      'সালাহ আবদুর রহমান বুখাতির';

  @override
  String get salah_albudair_murattal => 'সালাহ আল-বুদাইর';

  @override
  String get tawfeeq_assayegh => 'তৌফিক আস-সাইঘ';

  @override
  String get yaser_salamah_murattal => 'ইয়াসির সালামাহ';

  @override
  String get yassin_aljazairi => 'ইয়াসিন আল-জাযাইরি';

  @override
  String get mohammed_abdelkarim => 'মোহাম্মদ আব্দেলকারিম';

  @override
  String get alayoun_elkouchi => 'আল-আয়ুন এল-কুশি';

  @override
  String get abdelkarim_daghoush => 'আবদেলকারিম দাগ্শ';

  @override
  String get murattal => 'মুরাত্তাল';

  @override
  String get mujawwad => 'মুজাওয়াদ';

  @override
  String get moallim => 'আল-মুসহাফ আল-মু‘াল্লিম';

  @override
  String get play => 'চালান';

  @override
  String get playback_settings => 'প্লেব্যাক সেটিংস';

  @override
  String get from => 'থেকে';

  @override
  String get to => 'প্রতি';

  @override
  String get range => 'পরিসর';

  @override
  String get select_starting_verse => 'শুরু আয়াত নির্বাচন করুন';

  @override
  String get select_ending_verse => 'শেষ আয়াত নির্বাচন করুন';

  @override
  String get range_repeat => 'পরিসর পুনরাবৃত্তি';

  @override
  String get verse_repeat => 'আয়াত পুনরাবৃত্তি';

  @override
  String get improved_Hafs_Rewaya_Force_Update =>
      'আমরা আপনার হাফস রেওয়ায়ার উন্নতি করেছি। এখনই ডাউনলোড করুন, যাতে যেকোনো আয়াত দীর্ঘক্ষণ চেপে ধরে তাফসির দেখতে এবং তিলাওয়াত শুনতে পারেন।';

  @override
  String get mawaqit_three_sixty_description =>
      'আমাদের নতুন অ্যানিমেটেড ঘড়ির মাধ্যমে নামাজের সময়গুলোকে জীবন্তভাবে দেখুন';

  @override
  String get hnpai_User_Guidance_MAWAQIT_360 =>
      '৩৬০ ভিউ ব্যবহার করার সময় আপনি এই সেটিংটি বন্ধ করতে পারবেন না';

  @override
  String get mawaqit_three_sixty_beta => 'MAWAQIT 360 (বেটা)';

  @override
  String get three_sixty_setting => '৩৬০ সেটিংস';

  @override
  String get behavior => 'আচরণ';

  @override
  String get next_prayer_open => 'পরবর্তী নামাজ';

  @override
  String get all_prayer_closed => 'সব বন্ধ';

  @override
  String get recomended_next_prayer => 'প্রস্তাবিত: শুধুমাত্র পরবর্তী নামাজ';

  @override
  String get live_preview => 'সরাসরি পূর্বদর্শন';

  @override
  String get font_Size_Normal => 'স্বাভাবিক';

  @override
  String get font_Size_Large => 'বড়';

  @override
  String get all_Prayer_Open => 'সব খোলা';

  @override
  String get settingFontSize => 'Font Size';

  @override
  String get last_Third_Of_The_Night => 'রাতের শেষ তৃতীয়াংশ';

  @override
  String get shaykh_aminuddin_muhammad => 'শেখ আমিনুদ্দিন মুহাম্মদ';

  @override
  String get hafs_an_asim => 'হাফস আন আসিম';

  @override
  String get most_widespread_riwayah =>
      'বিশ্বব্যাপী সর্বাধিক প্রচলিত রিওয়ায়াহ';

  @override
  String get warsh_an_nafi => 'ওয়ারশ আন নাফি';

  @override
  String get popular_across_north_west_africa =>
      'উত্তর ও পশ্চিম আফ্রিকা জুড়ে জনপ্রিয়';

  @override
  String get select => 'নির্বাচন করুন';

  @override
  String get unknown_smartwatch => 'অজানা স্মার্টওয়াচ';

  @override
  String get connected_watch => 'সংযুক্ত ঘড়ি';

  @override
  String get ready_play_watch => 'আপনার ঘড়িতে চালানোর জন্য প্রস্তুত';

  @override
  String get transliteration => 'ধ্বনিলিপি';

  @override
  String get highlight_Tahajjud_Time => 'তাহাজ্জুদ সময় হাইলাইট করুন';

  @override
  String get calendar_access_not_available =>
      'ক্যালেন্ডার অ্যাক্সেস উপলব্ধ নয়।';

  @override
  String get calendar_allow_access_mawaqit =>
      'MAWAQIT 360-এ আপনার ইভেন্ট দেখানোর জন্য অনুগ্রহ করে ক্যালেন্ডার অ্যাক্সেসের অনুমতি দিন।';

  @override
  String get calendar_access_required =>
      'আপনার ইভেন্ট দেখানোর জন্য ক্যালেন্ডার অ্যাক্সেস প্রয়োজন।';

  @override
  String get calendar_allow_access_button =>
      'ক্যালেন্ডার অ্যাক্সেস অনুমোদন করুন';

  @override
  String get calendar_loading => 'ক্যালেন্ডার লোড হচ্ছে…';

  @override
  String get calendar_select_for_mawaqit =>
      'যে ক্যালেন্ডারগুলি আপনি MAWAQIT 360-এ দেখাতে চান সেগুলি নির্বাচন করুন।';

  @override
  String get calendar_enable_all => 'সব সক্ষম করুন';

  @override
  String get calendar_disable_all => 'সব অক্ষম করুন';

  @override
  String get calendar_other_calendars => 'অন্যান্য ক্যালেন্ডার';

  @override
  String get calendar_times_shown_in => 'সময় দেখানো হয়েছে';

  @override
  String get calendar_starts_on => 'শুরু হয়';

  @override
  String get calendar_ends_on => 'শেষ হয়';

  @override
  String get calendar_events_at_this_time => 'এই সময়ের ইভেন্টসমূহ';

  @override
  String get hamza_roberto_piccardo => 'হামজা রবের্তো পিকারদো';

  @override
  String get open_in_mawaqit_app => 'এই মসজিদটি MAWAQIT অ্যাপে খুলুন';

  @override
  String get mosque_name => 'মসজিদের নাম';

  @override
  String get mosque_not_found => 'মসজিদ খুঁজে পাওয়া যায়নি';

  @override
  String get invalid_mosque_link => 'অবৈধ মসজিদ লিংক';

  @override
  String get not_open_mosque_link =>
      'এই মসজিদ লিংকটি খোলা যায়নি। অনুগ্রহ করে আবার চেষ্টা করুন।';

  @override
  String get no_calendars_on_this_device =>
      'এই ডিভাইসে কোনো ক্যালেন্ডার পাওয়া যায়নি।';

  @override
  String get calendar_event_at_this_time => 'এই সময়ের ইভেন্ট';

  @override
  String get search_ayah_surah => 'আয়াত বা সূরা দিয়ে অনুসন্ধান করুন';

  @override
  String get matching_surahs => 'মিলছে এমন সূরাগুলো';

  @override
  String get matching_ayahs => 'মিলছে এমন আয়াতগুলো';

  @override
  String get live_timer => 'লাইভ টাইমার';

  @override
  String get notify_before => 'আগে থেকে জানান';

  @override
  String get take_time_appear_watch =>
      'আপনার ঘড়িতে দেখাতে কয়েক সেকেন্ড সময় লাগতে পারে।';

  @override
  String get eid1 => 'ঈদ';

  @override
  String get hide_live_activity_for_prayer =>
      'এই নামাজের জন্য শুধুমাত্র লাইভ অ্যাক্টিভিটি লুকান';

  @override
  String get settings => 'সেটিংস';

  @override
  String get azkar => 'আযকার';

  @override
  String get quran_tajweed => 'কুরআন তাজবীদ।';

  @override
  String get tajweed_description =>
      'সঠিক তিলাওয়াতের জন্য রঙবিন্যাসকৃত তাজভীদের নিয়ম';

  @override
  String get tajwid_rules => 'তাজভীদের নিয়ম';

  @override
  String get examples => 'উদাহরণসমূহ';

  @override
  String get real_time_weather => 'রিয়েল-টাইম আবহাওয়া';

  @override
  String get layout => 'লেআউট';

  @override
  String get list => 'তালিকা';

  @override
  String get grid => 'গ্রিড';

  @override
  String get real_time_weather_needs_internet =>
      'রিয়েল-টাইম আবহাওয়ার আপডেট পেতে ইন্টারনেট সংযোগ প্রয়োজন।.';

  @override
  String get view_detail => 'বিস্তারিত দেখুন';

  @override
  String get temperature => 'তাপমাত্রা';

  @override
  String get tahajjud_time => 'তাহাজ্জুদের সময়';

  @override
  String get last_third => 'শেষ তৃতীয়াংশ';

  @override
  String get midnight => 'মধ্যরাত্রি';

  @override
  String get failed_to_update => 'আপডেট করতে ব্যর্থ হয়েছে।';

  @override
  String get tap_to_retry => 'পুনরায় চেষ্টা করতে ট্যাপ করুন';

  @override
  String get support_mawaqit_association => 'MAWAQIT সমিতিকে সমর্থন করুন';

  @override
  String get help_improve_mawaqit => 'MAWAQIT উন্নত করতে সহায়তা করুন';

  @override
  String get analytics_description =>
      'অ্যাপ উন্নত করতে আমাদেরকে নামবিহীন ব্যবহার বিশ্লেষণ করার অনুমতি দিন। কোনো ব্যক্তিগত তথ্য নেওয়া হবে না। কখনও কারো সাথে শেয়ার করা হবে না।';

  @override
  String get weather_clear => 'পরিষ্কার';

  @override
  String get weather_clouds => 'মেঘলা';

  @override
  String get weather_tornado => 'ঘূর্ণিঝড়';

  @override
  String get weather_ash => 'ছাই';

  @override
  String get weather_dust => 'ধুলা';

  @override
  String get weather_sand => 'বালু';

  @override
  String get weather_fog => 'কুয়াশা';

  @override
  String get weather_haze => 'হালকা কুয়াশা';

  @override
  String get weather_smoke => 'ধোঁয়া';

  @override
  String get weather_mist => 'মিস্ট (মৃদু কুয়াশা)';

  @override
  String get weather_snow => 'তুষার';

  @override
  String get weather_rain => 'বৃষ্টি';

  @override
  String get weather_drizzle => 'গুঁড়ি গুঁড়ি বৃষ্টি';

  @override
  String get weather_thunderstorm => 'বজ্রঝড়';

  @override
  String get weather_squall => 'ঝড়ো হাওয়া';

  @override
  String get weather_celsius => '°সে.';

  @override
  String get animated_background => 'অ্যানিমেটেড পটভূমি';

  @override
  String get weather => 'আবহাওয়া';

  @override
  String get weather_need_internet =>
      'আপডেট ডাউনলোড করতে এই ফিচারটির ইন্টারনেট সংযোগ প্রয়োজন।';

  @override
  String get weather_light_rain => 'হালকা বৃষ্টি';

  @override
  String get edit => 'সম্পাদনা করুন';

  @override
  String get iqama_countdown => 'ইকামা কাউন্টডাউন';

  @override
  String get next_prayer_countdown => 'পরবর্তী নামাজের কাউন্টডাউন';

  @override
  String get next_prayer => 'পরবর্তী নামাজ/ সালাত ';

  @override
  String get live_countdown_for_all_pray =>
      'লাইভ কাউন্টডাউন সব নামাজের জন্য সক্রিয় থাকবে এবং প্রতিটি আসন্ন নামাজের জন্য স্বয়ংক্রিয়ভাবে একটি টাইমার দেখাবে।';

  @override
  String get live_countdown => 'লাইভ কাউন্টডাউন';

  @override
  String get quick_quran_reading_access => 'ত্বরিত কুরআন পাঠের প্রবেশাধিকার';

  @override
  String get quick_quran_discription =>
      'আপনি যখন কুরআন ট্যাব খুলবেন, তখন যেখানে শেষ করেছেন সেখান থেকেই শুরু করতে পারবেন।';

  @override
  String get live_activity_tooltip =>
      'লাইভ অ্যাক্টিভিটিগুলো আপনার লক স্ক্রিন ও ডাইনামিক আইল্যান্ডে (যদি সমর্থিত হয়) প্রদর্শিত হবে। চালু করলে, লাইভ টাইমার পরবর্তী নামাজ বা ইকামার কাউন্টডাউন দেখাবে।';

  @override
  String get prayer => 'নামাজ';

  @override
  String get weather_fahrenheit => '°ফা.';

  @override
  String get weather_your_location =>
      'আপনার অবস্থানের জন্য আবহাওয়া দেখানো হচ্ছে, মসজিদের জন্য নয়।';

  @override
  String get view_360_currently_selected => '৩৬০ ভিউ বর্তমানে নির্বাচিত হয়েছে';

  @override
  String get tap_change_layout => 'বিন্যাস পরিবর্তন করতে এখানে ট্যাপ করুন';

  @override
  String get step => 'ধাপ';

  @override
  String get jump_to_page => 'পৃষ্ঠায় যান';

  @override
  String get select_surah => 'সূরা নির্বাচন করুন';

  @override
  String get please_select_surah => 'অনুগ্রহ করে একটি সূরা নির্বাচন করুন';

  @override
  String get ayah_number => 'আয়াত নম্বর';

  @override
  String get ayah_num_greater_zero => 'আয়াত নম্বর ০-এর বেশি হতে হবে';

  @override
  String get ayah_number_exceeds_max =>
      'এই সূরার জন্য আয়াত নম্বর সর্বোচ্চ সীমা অতিক্রম করেছে';

  @override
  String get update_page => 'পৃষ্ঠা আপডেট করুন';

  @override
  String get enter_ayah_num => 'আয়াত নম্বর লিখুন';

  @override
  String get help => 'সাহায্য';

  @override
  String get sofian_s_siregar => 'সোফিয়ান এস. সিরেগার';

  @override
  String get download_all_surahs => 'সব সূরা ডাউনলোড করুন';

  @override
  String get this_will_download_all_surahs_for_offline =>
      'এটি অফলাইনে শোনার জন্য সব সূরা ডাউনলোড করবে। চালিয়ে যাওয়ার আগে দয়া করে নিশ্চিত করুন যে আপনি Wi-Fi-এ সংযুক্ত আছেন।.';

  @override
  String get tap_change_notification => 'নোটিফিকেশন পরিবর্তন করতে ট্যাপ করুন';

  @override
  String get adhan_at => 'আজান এ';

  @override
  String get semantic_mawaqit_notification => 'MAWAQIT থেকে অপঠিত নোটিফিকেশন';

  @override
  String get semantic_open_notifications => 'নোটিফিকেশন খুলুন';

  @override
  String get semantic_tap_to_change_notification =>
      'নোটিফিকেশন সেটিংস পরিবর্তন করতে ট্যাপ করুন';

  @override
  String get semantic_edit_mosques => 'আপনার মসজিদ সম্পাদনা করতে খুলুন';

  @override
  String get semantic_change_layout => 'হোম লেআউট পরিবর্তন করুন';

  @override
  String get semantic_open_layout_menu => 'লেআউট মেনু খুলুন';

  @override
  String get semantic_edit_home_settings => 'হোম সেটিংস সম্পাদনা করুন';

  @override
  String get semantic_list_layout => 'তালিকা লেআউট';

  @override
  String get semantic_grid_layout => 'গ্রিড লেআউট';

  @override
  String get sematnic_360_clock_layout => '৩৬০ ঘড়ির লেআউট';

  @override
  String get semantic_layout_menu_open => 'লেআউট মেনু খোলা হয়েছে';

  @override
  String get close => 'বন্ধ করুন';

  @override
  String get semantic_mosque_page => 'মসজিদ পৃষ্ঠা';

  @override
  String get semantic_of => 'এর মধ্যে';

  @override
  String get semantic_unread_mawaqit_notifications =>
      'MAWAQIT-এর অপঠিত নোটিফিকেশন';

  @override
  String get back => 'ফিরে যান';

  @override
  String get semantic_download_all_surahs_for_offline_listening =>
      'অফলাইনে শোনার জন্য সব সূরা ডাউনলোড করুন';

  @override
  String get semantic_download_all_surahs => 'সব সূরা ডাউনলোড করুন';

  @override
  String get semantic_bulk_download_progress => 'একসাথে ডাউনলোডের অগ্রগতি';

  @override
  String get semantic_surahs_downloaded => 'ডাউনলোড করা সূরা';

  @override
  String get semantic_cancel_download_all_surahs =>
      'সব সূরা ডাউনলোড বাতিল করুন';

  @override
  String get semantic_bulk_download_in_progress => 'একসাথে ডাউনলোড চলছে।';

  @override
  String get semantic_replay_current_surah => 'বর্তমান সূরা আবার চালান';

  @override
  String get semantic_pause_current_surah => 'বর্তমান সূরা বিরতি দিন';

  @override
  String get semantic_play_current_surah => 'বর্তমান সূরা চালান';

  @override
  String get semantic_open_play_on_watch_options_for =>
      'ঘড়িতে চালানোর বিকল্প খুলুন';

  @override
  String get semantic_now_playing_surah => 'এখন চলছে। সূরা';

  @override
  String get semantic_recited_by => 'তিলাওয়াত করেছেন';

  @override
  String get semantic_minimize_player => 'প্লেয়ার ছোট করুন';

  @override
  String get semantic_elapsed => 'অতিবাহিত';

  @override
  String get semantic_playback_time_elapsed => 'প্লেব্যাক সময়। অতিবাহিত';

  @override
  String get semantic_remaining => 'বাকি';

  @override
  String get semantic_turn_shuffle_off => 'শাফল বন্ধ করুন';

  @override
  String get semantic_turn_shuffle_on => 'শাফল চালু করুন';

  @override
  String get semantic_previous_surah => 'পূর্ববর্তী সূরা';

  @override
  String get semantic_next_surah => 'পরবর্তী সূরা';

  @override
  String get semantic_turn_repeat_current_surah_off =>
      'বর্তমান সূরা পুনরাবৃত্তি বন্ধ করুন';

  @override
  String get semantic_turn_repeat_current_surah_on =>
      'বর্তমান সূরা পুনরাবৃত্তি চালু করুন';

  @override
  String get semantic_pause => 'বিরতি';

  @override
  String get semantic_use_current_location => 'বর্তমান অবস্থান ব্যবহার করুন';

  @override
  String get semantic_collapse_mosque_list => 'মসজিদের তালিকা সংকুচিত করুন';

  @override
  String get semantic_expand_mosque_list => 'মসজিদের তালিকা প্রসারিত করুন';

  @override
  String get added => 'যোগ করা হয়েছে';

  @override
  String get not_added => 'যোগ করা হয়নি';

  @override
  String get semantic_navigate_to_mosque => 'মসজিদে নেভিগেট করুন';

  @override
  String get semantic_decrease_pre_notification => 'পূর্ব-নোটিফিকেশন সময় কমান';

  @override
  String get semantic_increase_pre_notification =>
      'পূর্ব-নোটিফিকেশন সময় বাড়ান';

  @override
  String get semantic_facing_qibla => 'আপনি কিবলার দিকে মুখ করে আছেন';

  @override
  String get semantic_clockwise => 'ঘড়ির কাঁটার দিকে';

  @override
  String get sematnic_counter_clockwise => 'ঘড়ির কাঁটার বিপরীত দিকে';

  @override
  String get semantic_rotate_slightly => 'প্রায় পৌঁছে গেছেন, একটু ঘুরুন';

  @override
  String get semanitc_degrees_to_qibla => 'কিবলার দিকে ডিগ্রি';

  @override
  String get about => 'প্রায়';

  @override
  String get rotate => 'ঘোরান';

  @override
  String get semantic_close_audio_player => 'অডিও প্লেয়ার বন্ধ করুন';

  @override
  String get reciter_settings => 'তিলাওয়াতকারীর সেটিংস';

  @override
  String get previous_verse => 'পূর্ববর্তী আয়াত';

  @override
  String get next_verse => 'পরবর্তী আয়াত';

  @override
  String get decrease => 'কমান';

  @override
  String get increase => 'বাড়ান';

  @override
  String get quran_settings => 'কুরআনের সেটিংস';

  @override
  String get semantic_mushaf_view => 'মুসহাফ ভিউ';

  @override
  String get semantic_verse_list_view => 'আয়াত তালিকা ভিউ';

  @override
  String get not_downloaded => 'ডাউনলোড করা হয়নি';

  @override
  String get verse_list_enabled_for_accessibility =>
      'অ্যাক্সেসিবিলিটির জন্য আয়াত তালিকা মোড সক্রিয় করা হয়েছে';

  @override
  String get semantic_switch_verse_list =>
      'মুসহাফ পৃষ্ঠা। সম্পূর্ণ অ্যাক্সেসিবিলিটির জন্য আয়াত তালিকা ভিউতে পরিবর্তন করুন';

  @override
  String get semantic_decrease_hijri_adjustment => 'হিজরি সমন্বয় কমান';

  @override
  String get semantic_minimum_adjustment_reached =>
      'সর্বনিম্ন সমন্বয় পৌঁছানো হয়েছে';

  @override
  String get semantic_current_adjustment => 'বর্তমান সমন্বয়';

  @override
  String get semantic_increase_hijri_adjustment => 'হিজরি সমন্বয় বাড়ান';

  @override
  String get semantic_maximum_adjustment_reached =>
      'সর্বোচ্চ সমন্বয় পৌঁছানো হয়েছে';

  @override
  String get semantic_minimum_size_reached => 'সর্বনিম্ন আকার পৌঁছানো হয়েছে';

  @override
  String get semantic_decrease => 'কমান';

  @override
  String get semantic_current_size => 'বর্তমান আকার';

  @override
  String get semantic_increase => 'বাড়ান';

  @override
  String get semantic_maximum_size_reached => 'সর্বোচ্চ আকার পৌঁছানো হয়েছে';

  @override
  String get semantic_favorite => 'পছন্দের';

  @override
  String get semantic_added_to_favorites => 'পছন্দের তালিকায় যোগ করা হয়েছে';

  @override
  String get semantic_not_in_favorites => 'পছন্দের তালিকায় নেই';

  @override
  String get semantic_for => 'জন্য';

  @override
  String get semantic_view_style => 'ভিউ স্টাইল';

  @override
  String get semantic_double_tap_to_change_view_style =>
      'ভিউ স্টাইল পরিবর্তন করতে ডাবল ট্যাপ করুন';

  @override
  String get semantic_share_dua => 'দোয়া শেয়ার করুন';

  @override
  String get semantic_tasbih_settings => 'তাসবিহ সেটিংস';

  @override
  String get semantic_tasbih_counter => 'তাসবিহ কাউন্টার';

  @override
  String get semantic_previous_dua => 'পূর্ববর্তী দোয়া';

  @override
  String get semantic_next_dua => 'পরবর্তী দোয়া';

  @override
  String get semantic_dua => 'দোয়া';

  @override
  String get semantic_opens => 'খোলে';

  @override
  String get semantic_opens_language_selection => 'ভাষা নির্বাচন খোলে';

  @override
  String get semantic_device => 'ডিভাইস';

  @override
  String get semantic_light => 'লাইট';

  @override
  String get semantic_dark => 'ডার্ক';

  @override
  String get semantic_on => 'চালু';

  @override
  String get semantic_off => 'বন্ধ';

  @override
  String get semantic_ayah_list_actions => 'বুকমার্ক সরান';

  @override
  String get remove_bookmark => 'বুকমার্ক সরান';

  @override
  String get add_bookmark => 'বুকমার্ক যোগ করুন';

  @override
  String get selected => 'নির্বাচিত';

  @override
  String get double_tap_to_continue => 'চালিয়ে যেতে ডাবল ট্যাপ করুন';

  @override
  String get clear_search => 'অনুসন্ধান পরিষ্কার করুন';

  @override
  String get mosques_found => 'মসজিদ পাওয়া গেছে';

  @override
  String get mosque_added => 'মসজিদ যোগ করা হয়েছে';

  @override
  String get mosque_removed => 'মসজিদ সরানো হয়েছে';

  @override
  String get scroll_left_right_select_surah =>
      'একটি সূরা নির্বাচন করতে বামে বা ডানে স্ক্রল করুন';

  @override
  String get swipe_left_right_select_mushaf_type =>
      'ভিন্ন মুসহাফ ধরন নির্বাচন করতে বামে বা ডানে সোয়াইপ করুন';

  @override
  String get location_permission_required => 'অবস্থান অনুমতি প্রয়োজন';

  @override
  String get semantic_previous_month => 'পূর্ববর্তী মাস';

  @override
  String get semantic_next_month => 'পরবর্তী মাস';

  @override
  String get semantic_view_image => 'ছবি দেখুন';

  @override
  String get access_location_details =>
      'আপনার বর্তমান অবস্থানের ভিত্তিতে সঠিক স্থানীয় আবহাওয়া, কিবলার দিক এবং মসজিদের তথ্য দেখানোর জন্য MAWAQIT আপনার অবস্থানে অ্যাক্সেস প্রয়োজন। আপনি পূর্বে এই অনুমতিটি প্রত্যাখ্যান করেছেন।';

  @override
  String get location_off => 'অবস্থান বন্ধ';

  @override
  String get semantic_pre_notification_time => 'পূর্ব-নোটিফিকেশন সময়';

  @override
  String get search_for_mosque_or_enable_location =>
      'অবস্থান অ্যাক্সেস অস্বীকার করা হয়েছে, তাই আমরা নিকটস্থ মসজিদের পরামর্শ দিতে পারছি না। অনুগ্রহ করে একটি মসজিদ অনুসন্ধান করুন অথবা অবস্থানের অনুমতি চালু করুন।';

  @override
  String get location => 'লোকেশন';

  @override
  String get stop => 'বন্ধ করুন';

  @override
  String get plays_adhan_prayer_arrives => 'নামাজের সময় হলে আযান বাজায়';

  @override
  String get play_in_silent_mode => 'সাইলেন্ট মোডে চালান';

  @override
  String get notifications_not_arriving => 'নোটিফিকেশন আসছে না?';

  @override
  String get android_phones_aggressively =>
      'কিছু অ্যান্ড্রয়েড ফোন ব্যাকগ্রাউন্ডে অ্যাপগুলো আক্রমণাত্মকভাবে বন্ধ করে দেয়।.';

  @override
  String get elmir_kuliev => 'এলমির কুলিয়েভ';

  @override
  String get besim_korkut => 'বেসিম কোরকুত';

  @override
  String get settings_language_bs => 'বসনীয়';

  @override
  String get something_wrong_getting_location =>
      'আপনার অবস্থান পাওয়ার সময় কিছু ভুল হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।.';

  @override
  String get custom_volume => 'কাস্টম ভলিউম';

  @override
  String get general => 'সাধারণ';

  @override
  String get prayer_location => 'নামাজ ও লোকেশন';

  @override
  String get privacy => 'গোপনীয়তা';

  @override
  String get play_full_adhan => 'সম্পূর্ণ আজান চালান';

  @override
  String get full_adhan_will_keep_repeating =>
      'সম্পূর্ণ আজান আপনি বন্ধ না করা পর্যন্ত বারবার চলতে থাকবে।.';

  @override
  String get hussein_taji_kal_dari => 'হুসেইন তাজি কাল দারি';

  @override
  String get settings_language_fa => 'ফারসি';

  @override
  String get mawaqit_non_profit =>
      'MAWAQIT organizatë jofitimprurëse · falas për të gjithë';

  @override
  String get garmin_watch_detected => 'Garmin ঘড়ি শনাক্ত হয়েছে';

  @override
  String get garmin_support_detected =>
      'আপনার ডিভাইসে Garmin সমর্থন শনাক্ত হয়েছে। আপনি কি আপনার নামাজের সময়সূচি Garmin ঘড়ির সাথে সিঙ্ক করতে চান?';

  @override
  String get never_remind_me => 'আর মনে করিয়ে দেবেন না';

  @override
  String get remind_me_later => 'পরে মনে করিয়ে দিন';

  @override
  String get you_can_enable_garmin_sync =>
      'আপনি যেকোনো সময় সেটিংস → অপেক্ষমাণ কার্যক্রম থেকে Garmin সিঙ্ক চালু করতে পারবেন।';

  @override
  String get pending_actions => 'অপেক্ষমাণ কার্যক্রম';

  @override
  String get setup_garmin_watch => 'Garmin ঘড়ি সেট আপ করুন';

  @override
  String get sync_prayer_times_garmin =>
      'নামাজের সময়সূচি Garmin ঘড়ির সাথে সিঙ্ক করুন';

  @override
  String get setup_garmin => 'Garmin সেট আপ করুন';

  @override
  String get open_garmin_device_selection => 'Garmin ডিভাইস নির্বাচন খুলুন';

  @override
  String get admin_adminTitle => 'মসজিদ ব্যবস্থাপনা';

  @override
  String get admin_adminTileSubtitle => 'ঘোষণা, নামাজের সেটিংস ও আযান';

  @override
  String get admin_signInHeadline => 'আপনার মসজিদ পরিচালনা করুন';

  @override
  String get admin_signInIntro =>
      'আপনি যেসব মসজিদের দায়িত্বে আছেন সেগুলো পরিচালনা করতে mawaqit.net-এ ব্যবহৃত অ্যাকাউন্ট দিয়ে সাইন ইন করুন।';

  @override
  String get admin_capabilityMessages => 'ঘোষণা ও দ্রুত বার্তা প্রকাশ করা';

  @override
  String get admin_capabilityTimes => 'ইকামত, জুমা ও ঈদের সময় নির্ধারণ করা';

  @override
  String get admin_capabilityAdhan => 'আযান ও তা বাজানোর ধরন বেছে নেওয়া';

  @override
  String get admin_notAnAdminTitle => 'এখনও অ্যাডমিন নন?';

  @override
  String get admin_notAnAdminBody =>
      'মসজিদ MAWAQIT ওয়েবসাইটে নিবন্ধন করা হয়, যেখানে অ্যাকাউন্ট সক্রিয় করার আগে টিম তা যাচাই করে।';

  @override
  String get admin_registerMosque => 'মসজিদ নিবন্ধন করুন';

  @override
  String get admin_registerUnavailable => 'ওয়েবসাইট খোলা যায়নি।';

  @override
  String get admin_signIn => 'সাইন ইন';

  @override
  String get admin_signInAgain => 'আবার সাইন ইন করুন';

  @override
  String get admin_sessionExpired => 'আপনার সেশন শেষ হয়ে গেছে।';

  @override
  String get admin_myMosques => 'আমার মসজিদ';

  @override
  String get admin_logOut => 'লগ আউট';

  @override
  String get admin_noMosques => 'কোনো মসজিদ পাওয়া যায়নি।';

  @override
  String get admin_archivedNotConfigurable =>
      'এই মসজিদটি আর্কাইভ করা — এর সেটিংস বদলানো যাবে না।';

  @override
  String admin_mosqueId(String id) {
    return 'ID $id';
  }

  @override
  String get admin_statusArchived => 'আর্কাইভ করা';

  @override
  String get admin_openMessages => 'বার্তা';

  @override
  String get admin_openConfiguration => 'কনফিগারেশন';

  @override
  String get admin_configurationTitle => 'কনফিগারেশন';

  @override
  String get admin_configurationSaved => 'কনফিগারেশন সংরক্ষিত হয়েছে';

  @override
  String get admin_oneUnsavedChange => '1টি অসংরক্ষিত পরিবর্তন';

  @override
  String get admin_sectionRegional => 'আঞ্চলিক সেটিংস';

  @override
  String get admin_sectionAdhan => 'আযান';

  @override
  String get admin_sectionIqama => 'ইকামত';

  @override
  String get admin_sectionJumua => 'জুমা';

  @override
  String get admin_sectionPrayerDuration => 'নামাজের সময়কাল';

  @override
  String get admin_sectionDuas => 'দোয়া ও হাদিস';

  @override
  String get admin_sectionEid => 'ঈদ ও রমজান';

  @override
  String get admin_sectionNotYet => 'অ্যাপে এখনও সম্পাদনা করা যায় না';

  @override
  String get admin_iqamaEnabled => 'ইকামত চালু';

  @override
  String get admin_iqamaEnabledHelp =>
      'এটি বন্ধ করলে মসজিদের স্ক্রিনে অপেক্ষার সময়, ইকামতের সংকেত ও নামাজের পরের দোয়াগুলোও দেখা যাবে না।';

  @override
  String get admin_iqamaWaitingTimes => 'আযানের পর অপেক্ষা (মিনিট)';

  @override
  String get admin_iqamaSignal => 'ইকামতের সংকেত';

  @override
  String get admin_iqamaSound => 'শব্দ';

  @override
  String get admin_iqamaCountdown => 'পূর্ণ স্ক্রিনে গণনা';

  @override
  String get admin_iqamaDisplayTime => 'সংকেত দেখানোর সময়';

  @override
  String get admin_iqamaAlwaysHhMm =>
      'ইকামতের সময় সবসময় ঘণ্টা:মিনিট হিসেবে দেখান';

  @override
  String get admin_iqamaCalendarNote =>
      'ক্যালেন্ডার দিয়ে নির্ধারিত ইকামতের সময় অপরিবর্তিত থাকে — সেগুলো ওয়েবসাইটে সম্পাদনা করুন।';

  @override
  String get admin_eidPrayerTimes => 'ঈদের নামাজের সময়';

  @override
  String get admin_ramadanImsak => 'রমজান';

  @override
  String get admin_eidPrayerHelp =>
      'ঈদের আগের সপ্তাহে মসজিদের স্ক্রিনে দেখানো হয়। যে নামাজ হবে না তা সময় ছাড়া রেখে দিন।';

  @override
  String get admin_imsakBeforeFajr => 'ফজরের আগে ইমসাক দেখান';

  @override
  String get admin_imsakHelp =>
      'ইমসাক কত আগে দেখা যাবে। ফজর 04:00 এবং এখানে 30 মিনিট হলে স্ক্রিনে 03:30 দেখাবে।';

  @override
  String get admin_imsakOff => 'বন্ধ';

  @override
  String get admin_eidNoPrayerTime => 'কোনো সময় নির্ধারণ করা হয়নি';

  @override
  String get admin_adhanSoundLabel => 'শব্দ';

  @override
  String get admin_adhanPlaysFor => 'যেসব ওয়াক্তে বাজে';

  @override
  String get admin_adhanDurationLabel => 'আযানের দৈর্ঘ্য';

  @override
  String get admin_adhanDurationHelp =>
      'এই শব্দটি আযান শেষ হওয়ার আগেই থেমে যায়, তাই আপনার মুয়াজ্জিনের কত সময় লাগে তা স্ক্রিনের জানা দরকার।';

  @override
  String admin_customAdhanFile(String name) {
    return 'আপলোড করা ফাইল: $name';
  }

  @override
  String get admin_customAdhanMissing =>
      'এখনও কোনো ফাইল আপলোড করা হয়নি — ওয়েবসাইটে একটি যোগ করুন।';

  @override
  String get admin_adhanNoPrayers => 'কোনো নামাজ নেই';

  @override
  String get admin_prayerDurationGroup => 'প্রতি নামাজে মিনিট';

  @override
  String get admin_prayerDurationHelp =>
      'নামাজের সময়ের কালো স্ক্রিন ও পরের দোয়াগুলো এতক্ষণ চলে।';

  @override
  String get admin_duasInvocations => 'দোয়া';

  @override
  String get admin_duasHadiths => 'হাদিস';

  @override
  String get admin_duaAfterAdhan => 'আযানের পর দোয়া';

  @override
  String get admin_duaAfterPrayer => 'নামাজের পর দোয়া';

  @override
  String get admin_randomHadith => 'প্রতি 5 মিনিটে একটি হাদিস দেখান';

  @override
  String get admin_hadithSource => 'ইমাম নববীর রিয়াদুস সালিহীন থেকে।';

  @override
  String get admin_hadithPause => 'যার মাঝে বিরতি';

  @override
  String get admin_hadithPauseNever => 'কখনও নয়';

  @override
  String get admin_hadithPauseHelp =>
      'যে মসজিদে দুই নামাজের মাঝে দরস হয় তার জন্য।';

  @override
  String get admin_duasAllOff => 'সব বন্ধ';

  @override
  String get admin_summaryDuaAdhan => 'আযানের পর';

  @override
  String get admin_summaryDuaPrayer => 'নামাজের পর';

  @override
  String get admin_summaryHadiths => 'হাদিস';

  @override
  String get admin_regionalHijri => 'হিজরি তারিখ';

  @override
  String get admin_regionalMosqueScreen => 'মসজিদের স্ক্রিন';

  @override
  String get admin_regionalRegion => 'অঞ্চল';

  @override
  String get admin_timezoneLabel => 'সময় অঞ্চল (ওয়েবসাইটে নির্ধারিত)';

  @override
  String get admin_timezoneUnknown => 'অজানা';

  @override
  String get admin_dstLabel => 'গ্রীষ্মকালীন সময়';

  @override
  String get admin_dstDatesNote =>
      'গ্রীষ্ম ও শীতকালীন সময় বদলের তারিখ অপরিবর্তিত থাকে — সেগুলো ওয়েবসাইটে নির্ধারণ করুন।';

  @override
  String get admin_hijriAdjustmentLabel => 'তারিখ সমন্বয়';

  @override
  String get admin_jumuaTimeGroup => 'জুমার সময়';

  @override
  String get admin_jumuaDuringGroup => 'জুমার সময়ে';

  @override
  String get admin_jumuaSameAsDuhr => 'দুহরের সময়েই';

  @override
  String get admin_jumuaFollowsDuhr =>
      'দুহরের সময়ে হয় — আপনার নিজের সময় সংরক্ষিত থাকে।';

  @override
  String get admin_jumuaApplyDst => 'গ্রীষ্মকালীন সময়ে এক ঘণ্টা যোগ করুন';

  @override
  String get admin_jumuaDuration => 'সময়কাল (খুতবা + নামাজ)';

  @override
  String get admin_jumuaDurationHelp =>
      'জুমার সময়ে যা কিছু দেখানো হয় তা এতক্ষণ চলে।';

  @override
  String get admin_jumuaBlackScreen => 'জুমার সময়ে কালো স্ক্রিন';

  @override
  String get admin_jumuaDhikr => 'জুমার আদব নিয়ে একটি হাদিস দেখান';

  @override
  String get admin_jumuaNoTime => 'কোনো সময় নির্ধারণ করা হয়নি';

  @override
  String get admin_summaryAsDuhr => 'দুহর';

  @override
  String get admin_timeNotSet => 'নির্ধারণ করা হয়নি';

  @override
  String get admin_clearTime => 'মুছুন';

  @override
  String get admin_flashContent => 'বিষয়বস্তু';

  @override
  String get admin_flashContentHint => 'মসজিদের স্ক্রিনে দেখানো বার্তা';

  @override
  String get admin_flashOrientation => 'দিকনির্দেশ';

  @override
  String get admin_flashColor => 'রঙ';

  @override
  String get admin_flashStartDate => 'শুরুর তারিখ';

  @override
  String get admin_flashExpireDate => 'শেষের তারিখ';

  @override
  String get admin_flashTooShort => 'কমপক্ষে 30টি অক্ষর';

  @override
  String get admin_flashSaved => 'দ্রুত বার্তা সংরক্ষিত হয়েছে';

  @override
  String get admin_flashSaving => 'সংরক্ষণ করা হচ্ছে…';

  @override
  String get admin_flashDelete => 'মুছুন';

  @override
  String get admin_flashDeleteTitle => 'এই দ্রুত বার্তাটি মুছে ফেলবেন?';

  @override
  String get admin_deleteBody =>
      'এটি আর মসজিদের স্ক্রিনে দেখা যাবে না। এটি ফেরানো যাবে না।';

  @override
  String get admin_flashDeleted => 'দ্রুত বার্তা মুছে ফেলা হয়েছে';

  @override
  String get admin_tabAnnouncements => 'ঘোষণা';

  @override
  String get admin_tabFlash => 'দ্রুত বার্তা';

  @override
  String get admin_announcementsEmpty => 'এখনও কোনো ঘোষণা নেই।';

  @override
  String get admin_announcementNew => 'নতুন ঘোষণা';

  @override
  String get admin_announcementEdit => 'ঘোষণা সম্পাদনা';

  @override
  String get admin_announcementSaved => 'ঘোষণা সংরক্ষিত হয়েছে';

  @override
  String get admin_announcementDeleted => 'ঘোষণা মুছে ফেলা হয়েছে';

  @override
  String get admin_announcementDeleteTitle => 'এই ঘোষণাটি মুছে ফেলবেন?';

  @override
  String get admin_announcementLimitReached =>
      'আপনি সক্রিয় ঘোষণার সীমায় পৌঁছে গেছেন।';

  @override
  String get admin_discardTitle => 'পরিবর্তনগুলো বাতিল করবেন?';

  @override
  String get admin_discardBody => 'আপনি এখানে যা লিখেছেন তা সংরক্ষিত হবে না।';

  @override
  String get admin_discard => 'বাতিল করুন';

  @override
  String get admin_keepEditing => 'সম্পাদনা চালিয়ে যান';

  @override
  String get admin_chipScreen => 'স্ক্রিন';

  @override
  String get admin_chipMobile => 'মোবাইল';

  @override
  String get admin_statusEnabled => 'চালু';

  @override
  String get admin_statusDisabled => 'বন্ধ';

  @override
  String get admin_typeText => 'লেখা';

  @override
  String get admin_fieldTitle => 'শিরোনাম';

  @override
  String get admin_fieldMessage => 'বার্তা';

  @override
  String get admin_fieldVideo => 'ইউটিউব লিঙ্ক';

  @override
  String get admin_fieldVideoHint => 'https://www.youtube.com/watch?v=…';

  @override
  String get admin_fieldVideoHelp =>
      'শুধু ইউটিউব ভিডিও — শর্টস গ্রহণ করা হয় না।';

  @override
  String get admin_fieldOrientation => 'স্ক্রিনের দিকনির্দেশ';

  @override
  String get admin_fieldDuration => 'সময়কাল';

  @override
  String get admin_fieldIsEvent => 'এটি একটি অনুষ্ঠান';

  @override
  String admin_fieldStartsAt(String date) {
    return 'শুরু $date';
  }

  @override
  String admin_fieldEndsAt(String date) {
    return 'শেষ $date';
  }

  @override
  String get admin_fieldShowOn => 'যেখানে দেখাবে';

  @override
  String get admin_fieldMainScreen => 'মসজিদের প্রধান স্ক্রিন';

  @override
  String get admin_fieldSecondaryScreen => 'দ্বিতীয় স্ক্রিন';

  @override
  String get admin_fieldMobile => 'মাওয়াকিত অ্যাপ';

  @override
  String get admin_pickImage => 'একটি ছবি বেছে নিন';

  @override
  String get admin_replaceImage => 'ছবি বদলান';

  @override
  String get admin_orientationLandscape => 'আড়াআড়ি';

  @override
  String get admin_orientationPortrait => 'লম্বালম্বি';

  @override
  String get admin_requiredField => 'আবশ্যক';

  @override
  String get admin_invalidYoutube => 'একটি ইউটিউব ভিডিও লিঙ্ক দিন।';

  @override
  String get admin_flashUnavailable =>
      'দ্রুত বার্তা সম্পাদনা এখন করা যাচ্ছে না।';

  @override
  String admin_unsavedChanges(int count) {
    return 'অসংরক্ষিত পরিবর্তন: $count';
  }

  @override
  String get admin_prayerSlot1 => 'প্রথম নামাজ';

  @override
  String get admin_prayerSlot2 => 'দ্বিতীয় নামাজ';

  @override
  String get admin_prayerSlot3 => 'তৃতীয় নামাজ';

  @override
  String admin_minutes(int count) {
    return '$count মিনিট';
  }

  @override
  String admin_seconds(int count) {
    return '$count সেকেন্ড';
  }

  @override
  String admin_announcementCounts(
    int total,
    int maxTotal,
    int enabled,
    int maxEnabled,
  ) {
    return '$maxTotal-এর মধ্যে $total · $maxEnabled-এর মধ্যে $enabled সক্রিয়';
  }

  @override
  String get beta => 'বিটা';

  @override
  String get for_mosque_administrators_only =>
      'শুধুমাত্র মসজিদের প্রশাসকদের জন্য';

  @override
  String get admins_only => 'শুধু অ্যাডমিন';
}
